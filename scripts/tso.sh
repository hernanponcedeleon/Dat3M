...
for METHOD in ${METHODS[@]}; do
    ...
    for BENCHMARK in ${BENCHMARKS[@]}; do
        ...
        OUTPUT=$(timeout $TIMEOUT java -Xmx2048m -jar dartagnan/target/dartagnan-3.0.0.jar cat/$CAT --bound=2 --target=tso --method=$MOPT --solver=$SOLVER $BPL_PATH$BENCHMARK.bpl)
