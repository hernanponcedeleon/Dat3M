...
declare -a BENCHMARKS=( "locks/clh_mutex" 
                        "locks/cna"
                        "locks/linuxrwlock"
                        "locks/mutex_musl"
                        "locks/mutex"
                        "locks/spinlock"
                        "locks/ticket_awnsb_mutex"
                        "locks/ticketlock"
                        "locks/ttas"
                        "lfds/chase-lev"
                        "lfds/dglm"
...
for METHOD in ${METHODS[@]}; do
    for XRA_OPT in "${XRA_OPTS[@]}"; do
        ...
        ## Start CSV files
        echo benchmark, threads, may-size, must-size, act-size, smt-vars, acyc-size, result, ra_time, xra_time, veri_time > $DAT3M_OUTPUT/csv/$TARGET-$TOOL-C11.csv
        ...
        for BENCHMARK in ${BENCHMARKS[@]}; do
            ...
            case "$BENCHMARK" in
                "locks/ttas" | "locks/ticketlock" | "locks/spinlock" | "locks/clh_mutex")
                    THREADS=6 ;;
                "locks/linuxrwlock" | "lfds/chase-lev" | "locks/ticket_awnsb_mutex")
                    THREADS=5 ;;
                "locks/mutex" | "locks/mutex_musl" | "lfds/treiber" | "locks/cna")
                    THREADS=4 ;;
                "lfds/dglm" | "lfds/ms")
                    THREADS=3 ;;
                *)
            ...
            export CFLAGS="-I$DAT3M_HOME/include -DNTHREADS="$THREADS
            ...
            ## Save CSV
            echo $BENCHMARK, $THREADS, $MAY_SET, $MUST_SET, $ACT_SET, $SMT_VARS, $ACYC, $RESULT, $RA_TIME, $XRA_TIME, $VERI_TIME >> $DAT3M_OUTPUT/csv/$TARGET-$TOOL-C11.csv
