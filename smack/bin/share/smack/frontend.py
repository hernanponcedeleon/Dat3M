...
def default_clang_compile_command(args, lib = False):
  cmd = ['clang-3.9', '-c', '-emit-llvm', '-O0', '-g', '-gcolumn-info']
  ...
  with open(input_file) as f:
    for cc in json.load(f):
      if 'objects' in cc:
        ...
        try_command(['llvm-link-3.9', '-o', args.bc_file] + bit_codes)
        try_command(['llvm-link-3.9', '-o', args.linked_bc_file, args.bc_file] + build_libs(args))
...
def link_bc_files(bitcodes, libs, args):
  ...
  try_command(['llvm-link-3.9', '-o', args.bc_file] + bitcodes)
  try_command(['llvm-link-3.9', '-o', args.linked_bc_file, args.bc_file] + smack_libs)
