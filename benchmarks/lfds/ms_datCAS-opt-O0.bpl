...
procedure  enqueue($val: i32)
{
  var $tail_addr: ref; // &tail: Node**
  var $next_addr: ref; // &next: Node**
  var $node: ref; // node: Node*
  var $node.next: ref; // node->next: Node*
  var $tail.next: ref; // tail->next: Node*
  var $tailEqTail: bool; // tail == Tail
  var $nextNotNull: bool; // next != NULL
  var $casSucc: bool;
...
$bb1:
  ...
  call $temp := atomic_load_explicit.ref.i32(Tail, 2);
...
$bb5:
  ...
  call $temp := __DAT3M_CAS.ref.ref.ref.i32(Tail, $tail, $next, 4); // CAS(&Tail, &tail, next)
  goto $bb1; // Loop
$bb6:
  assume !($nextNotNull == true); // CASE: next == NULL;
  call $temp:= __DAT3M_CAS.ref.ref.ref.i32($tail.next, $next, $node, 4); // CAS(&tail->next, &next, node)
...
$bb9:
  ...
  call $temp := __DAT3M_CAS.ref.ref.ref.i32(Tail, $tail, $node, 4); // CAS(&Tail, &tail, node)
  return;
}
...
axiom (__DAT3M_CAS == $sub.ref(0, 14448));
...
$bb9:
  ...
  call $temp := __DAT3M_CAS.ref.ref.ref.i32(Head, $head, $next, 4); // CAS(&Head, &head, next)
  $casSucc := $temp != 0; // CAS result
  goto $bb11, $bb12;
$bb11:
  assume !($casSucc  == true); // CASE: CAS fail
