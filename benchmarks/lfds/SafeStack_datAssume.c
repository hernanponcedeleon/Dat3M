...
SafeStackItem array[3];
    ...
    int pop() 
    {
        while (atomic_load_explicit(&count, memory_order_acquire) > 1) {
            ...
            if (next1 >= 0) {
                if (__DAT3M_CAS(&head, head1, next1, memory_order_seq_cst)) {
    ...
    void push(int index) 
    {
        int head1;
        do {
            head1 = atomic_load_explicit(&head, memory_order_acquire);
            atomic_store_explicit(&array[index].next, head1, memory_order_release);
        } while (__DAT3M_CAS(&head, head1, index, memory_order_seq_cst) == 0);
