// clspv MP-bug-2.cl --cl-std=CL2.0 --inline-entry-points --spv-version=1.6 -o kernel.spv
// spirv-opt --upgrade-memory-model kernel.spv -o kernel-opt.spv
// spirv-dis kernel-opt.spv > MP-bug-2.spv.dis
...
__kernel void test(global atomic_uint* flag, global uint* data, global uint* r0) {
    ...
    if (group_id != num_groups - 1) { // group 0
    ...
    if (group_id != 0) { // group 1
        // The WG waits the previous WG to be ready
        while (atomic_load_explicit(&flag[group_id - 1], memory_order_acquire) == 0);
