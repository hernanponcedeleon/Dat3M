#include <assert.h>

typedef struct {
    int a __attribute__ ((aligned (8)));
    int b __attribute__ ((aligned (8)));
    int c __attribute__ ((aligned (16)));
    int d __attribute__ ((aligned (16)));
} aligned_t;

typedef struct {
    int a;
    int b;
    int c;
    int d;
} unaligned_t;

void test(int *r_aligned, int* r_unaligned, aligned_t* aligned, unaligned_t* unaligned) {
    aligned[0].a = 0;
    aligned[0].b = 1;
    aligned[0].c = 2;
    aligned[0].d = 3;

    aligned[1].a = 4;
    aligned[1].b = 5;
    aligned[1].c = 6;
    aligned[1].d = 7;

    unaligned[0].a = 8;
    unaligned[0].b = 9;
    unaligned[0].c = 10;
    unaligned[0].d = 11;

    unaligned[1].a = 12;
    unaligned[1].b = 13;
    unaligned[1].c = 14;
    unaligned[1].d = 15;

    for (int i = 0; i < 24; i++) {
        r_aligned[i] = *(((int*) aligned) + i);
        r_unaligned[i] = *(((int*) unaligned) + i);
    }
}

int main() {
    int r_aligned[24];
    int r_unaligned[24];
    aligned_t aligned[3];
    unaligned_t unaligned[3];

    test(r_aligned, r_unaligned, aligned, unaligned);

    assert(r_aligned[0] == 0);
    assert(r_aligned[2] == 1);
    assert(r_aligned[4] == 2);
    assert(r_aligned[8] == 3);

    assert(r_aligned[12] == 4);
    assert(r_aligned[14] == 5);
    assert(r_aligned[16] == 6);
    assert(r_aligned[20] == 7);

    assert(r_unaligned[0] == 8);
    assert(r_unaligned[1] == 9);
    assert(r_unaligned[2] == 10);
    assert(r_unaligned[3] == 11);

    assert(r_unaligned[4] == 12);
    assert(r_unaligned[5] == 13);
    assert(r_unaligned[6] == 14);
    assert(r_unaligned[7] == 15);

    return 0;
}
