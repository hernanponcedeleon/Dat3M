	...
	public static void compile(File file, SVCOMPOptions opt, boolean ownAtomics) {
    	...
    	if(ownAtomics) {
        	cmd.add("--clang-options=-DCUSTOM_VERIFIER_ASSERT -" + opt.getOptimization() + 
        			" -fno-vectorize -fno-slp-vectorize -I" + System.getenv().get("PLDI_HOME") + "/include/");    		    		
    	} else {
        	cmd.add("--clang-options=-DCUSTOM_VERIFIER_ASSERT -" + opt.getOptimization() + 
        			" -fno-vectorize -fno-slp-vectorize");    		
    	}
    	cmd.addAll(asList("-bpl", System.getenv().get("PLDI_HOME") + "/output/" + name + "-" + opt.getOptimization() + ".bpl"));
