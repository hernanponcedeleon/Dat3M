...
do
   ...
   java -Xmx128m dartagnan/Dartagnan -t tso -i $file -cat cat/tso.cat -idl > ./dart_tso.out
   dart=$(grep -e 'No' dart_tso.out | wc -l)
   rm dart_tso.out
   if [ $dart != $herd ]
   then
   ...
   java -Xmx128m dartagnan/Dartagnan -t tso -i $file -cat cat/tso.cat -relax > ./dart_tso.out
   dart=$(grep -e 'No' dart_tso.out | wc -l)
   rm dart_tso.out
   if [ $dart != $herd ]
   then
