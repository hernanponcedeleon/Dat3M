...
public class GroupOp extends GenericVisibleEvent implements RegWriter, RegReader {
    ...
    public GroupOp(String name, BoolBinaryOp op, String scope, Register register, Expression expr) {
        super(name);
        Preconditions.checkArgument(BoolBinaryOp.AND == op || BoolBinaryOp.OR == op);
    ...
    public String defaultString() {
        return String.format("%s <- group(%s, %s, %s)", register, name, op, expr);
    }
    ...
    public BooleanFormula encodeExec(EncodingContext ctx) {
        ...
        BooleanFormula value = op == BoolBinaryOp.AND ? bmgr.and(conditions) : bmgr.or(conditions);
