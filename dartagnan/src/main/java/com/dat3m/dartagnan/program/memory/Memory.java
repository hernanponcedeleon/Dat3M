...
public class Memory {
    ...
    public BoolExpr encode(Context ctx){
        ...
        for(List<Address> array : arrays.values()){
            int size = array.size();
            Expr e1 = array.get(0).toZ3NumExpr(ctx);
            if(!array.get(0).hasConstValue()) {
                expressions.add(e1);            	
            }

            for(int i = 1; i < size; i++){
                Expr e2 = array.get(i).toZ3NumExpr(ctx);
                Expr newAddress = USEBV ? ctx.mkBVAdd((BitVecExpr)e1, ctx.mkBV(1, 32)) : ctx.mkAdd((IntExpr)e1, ctx.mkInt(1));
