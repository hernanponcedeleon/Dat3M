...
public class Memory {
    ...
    public MemoryObject allocate(int size) {
        ...
        final MemoryObject memoryObject = new MemoryObject(nextIndex++, size, null);
    ...
    public MemoryObject allocate(Alloc allocationSite) {
        ...
        final MemoryObject memoryObject = new MemoryObject(nextIndex++, size, allocationSite);
        objects.add(memoryObject);
        return memoryObject;
    }
    ...
    public VirtualMemoryObject allocateVirtual(int size, boolean generic, VirtualMemoryObject alias) {
        Preconditions.checkArgument(size > 0, "Illegal allocation. Size must be positive");
        final VirtualMemoryObject address = new VirtualMemoryObject(nextIndex++, size, generic, alias);
        objects.add(address);
        return address;
    }
