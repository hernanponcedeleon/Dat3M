...
public class RegisterDecomposition implements FunctionProcessor {
    ...
    public void run(Function function) {
        ...
        for (RegWriter writer : function.getEvents(RegWriter.class)) {
            if (writer instanceof LlvmCmpXchg || !(writer.getResultRegister().getType() instanceof AggregateType)) {
                continue;
            }

            if (!(writer instanceof Local local && local.getExpr() instanceof ConstructExpr construct)) {
            ...
            for (int i = 0; i < arguments.size(); i++) {
                ...
                // FIXME: This might fail on non-SSA code because we could generate multiple identical
                //  RegisterIndex objects for different local assignments.
                final var index = new RegisterIndex(local.getResultRegister(), i);
                assert !transformer.map.containsKey(index);
            ...
            componentAssignments.forEach(e -> e.copyAllMetadataFrom(local));
        }
