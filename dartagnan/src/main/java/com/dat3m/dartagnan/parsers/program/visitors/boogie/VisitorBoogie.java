...
public class VisitorBoogie extends BoogieBaseVisitor<Object> implements BoogieVisitor<Object> {
	        ...
	        if(register != null){
	        	if(ctx.getText().contains("$load.") || value instanceof Address) {
	        		...
	        		if(!allocationRegs.contains(value)) {
	        			child = new Load(register, (IExpr)value, null, currentLine);
	        		} else {
	        			child = new Load(register, (IExpr)value, null);
	        		}
	        		String region;
	        		if(ctx.getText().contains("load.")) {
	        			region = ctx.def_body().exprs().expr(0).getText();
	        			region = region.substring(region.indexOf("(") + 1, region.indexOf(","));
	        		} else {
	        			region = ctx.getText().split(":=")[1];
	        			region = tmp.substring(0, region.length() - 1);
	        		}
	...
	public Object visitFun_expr(Fun_exprContext ctx) {
		...
		if(name.contains("$store.")) {
			...
			child.setRegion(ctx.expr(0).getText());
			programBuilder.addChild(threadCount, child);	
			return null;
		}
