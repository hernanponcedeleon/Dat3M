...
public class InputBuilder {
    ...
    private static Expression castExpression(String id, Type type, Expression value) {
        ...
        if (value instanceof ConstructExpr aValue) {
            ...
            if (expectedSize == -1 || expectedSize == actualSize) {
                ...
                for (int i = 0; i < actualSize; i++) {
                    elements.add(castExpression(String.format("%s[%d]", id, i), elementType, aValue.getOperands().get(i)));
                }
    ...
    private static Expression castAggregate(String id, AggregateType type, Expression value) {
        if (value instanceof ConstructExpr aValue) {
            ...
            if (expectedSize == actualSize) {
                List<Expression> elements = new ArrayList<>();
                for (int i = 0; i < actualSize; i++) {
                    elements.add(castExpression(String.format("%s[%d]", id, i), type.getFields().get(i).type(), aValue.getOperands().get(i)));
                }
