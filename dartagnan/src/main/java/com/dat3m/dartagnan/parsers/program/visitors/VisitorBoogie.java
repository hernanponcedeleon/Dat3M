...
public class VisitorBoogie extends BoogieBaseVisitor<Object> implements BoogieVisitor<Object> {
	...
	public Object visitAssume_cmd(BoogieParser.Assume_cmdContext ctx) {
		...
		if(!ctx.proposition().expr().getText().equals("true")) {
			// This might not work for the general Boogie case with non deterministic choice in gotos with 
			// several branches, but it should work for the case of code generated by SMACK where gotos 
			// have at most two branches and in this case the expressions in the assumes are b and ~b
			if(pairLabels.keySet().contains(currentLabel)) {
				...
				if(c != null) {
					programBuilder.addChild(threadCount, new CondJump(new BExprUn(NOT, c), pairLabels.get(currentLabel)));	
				}
			}
		}
        return null;
