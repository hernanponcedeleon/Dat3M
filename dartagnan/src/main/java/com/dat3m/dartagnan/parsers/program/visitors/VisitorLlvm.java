...
public class VisitorLlvm extends LLVMIRBaseVisitor<Expression> {
    ...
    public Expression visitCallInst(CallInstContext ctx) {
        ...
        if (ctx.inlineAsm() != null) {
            ...
            if (resultRegister != null) {
                ...
                // System.out.println(ctx.inlineAsm().getText());
                // System.out.println(ctx.inlineAsm().StringLit(0).getText() + ", " + ctx.inlineAsm().StringLit(1).getText());
                inlineAArch64Wrapper(ctx.inlineAsm().StringLit(0).getText()+ ","+ ctx.inlineAsm().StringLit(1).getText()); // ctx.StringLit(0).getText() + ", " + ctx.StringLit(1).getText() 
                // save into list and programBuilder.addEvent/Child() for each event generated by parser
    ...
    public Expression visitInlineAsm(InlineAsmContext ctx) {
        ...
        final LinkedList<GenericVisibleEvent> fences = new LinkedList<>();
        switch(asm) {
            ...
            default -> fences.addAll(inlineAArch64Wrapper(ctx.StringLit(0).getText()+ ctx.StringLit(1).getText()));//throw new ParsingException(String.format("Encountered unsupported inline assembly:  %s.", asm)); HAS TO BECOME INLINEWRAPPER
        }
        if(!fences.isEmpty()) {
    ...
    public List<GenericVisibleEvent> inlineAArch64Wrapper(String inlineAsm){
        List<GenericVisibleEvent> events = new LinkedList();
        // I have to push all events into this one and return it
        // remove cleaning
        // String cleanedString = inlineAsm.substring(inlineAsm.indexOf("\""));
        // logger.error((String.format("Parsing this one : %s ", cleanedString)));
        CharStream charStream = CharStreams.fromString(inlineAsm);
        ParserInlineAArch64 parser = new ParserInlineAArch64();
