...
final class EncodeSets implements Visitor<Map<Relation, MutableEventGraph>> {

    private final RelationAnalysis ra;
    MutableEventGraph news;
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitDefinition(Definition def) {
        return Map.of();
    }

    @Override
    public Map<Relation, MutableEventGraph> visitUnion(Union union) {
        final Map<Relation, MutableEventGraph> m = new HashMap<>();
        for (Relation r : union.getOperands()) {
            m.merge(r, filterUnknowns(news, r), MutableEventGraph::union);
        }
        return m;
    }

    @Override
    public Map<Relation, MutableEventGraph> visitIntersection(Intersection inter) {
        final Map<Relation, MutableEventGraph> m = new HashMap<>();
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitDifference(Difference diff) {
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitComposition(Composition comp) {
        if (news.isEmpty()) {
            return Map.of();
        }
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitDomainIdentity(DomainIdentity domId) {
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitRangeIdentity(RangeIdentity rangeId) {
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitInverse(Inverse inv) {
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitTransitiveClosure(TransitiveClosure trans) {
    ...
    @Override
    public Map<Relation, MutableEventGraph> visitLinuxCriticalSections(LinuxCriticalSections rscs) {
    ...
    private MutableEventGraph filterUnknowns(MutableEventGraph news, Relation relation) {
        RelationAnalysis.Knowledge k = ra.getKnowledge(relation);
        EventGraph may = k.getMaySet();
        EventGraph must = k.getMustSet();
        return MapEventGraph.from(news.filter((e1, e2) -> may.contains(e1, e2) && !must.contains(e1, e2)));
    }

    private static Map<Relation, MutableEventGraph> map(Relation r1, MutableEventGraph s1, Relation r2, MutableEventGraph s2) {
        return r1.equals(r2) ? Map.of(r1, MutableEventGraph.union(s1, s2)) : Map.of(r1, s1, r2, s2);
    }
}
