...
public class Utils {
	...
	public static BooleanFormula edge(String relName, Event e1, Event e2, SolverContext ctx) {
		return ctx.getFormulaManager().getBooleanFormulaManager().makeVariable(relName + "(" + e1.repr() + "," + e2.repr() + ")");
	}
	...
	public static IntegerFormula intVar(String relName, Event e, SolverContext ctx) {
		return ctx.getFormulaManager().getIntegerFormulaManager().makeVariable(relName + "(" + e.repr() + ")");
	}
	
	public static IntegerFormula intCount(String relName, Event e1, Event e2, SolverContext ctx) {
		return ctx.getFormulaManager().getIntegerFormulaManager().makeVariable(relName + "(" + e1.repr() + "," + e2.repr() + ")");
	}
}