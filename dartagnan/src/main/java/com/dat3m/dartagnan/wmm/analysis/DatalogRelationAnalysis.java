...
import static com.dat3m.dartagnan.wmm.RelationNameRepository.RMW;
...
public class DatalogRelationAnalysis implements RelationAnalysis {
    ...
    public void run() {
        ...
        for (Relation r : memoryModel.getRelations()) {
            logger.info("relation " + r.getName() + " -> ");
    ...
    private class DefinitionToNameVisitor implements Definition.Visitor<StringBuilder> {
        ...
        public StringBuilder visitDomainIdentity(DomainIdentity def) {
            name.append("domain_");
            def.getOperand().getDefinition().accept(this);
            return name;
        }
        ...
        public StringBuilder visitSetIdentity(SetIdentity def) {
            return name.append("identity_").append(def.getFilter().getName());
        }

        @Override
        public StringBuilder visitProduct(CartesianProduct def) {
            return name.append("product_").append(def.getFirstFilter().getName()).append("_").append(def.getSecondFilter().getName());
        }

        @Override
        public StringBuilder visitFences(Fences fence) {
            return name.append("fence_").append(fence.getFilter().getName());
        }

        @Override
        public StringBuilder visitFree(Free def) {
            return name.append("free");
        }
        ...
        public StringBuilder visitSyncBarrier(SyncBar sync_bar) {
            return name.append(SYNC_BARRIER);
        }

        @Override
        public StringBuilder visitSyncWith(SyncWith sync_with) {
            return name.append(SYNC_FENCE);
        }
