    ...
    public TupleSet getMaxTupleSet(){
        if(maxTupleSet == null){
            ...
            for(Thread thread : task.getProgram().getThreads()){
                for(Event lock : thread.getCache().getEvents(FilterBasic.get(Tag.RCU_LOCK))){
                    for(Event unlock : thread.getCache().getEvents(FilterBasic.get(Tag.RCU_UNLOCK))){
                ...
                for(Event unlock : thread.getCache().getEvents(FilterBasic.get(Tag.RCU_UNLOCK))){
                    if(lock.getCId() < unlock.getCId()){
                        ...
                        if(encodeTupleSet.contains(tuple)){
                            ...
                            for(Event otherLock : thread.getCache().getEvents(FilterBasic.get(Tag.RCU_LOCK))){
                            ...
                            for(Event otherUnlock : thread.getCache().getEvents(FilterBasic.get(Tag.RCU_UNLOCK))){
