...
//
//        Expr body2 = ctx.mkEq(PredicateUtils.getEdge(name, e1, e2, ctx), ctx.mkOr(ctx.mkEq(e1, e2), PredicateUtils.getEdge(r1.getName(), e1, e2, ctx), ctx.mkExists(evts3, body, 0, null, null, null, null)));
...
//        opt1 = ctx.mkAnd(opt1, ctx.mkGt((IntExpr) PredicateUtils.getBinaryInt(String.format("(%s^+;%s^+)", r1.getName(), r1.getName()), ctx).apply(e1, e2), (IntExpr) PredicateUtils.getBinaryInt(r1.getName(), ctx).apply(e1, e3)));
...
//
//        BoolExpr body2 = ctx.mkEq(PredicateUtils.getEdge(this.getName(), e1, e2, ctx), ctx.mkOr(ctx.mkEq(e1, e2), ctx.mkOr(fromr1, fromconcat)));
//
