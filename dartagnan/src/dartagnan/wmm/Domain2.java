		...
		for(Event e1 : mEvents) {
			for(Event e2 : mEvents) {
				...
				if(e1.getMainThread() == e2.getMainThread()) {
					...
					if(!(e1 instanceof Load) && e2 instanceof Store && e1.getEId() < e2.getEId()) {
						enc = ctx.mkAnd(enc, ctx.mkNot(Utils.edge("data", e1, e2, ctx)));
					}
					if(!(e1 instanceof Init) && e2 instanceof Store && e1.getEId() == e2.getEId() - 1) {
					...
					if(noSync) {
						enc = ctx.mkAnd(enc, ctx.mkNot(Utils.edge("sync@" + program.hashCode(), e1, e2, ctx)));
					}
					if(noLwsync) {
						enc = ctx.mkAnd(enc, ctx.mkNot(Utils.edge("lwsync@" + program.hashCode(), e1, e2, ctx)));
					}
					if(noIsync) {
				...
				enc = ctx.mkAnd(enc, ctx.mkEq(Utils.edge("rf", e1, e2, ctx),
										ctx.mkOr(Utils.edge("rfe", e1, e2, ctx), Utils.edge("rfi", e1, e2, ctx))));
				...
				enc = ctx.mkAnd(enc, ctx.mkEq(Utils.edge("co", e1, e2, ctx),
										ctx.mkOr(Utils.edge("coe", e1, e2, ctx), Utils.edge("coi", e1, e2, ctx))));
				...
				enc = ctx.mkAnd(enc, ctx.mkEq(Utils.edge("fr", e1, e2, ctx),
										ctx.mkOr(Utils.edge("fre", e1, e2, ctx), Utils.edge("fri", e1, e2, ctx))));
