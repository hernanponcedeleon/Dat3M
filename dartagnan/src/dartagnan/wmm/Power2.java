...
public class Power2 {
	...
	public static BoolExpr encode(Program program, Context ctx) throws Z3Exception {
	    ...
	    // Fences in Power
	    enc = ctx.mkAnd(enc, Encodings.satMinus("lwsync@" + program.hashCode(), "WR", events, ctx));
	    enc = ctx.mkAnd(enc, Encodings.satUnion("fence-power", "sync@" + program.hashCode(), "(lwsync@" + program.hashCode() +"\\WR)", events, ctx));
	    // Happens before
	    ...
	    enc = ctx.mkAnd(enc, Encodings.satComp("((com)*;(prop-base)*)", "sync@" + program.hashCode(), events, ctx));
	    enc = ctx.mkAnd(enc, Encodings.satComp("(((com)*;(prop-base)*);sync@" + program.hashCode() + ")", "(hb-power)*", events, ctx));
	    enc = ctx.mkAnd(enc, Encodings.satIntersection("WW", "prop-base", events, ctx));
	    enc = ctx.mkAnd(enc, Encodings.satUnion("prop", "(WW&prop-base)", "((((com)*;(prop-base)*);sync@" + program.hashCode() + ");(hb-power)*)", events, ctx));
