...
define internal void @ck_pr_md_store_ptr(ptr noundef %0, ptr noundef %1) #0 {
  ...
  %7 = load ptr, ptr %4, align 8
  call void asm sideeffect "str $2, [$1]", "=*m,r,r,~{memory}"(ptr elementtype(ptr) %5, ptr %6, ptr %7) #7, !srcloc !10
  ret void
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define internal void @ck_pr_fence_strict_store() #0 {
  call void asm sideeffect "dmb ishst", "r,~{memory}"(i32 0) #7, !srcloc !11
  ret void
}
...
define internal ptr @ck_pr_md_load_ptr(ptr noundef %0) #0 {
  ...
  %5 = call i64 asm sideeffect "ldr $0, [$1]\0A", "=r,r,~{memory}"(ptr %4) #7, !srcloc !12
