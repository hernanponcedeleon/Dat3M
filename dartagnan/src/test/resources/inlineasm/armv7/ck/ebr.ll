...
80:                                               ; preds = %73
  call void @__assert_rtn(ptr noundef @__func__.thread, ptr noundef @.str, i32 noundef 36, ptr noundef @.str.1) #6
  unreachable
...
define internal void @ck_pr_fence_strict_store() #0 {
  call void asm sideeffect "dmb st", "r,~{memory}"(i32 0) #5, !srcloc !29
  ret void
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define internal void @ck_pr_fence_strict_load() #0 {
  call void asm sideeffect "dmb ish", "~{memory}"() #5, !srcloc !30
  ret void
}
...
define internal void @ck_pr_fence_strict_memory() #0 {
  call void asm sideeffect "dmb ish", "~{memory}"() #5, !srcloc !36
  ret void
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define internal void @ck_pr_fence_strict_atomic_load() #0 {
  call void asm sideeffect "dmb st", "r,~{memory}"(i32 0) #5, !srcloc !37
  ret void
}
...
define internal void @ck_pr_fence_strict_release() #0 {
  call void asm sideeffect "dmb ish", "~{memory}"() #5, !srcloc !38
  ret void
}
