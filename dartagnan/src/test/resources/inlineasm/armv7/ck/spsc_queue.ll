...
define internal void @ck_pr_md_store_ptr(ptr noundef %0, ptr noundef %1) #0 {
  ...
  call void asm sideeffect "str $1, [$0]", "r,r,~{memory}"(ptr %5, ptr %6) #7, !srcloc !10
  ret void
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define internal void @ck_pr_fence_strict_store() #0 {
  call void asm sideeffect "dmb st", "r,~{memory}"(i32 0) #7, !srcloc !11
  ret void
}
...
define internal ptr @ck_pr_md_load_ptr(ptr noundef %0) #0 {
  ...
  %5 = call i64 asm sideeffect "ldr $0, [$1];", "=r,r,~{memory}"(ptr %4) #7, !srcloc !12
