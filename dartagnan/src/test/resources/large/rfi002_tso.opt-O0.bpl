// generated by SMACK version 2.5.0 for corral
// via /usr/local/bin/smack -q -t --no-memory-splitting --integer-encoding unbounded-integer --clang-options=-DCUSTOM_VERIFIER_ASSERT -O0 -fno-vectorize -fno-slp-vectorize -I/Users/ponce/git/Dat3M/include/ -bpl /Users/ponce/git/Dat3M/output/rfi002_tso.opt-O0.bpl /Users/ponce/git/Dat3M/output/rfi002_tso.opt_tmp.c

// Basic types
type i1 = int;
type i5 = int;
type i6 = int;
type i8 = int;
type i16 = int;
type i24 = int;
type i32 = int;
type i40 = int;
type i48 = int;
type i56 = int;
type i64 = int;
type i80 = int;
type i88 = int;
type i96 = int;
type i128 = int;
type i160 = int;
type i256 = int;
type ref = i64;
type float;

// Basic constants
const $0: i32;
axiom ($0 == 0);
const $1: i32;
axiom ($1 == 1);
const $0.ref: ref;
axiom ($0.ref == 0);
const $1.ref: ref;
axiom ($1.ref == 1);
const $1024.ref: ref;
axiom ($1024.ref == 1024);
// Memory model constants
const $GLOBALS_BOTTOM: ref;
const $EXTERNS_BOTTOM: ref;
const $MALLOC_TOP: ref;

// Memory maps (1 regions)
var $M.0: [ref] i8;

// Memory address bounds
axiom ($GLOBALS_BOTTOM == $sub.ref(0, 94733));
axiom ($EXTERNS_BOTTOM == $add.ref($GLOBALS_BOTTOM, $sub.ref(0, 32768)));
axiom ($MALLOC_TOP == 9223372036854775807);
function {:inline} $isExternal(p: ref) returns (bool) { $slt.ref.bool(p, $EXTERNS_BOTTOM) }

// SMT bit-vector/integer conversion
function {:builtin "(_ int2bv 64)"} $int2bv.64(i: i64) returns (bv64);
function {:builtin "bv2nat"} $bv2int.64(i: bv64) returns (i64);

// Integer arithmetic operations
function {:inline} $add.i1(i1: i1, i2: i1) returns (i1) { (i1 + i2) }
function {:inline} $add.i5(i1: i5, i2: i5) returns (i5) { (i1 + i2) }
function {:inline} $add.i6(i1: i6, i2: i6) returns (i6) { (i1 + i2) }
function {:inline} $add.i8(i1: i8, i2: i8) returns (i8) { (i1 + i2) }
function {:inline} $add.i16(i1: i16, i2: i16) returns (i16) { (i1 + i2) }
function {:inline} $add.i24(i1: i24, i2: i24) returns (i24) { (i1 + i2) }
function {:inline} $add.i32(i1: i32, i2: i32) returns (i32) { (i1 + i2) }
function {:inline} $add.i40(i1: i40, i2: i40) returns (i40) { (i1 + i2) }
function {:inline} $add.i48(i1: i48, i2: i48) returns (i48) { (i1 + i2) }
function {:inline} $add.i56(i1: i56, i2: i56) returns (i56) { (i1 + i2) }
function {:inline} $add.i64(i1: i64, i2: i64) returns (i64) { (i1 + i2) }
function {:inline} $add.i80(i1: i80, i2: i80) returns (i80) { (i1 + i2) }
function {:inline} $add.i88(i1: i88, i2: i88) returns (i88) { (i1 + i2) }
function {:inline} $add.i96(i1: i96, i2: i96) returns (i96) { (i1 + i2) }
function {:inline} $add.i128(i1: i128, i2: i128) returns (i128) { (i1 + i2) }
function {:inline} $add.i160(i1: i160, i2: i160) returns (i160) { (i1 + i2) }
function {:inline} $add.i256(i1: i256, i2: i256) returns (i256) { (i1 + i2) }
function {:inline} $sub.i1(i1: i1, i2: i1) returns (i1) { (i1 - i2) }
function {:inline} $sub.i5(i1: i5, i2: i5) returns (i5) { (i1 - i2) }
function {:inline} $sub.i6(i1: i6, i2: i6) returns (i6) { (i1 - i2) }
function {:inline} $sub.i8(i1: i8, i2: i8) returns (i8) { (i1 - i2) }
function {:inline} $sub.i16(i1: i16, i2: i16) returns (i16) { (i1 - i2) }
function {:inline} $sub.i24(i1: i24, i2: i24) returns (i24) { (i1 - i2) }
function {:inline} $sub.i32(i1: i32, i2: i32) returns (i32) { (i1 - i2) }
function {:inline} $sub.i40(i1: i40, i2: i40) returns (i40) { (i1 - i2) }
function {:inline} $sub.i48(i1: i48, i2: i48) returns (i48) { (i1 - i2) }
function {:inline} $sub.i56(i1: i56, i2: i56) returns (i56) { (i1 - i2) }
function {:inline} $sub.i64(i1: i64, i2: i64) returns (i64) { (i1 - i2) }
function {:inline} $sub.i80(i1: i80, i2: i80) returns (i80) { (i1 - i2) }
function {:inline} $sub.i88(i1: i88, i2: i88) returns (i88) { (i1 - i2) }
function {:inline} $sub.i96(i1: i96, i2: i96) returns (i96) { (i1 - i2) }
function {:inline} $sub.i128(i1: i128, i2: i128) returns (i128) { (i1 - i2) }
function {:inline} $sub.i160(i1: i160, i2: i160) returns (i160) { (i1 - i2) }
function {:inline} $sub.i256(i1: i256, i2: i256) returns (i256) { (i1 - i2) }
function {:inline} $mul.i1(i1: i1, i2: i1) returns (i1) { (i1 * i2) }
function {:inline} $mul.i5(i1: i5, i2: i5) returns (i5) { (i1 * i2) }
function {:inline} $mul.i6(i1: i6, i2: i6) returns (i6) { (i1 * i2) }
function {:inline} $mul.i8(i1: i8, i2: i8) returns (i8) { (i1 * i2) }
function {:inline} $mul.i16(i1: i16, i2: i16) returns (i16) { (i1 * i2) }
function {:inline} $mul.i24(i1: i24, i2: i24) returns (i24) { (i1 * i2) }
function {:inline} $mul.i32(i1: i32, i2: i32) returns (i32) { (i1 * i2) }
function {:inline} $mul.i40(i1: i40, i2: i40) returns (i40) { (i1 * i2) }
function {:inline} $mul.i48(i1: i48, i2: i48) returns (i48) { (i1 * i2) }
function {:inline} $mul.i56(i1: i56, i2: i56) returns (i56) { (i1 * i2) }
function {:inline} $mul.i64(i1: i64, i2: i64) returns (i64) { (i1 * i2) }
function {:inline} $mul.i80(i1: i80, i2: i80) returns (i80) { (i1 * i2) }
function {:inline} $mul.i88(i1: i88, i2: i88) returns (i88) { (i1 * i2) }
function {:inline} $mul.i96(i1: i96, i2: i96) returns (i96) { (i1 * i2) }
function {:inline} $mul.i128(i1: i128, i2: i128) returns (i128) { (i1 * i2) }
function {:inline} $mul.i160(i1: i160, i2: i160) returns (i160) { (i1 * i2) }
function {:inline} $mul.i256(i1: i256, i2: i256) returns (i256) { (i1 * i2) }
function {:builtin "div"} $idiv.i1(i1: i1, i2: i1) returns (i1);
function {:builtin "div"} $idiv.i5(i1: i5, i2: i5) returns (i5);
function {:builtin "div"} $idiv.i6(i1: i6, i2: i6) returns (i6);
function {:builtin "div"} $idiv.i8(i1: i8, i2: i8) returns (i8);
function {:builtin "div"} $idiv.i16(i1: i16, i2: i16) returns (i16);
function {:builtin "div"} $idiv.i24(i1: i24, i2: i24) returns (i24);
function {:builtin "div"} $idiv.i32(i1: i32, i2: i32) returns (i32);
function {:builtin "div"} $idiv.i40(i1: i40, i2: i40) returns (i40);
function {:builtin "div"} $idiv.i48(i1: i48, i2: i48) returns (i48);
function {:builtin "div"} $idiv.i56(i1: i56, i2: i56) returns (i56);
function {:builtin "div"} $idiv.i64(i1: i64, i2: i64) returns (i64);
function {:builtin "div"} $idiv.i80(i1: i80, i2: i80) returns (i80);
function {:builtin "div"} $idiv.i88(i1: i88, i2: i88) returns (i88);
function {:builtin "div"} $idiv.i96(i1: i96, i2: i96) returns (i96);
function {:builtin "div"} $idiv.i128(i1: i128, i2: i128) returns (i128);
function {:builtin "div"} $idiv.i160(i1: i160, i2: i160) returns (i160);
function {:builtin "div"} $idiv.i256(i1: i256, i2: i256) returns (i256);
function {:inline} $sdiv.i1(i1: i1, i2: i1) returns (i1) { $idiv.i1(i1, i2) }
function {:inline} $sdiv.i5(i1: i5, i2: i5) returns (i5) { $idiv.i5(i1, i2) }
function {:inline} $sdiv.i6(i1: i6, i2: i6) returns (i6) { $idiv.i6(i1, i2) }
function {:inline} $sdiv.i8(i1: i8, i2: i8) returns (i8) { $idiv.i8(i1, i2) }
function {:inline} $sdiv.i16(i1: i16, i2: i16) returns (i16) { $idiv.i16(i1, i2) }
function {:inline} $sdiv.i24(i1: i24, i2: i24) returns (i24) { $idiv.i24(i1, i2) }
function {:inline} $sdiv.i32(i1: i32, i2: i32) returns (i32) { $idiv.i32(i1, i2) }
function {:inline} $sdiv.i40(i1: i40, i2: i40) returns (i40) { $idiv.i40(i1, i2) }
function {:inline} $sdiv.i48(i1: i48, i2: i48) returns (i48) { $idiv.i48(i1, i2) }
function {:inline} $sdiv.i56(i1: i56, i2: i56) returns (i56) { $idiv.i56(i1, i2) }
function {:inline} $sdiv.i64(i1: i64, i2: i64) returns (i64) { $idiv.i64(i1, i2) }
function {:inline} $sdiv.i80(i1: i80, i2: i80) returns (i80) { $idiv.i80(i1, i2) }
function {:inline} $sdiv.i88(i1: i88, i2: i88) returns (i88) { $idiv.i88(i1, i2) }
function {:inline} $sdiv.i96(i1: i96, i2: i96) returns (i96) { $idiv.i96(i1, i2) }
function {:inline} $sdiv.i128(i1: i128, i2: i128) returns (i128) { $idiv.i128(i1, i2) }
function {:inline} $sdiv.i160(i1: i160, i2: i160) returns (i160) { $idiv.i160(i1, i2) }
function {:inline} $sdiv.i256(i1: i256, i2: i256) returns (i256) { $idiv.i256(i1, i2) }
function {:inline} $udiv.i1(i1: i1, i2: i1) returns (i1) { $idiv.i1(i1, i2) }
function {:inline} $udiv.i5(i1: i5, i2: i5) returns (i5) { $idiv.i5(i1, i2) }
function {:inline} $udiv.i6(i1: i6, i2: i6) returns (i6) { $idiv.i6(i1, i2) }
function {:inline} $udiv.i8(i1: i8, i2: i8) returns (i8) { $idiv.i8(i1, i2) }
function {:inline} $udiv.i16(i1: i16, i2: i16) returns (i16) { $idiv.i16(i1, i2) }
function {:inline} $udiv.i24(i1: i24, i2: i24) returns (i24) { $idiv.i24(i1, i2) }
function {:inline} $udiv.i32(i1: i32, i2: i32) returns (i32) { $idiv.i32(i1, i2) }
function {:inline} $udiv.i40(i1: i40, i2: i40) returns (i40) { $idiv.i40(i1, i2) }
function {:inline} $udiv.i48(i1: i48, i2: i48) returns (i48) { $idiv.i48(i1, i2) }
function {:inline} $udiv.i56(i1: i56, i2: i56) returns (i56) { $idiv.i56(i1, i2) }
function {:inline} $udiv.i64(i1: i64, i2: i64) returns (i64) { $idiv.i64(i1, i2) }
function {:inline} $udiv.i80(i1: i80, i2: i80) returns (i80) { $idiv.i80(i1, i2) }
function {:inline} $udiv.i88(i1: i88, i2: i88) returns (i88) { $idiv.i88(i1, i2) }
function {:inline} $udiv.i96(i1: i96, i2: i96) returns (i96) { $idiv.i96(i1, i2) }
function {:inline} $udiv.i128(i1: i128, i2: i128) returns (i128) { $idiv.i128(i1, i2) }
function {:inline} $udiv.i160(i1: i160, i2: i160) returns (i160) { $idiv.i160(i1, i2) }
function {:inline} $udiv.i256(i1: i256, i2: i256) returns (i256) { $idiv.i256(i1, i2) }
function {:builtin "mod"} $smod.i1(i1: i1, i2: i1) returns (i1);
function {:builtin "mod"} $smod.i5(i1: i5, i2: i5) returns (i5);
function {:builtin "mod"} $smod.i6(i1: i6, i2: i6) returns (i6);
function {:builtin "mod"} $smod.i8(i1: i8, i2: i8) returns (i8);
function {:builtin "mod"} $smod.i16(i1: i16, i2: i16) returns (i16);
function {:builtin "mod"} $smod.i24(i1: i24, i2: i24) returns (i24);
function {:builtin "mod"} $smod.i32(i1: i32, i2: i32) returns (i32);
function {:builtin "mod"} $smod.i40(i1: i40, i2: i40) returns (i40);
function {:builtin "mod"} $smod.i48(i1: i48, i2: i48) returns (i48);
function {:builtin "mod"} $smod.i56(i1: i56, i2: i56) returns (i56);
function {:builtin "mod"} $smod.i64(i1: i64, i2: i64) returns (i64);
function {:builtin "mod"} $smod.i80(i1: i80, i2: i80) returns (i80);
function {:builtin "mod"} $smod.i88(i1: i88, i2: i88) returns (i88);
function {:builtin "mod"} $smod.i96(i1: i96, i2: i96) returns (i96);
function {:builtin "mod"} $smod.i128(i1: i128, i2: i128) returns (i128);
function {:builtin "mod"} $smod.i160(i1: i160, i2: i160) returns (i160);
function {:builtin "mod"} $smod.i256(i1: i256, i2: i256) returns (i256);
function {:inline} $srem.i1(i1: i1, i2: i1) returns (i1) { (if ($ne.i1.bool($smod.i1(i1, i2), 0) && $slt.i1.bool(i1, 0)) then $sub.i1($smod.i1(i1, i2), $smax.i1(i2, $sub.i1(0, i2))) else $smod.i1(i1, i2)) }
function {:inline} $srem.i5(i1: i5, i2: i5) returns (i5) { (if ($ne.i5.bool($smod.i5(i1, i2), 0) && $slt.i5.bool(i1, 0)) then $sub.i5($smod.i5(i1, i2), $smax.i5(i2, $sub.i5(0, i2))) else $smod.i5(i1, i2)) }
function {:inline} $srem.i6(i1: i6, i2: i6) returns (i6) { (if ($ne.i6.bool($smod.i6(i1, i2), 0) && $slt.i6.bool(i1, 0)) then $sub.i6($smod.i6(i1, i2), $smax.i6(i2, $sub.i6(0, i2))) else $smod.i6(i1, i2)) }
function {:inline} $srem.i8(i1: i8, i2: i8) returns (i8) { (if ($ne.i8.bool($smod.i8(i1, i2), 0) && $slt.i8.bool(i1, 0)) then $sub.i8($smod.i8(i1, i2), $smax.i8(i2, $sub.i8(0, i2))) else $smod.i8(i1, i2)) }
function {:inline} $srem.i16(i1: i16, i2: i16) returns (i16) { (if ($ne.i16.bool($smod.i16(i1, i2), 0) && $slt.i16.bool(i1, 0)) then $sub.i16($smod.i16(i1, i2), $smax.i16(i2, $sub.i16(0, i2))) else $smod.i16(i1, i2)) }
function {:inline} $srem.i24(i1: i24, i2: i24) returns (i24) { (if ($ne.i24.bool($smod.i24(i1, i2), 0) && $slt.i24.bool(i1, 0)) then $sub.i24($smod.i24(i1, i2), $smax.i24(i2, $sub.i24(0, i2))) else $smod.i24(i1, i2)) }
function {:inline} $srem.i32(i1: i32, i2: i32) returns (i32) { (if ($ne.i32.bool($smod.i32(i1, i2), 0) && $slt.i32.bool(i1, 0)) then $sub.i32($smod.i32(i1, i2), $smax.i32(i2, $sub.i32(0, i2))) else $smod.i32(i1, i2)) }
function {:inline} $srem.i40(i1: i40, i2: i40) returns (i40) { (if ($ne.i40.bool($smod.i40(i1, i2), 0) && $slt.i40.bool(i1, 0)) then $sub.i40($smod.i40(i1, i2), $smax.i40(i2, $sub.i40(0, i2))) else $smod.i40(i1, i2)) }
function {:inline} $srem.i48(i1: i48, i2: i48) returns (i48) { (if ($ne.i48.bool($smod.i48(i1, i2), 0) && $slt.i48.bool(i1, 0)) then $sub.i48($smod.i48(i1, i2), $smax.i48(i2, $sub.i48(0, i2))) else $smod.i48(i1, i2)) }
function {:inline} $srem.i56(i1: i56, i2: i56) returns (i56) { (if ($ne.i56.bool($smod.i56(i1, i2), 0) && $slt.i56.bool(i1, 0)) then $sub.i56($smod.i56(i1, i2), $smax.i56(i2, $sub.i56(0, i2))) else $smod.i56(i1, i2)) }
function {:inline} $srem.i64(i1: i64, i2: i64) returns (i64) { (if ($ne.i64.bool($smod.i64(i1, i2), 0) && $slt.i64.bool(i1, 0)) then $sub.i64($smod.i64(i1, i2), $smax.i64(i2, $sub.i64(0, i2))) else $smod.i64(i1, i2)) }
function {:inline} $srem.i80(i1: i80, i2: i80) returns (i80) { (if ($ne.i80.bool($smod.i80(i1, i2), 0) && $slt.i80.bool(i1, 0)) then $sub.i80($smod.i80(i1, i2), $smax.i80(i2, $sub.i80(0, i2))) else $smod.i80(i1, i2)) }
function {:inline} $srem.i88(i1: i88, i2: i88) returns (i88) { (if ($ne.i88.bool($smod.i88(i1, i2), 0) && $slt.i88.bool(i1, 0)) then $sub.i88($smod.i88(i1, i2), $smax.i88(i2, $sub.i88(0, i2))) else $smod.i88(i1, i2)) }
function {:inline} $srem.i96(i1: i96, i2: i96) returns (i96) { (if ($ne.i96.bool($smod.i96(i1, i2), 0) && $slt.i96.bool(i1, 0)) then $sub.i96($smod.i96(i1, i2), $smax.i96(i2, $sub.i96(0, i2))) else $smod.i96(i1, i2)) }
function {:inline} $srem.i128(i1: i128, i2: i128) returns (i128) { (if ($ne.i128.bool($smod.i128(i1, i2), 0) && $slt.i128.bool(i1, 0)) then $sub.i128($smod.i128(i1, i2), $smax.i128(i2, $sub.i128(0, i2))) else $smod.i128(i1, i2)) }
function {:inline} $srem.i160(i1: i160, i2: i160) returns (i160) { (if ($ne.i160.bool($smod.i160(i1, i2), 0) && $slt.i160.bool(i1, 0)) then $sub.i160($smod.i160(i1, i2), $smax.i160(i2, $sub.i160(0, i2))) else $smod.i160(i1, i2)) }
function {:inline} $srem.i256(i1: i256, i2: i256) returns (i256) { (if ($ne.i256.bool($smod.i256(i1, i2), 0) && $slt.i256.bool(i1, 0)) then $sub.i256($smod.i256(i1, i2), $smax.i256(i2, $sub.i256(0, i2))) else $smod.i256(i1, i2)) }
function {:inline} $urem.i1(i1: i1, i2: i1) returns (i1) { $smod.i1(i1, i2) }
function {:inline} $urem.i5(i1: i5, i2: i5) returns (i5) { $smod.i5(i1, i2) }
function {:inline} $urem.i6(i1: i6, i2: i6) returns (i6) { $smod.i6(i1, i2) }
function {:inline} $urem.i8(i1: i8, i2: i8) returns (i8) { $smod.i8(i1, i2) }
function {:inline} $urem.i16(i1: i16, i2: i16) returns (i16) { $smod.i16(i1, i2) }
function {:inline} $urem.i24(i1: i24, i2: i24) returns (i24) { $smod.i24(i1, i2) }
function {:inline} $urem.i32(i1: i32, i2: i32) returns (i32) { $smod.i32(i1, i2) }
function {:inline} $urem.i40(i1: i40, i2: i40) returns (i40) { $smod.i40(i1, i2) }
function {:inline} $urem.i48(i1: i48, i2: i48) returns (i48) { $smod.i48(i1, i2) }
function {:inline} $urem.i56(i1: i56, i2: i56) returns (i56) { $smod.i56(i1, i2) }
function {:inline} $urem.i64(i1: i64, i2: i64) returns (i64) { $smod.i64(i1, i2) }
function {:inline} $urem.i80(i1: i80, i2: i80) returns (i80) { $smod.i80(i1, i2) }
function {:inline} $urem.i88(i1: i88, i2: i88) returns (i88) { $smod.i88(i1, i2) }
function {:inline} $urem.i96(i1: i96, i2: i96) returns (i96) { $smod.i96(i1, i2) }
function {:inline} $urem.i128(i1: i128, i2: i128) returns (i128) { $smod.i128(i1, i2) }
function {:inline} $urem.i160(i1: i160, i2: i160) returns (i160) { $smod.i160(i1, i2) }
function {:inline} $urem.i256(i1: i256, i2: i256) returns (i256) { $smod.i256(i1, i2) }
function $shl.i1(i1: i1, i2: i1) returns (i1);
function $shl.i5(i1: i5, i2: i5) returns (i5);
function $shl.i6(i1: i6, i2: i6) returns (i6);
function $shl.i8(i1: i8, i2: i8) returns (i8);
function $shl.i16(i1: i16, i2: i16) returns (i16);
function $shl.i24(i1: i24, i2: i24) returns (i24);
function $shl.i32(i1: i32, i2: i32) returns (i32);
function $shl.i40(i1: i40, i2: i40) returns (i40);
function $shl.i48(i1: i48, i2: i48) returns (i48);
function $shl.i56(i1: i56, i2: i56) returns (i56);
function $shl.i64(i1: i64, i2: i64) returns (i64);
function $shl.i80(i1: i80, i2: i80) returns (i80);
function $shl.i88(i1: i88, i2: i88) returns (i88);
function $shl.i96(i1: i96, i2: i96) returns (i96);
function $shl.i128(i1: i128, i2: i128) returns (i128);
function $shl.i160(i1: i160, i2: i160) returns (i160);
function $shl.i256(i1: i256, i2: i256) returns (i256);
function $lshr.i1(i1: i1, i2: i1) returns (i1);
function $lshr.i5(i1: i5, i2: i5) returns (i5);
function $lshr.i6(i1: i6, i2: i6) returns (i6);
function $lshr.i8(i1: i8, i2: i8) returns (i8);
function $lshr.i16(i1: i16, i2: i16) returns (i16);
function $lshr.i24(i1: i24, i2: i24) returns (i24);
function $lshr.i32(i1: i32, i2: i32) returns (i32);
function $lshr.i40(i1: i40, i2: i40) returns (i40);
function $lshr.i48(i1: i48, i2: i48) returns (i48);
function $lshr.i56(i1: i56, i2: i56) returns (i56);
function $lshr.i64(i1: i64, i2: i64) returns (i64);
function $lshr.i80(i1: i80, i2: i80) returns (i80);
function $lshr.i88(i1: i88, i2: i88) returns (i88);
function $lshr.i96(i1: i96, i2: i96) returns (i96);
function $lshr.i128(i1: i128, i2: i128) returns (i128);
function $lshr.i160(i1: i160, i2: i160) returns (i160);
function $lshr.i256(i1: i256, i2: i256) returns (i256);
function $ashr.i1(i1: i1, i2: i1) returns (i1);
function $ashr.i5(i1: i5, i2: i5) returns (i5);
function $ashr.i6(i1: i6, i2: i6) returns (i6);
function $ashr.i8(i1: i8, i2: i8) returns (i8);
function $ashr.i16(i1: i16, i2: i16) returns (i16);
function $ashr.i24(i1: i24, i2: i24) returns (i24);
function $ashr.i32(i1: i32, i2: i32) returns (i32);
function $ashr.i40(i1: i40, i2: i40) returns (i40);
function $ashr.i48(i1: i48, i2: i48) returns (i48);
function $ashr.i56(i1: i56, i2: i56) returns (i56);
function $ashr.i64(i1: i64, i2: i64) returns (i64);
function $ashr.i80(i1: i80, i2: i80) returns (i80);
function $ashr.i88(i1: i88, i2: i88) returns (i88);
function $ashr.i96(i1: i96, i2: i96) returns (i96);
function $ashr.i128(i1: i128, i2: i128) returns (i128);
function $ashr.i160(i1: i160, i2: i160) returns (i160);
function $ashr.i256(i1: i256, i2: i256) returns (i256);
function $and.i1(i1: i1, i2: i1) returns (i1);
function $and.i5(i1: i5, i2: i5) returns (i5);
function $and.i6(i1: i6, i2: i6) returns (i6);
function $and.i8(i1: i8, i2: i8) returns (i8);
function $and.i16(i1: i16, i2: i16) returns (i16);
function $and.i24(i1: i24, i2: i24) returns (i24);
function $and.i32(i1: i32, i2: i32) returns (i32);
function $and.i40(i1: i40, i2: i40) returns (i40);
function $and.i48(i1: i48, i2: i48) returns (i48);
function $and.i56(i1: i56, i2: i56) returns (i56);
function $and.i64(i1: i64, i2: i64) returns (i64);
function $and.i80(i1: i80, i2: i80) returns (i80);
function $and.i88(i1: i88, i2: i88) returns (i88);
function $and.i96(i1: i96, i2: i96) returns (i96);
function $and.i128(i1: i128, i2: i128) returns (i128);
function $and.i160(i1: i160, i2: i160) returns (i160);
function $and.i256(i1: i256, i2: i256) returns (i256);
function $or.i1(i1: i1, i2: i1) returns (i1);
function $or.i5(i1: i5, i2: i5) returns (i5);
function $or.i6(i1: i6, i2: i6) returns (i6);
function $or.i8(i1: i8, i2: i8) returns (i8);
function $or.i16(i1: i16, i2: i16) returns (i16);
function $or.i24(i1: i24, i2: i24) returns (i24);
function $or.i32(i1: i32, i2: i32) returns (i32);
function $or.i40(i1: i40, i2: i40) returns (i40);
function $or.i48(i1: i48, i2: i48) returns (i48);
function $or.i56(i1: i56, i2: i56) returns (i56);
function $or.i64(i1: i64, i2: i64) returns (i64);
function $or.i80(i1: i80, i2: i80) returns (i80);
function $or.i88(i1: i88, i2: i88) returns (i88);
function $or.i96(i1: i96, i2: i96) returns (i96);
function $or.i128(i1: i128, i2: i128) returns (i128);
function $or.i160(i1: i160, i2: i160) returns (i160);
function $or.i256(i1: i256, i2: i256) returns (i256);
function $xor.i1(i1: i1, i2: i1) returns (i1);
function $xor.i5(i1: i5, i2: i5) returns (i5);
function $xor.i6(i1: i6, i2: i6) returns (i6);
function $xor.i8(i1: i8, i2: i8) returns (i8);
function $xor.i16(i1: i16, i2: i16) returns (i16);
function $xor.i24(i1: i24, i2: i24) returns (i24);
function $xor.i32(i1: i32, i2: i32) returns (i32);
function $xor.i40(i1: i40, i2: i40) returns (i40);
function $xor.i48(i1: i48, i2: i48) returns (i48);
function $xor.i56(i1: i56, i2: i56) returns (i56);
function $xor.i64(i1: i64, i2: i64) returns (i64);
function $xor.i80(i1: i80, i2: i80) returns (i80);
function $xor.i88(i1: i88, i2: i88) returns (i88);
function $xor.i96(i1: i96, i2: i96) returns (i96);
function $xor.i128(i1: i128, i2: i128) returns (i128);
function $xor.i160(i1: i160, i2: i160) returns (i160);
function $xor.i256(i1: i256, i2: i256) returns (i256);
function $nand.i1(i1: i1, i2: i1) returns (i1);
function $nand.i5(i1: i5, i2: i5) returns (i5);
function $nand.i6(i1: i6, i2: i6) returns (i6);
function $nand.i8(i1: i8, i2: i8) returns (i8);
function $nand.i16(i1: i16, i2: i16) returns (i16);
function $nand.i24(i1: i24, i2: i24) returns (i24);
function $nand.i32(i1: i32, i2: i32) returns (i32);
function $nand.i40(i1: i40, i2: i40) returns (i40);
function $nand.i48(i1: i48, i2: i48) returns (i48);
function $nand.i56(i1: i56, i2: i56) returns (i56);
function $nand.i64(i1: i64, i2: i64) returns (i64);
function $nand.i80(i1: i80, i2: i80) returns (i80);
function $nand.i88(i1: i88, i2: i88) returns (i88);
function $nand.i96(i1: i96, i2: i96) returns (i96);
function $nand.i128(i1: i128, i2: i128) returns (i128);
function $nand.i160(i1: i160, i2: i160) returns (i160);
function $nand.i256(i1: i256, i2: i256) returns (i256);
function $not.i1(i: i1) returns (i1);
function $not.i5(i: i5) returns (i5);
function $not.i6(i: i6) returns (i6);
function $not.i8(i: i8) returns (i8);
function $not.i16(i: i16) returns (i16);
function $not.i24(i: i24) returns (i24);
function $not.i32(i: i32) returns (i32);
function $not.i40(i: i40) returns (i40);
function $not.i48(i: i48) returns (i48);
function $not.i56(i: i56) returns (i56);
function $not.i64(i: i64) returns (i64);
function $not.i80(i: i80) returns (i80);
function $not.i88(i: i88) returns (i88);
function $not.i96(i: i96) returns (i96);
function $not.i128(i: i128) returns (i128);
function $not.i160(i: i160) returns (i160);
function $not.i256(i: i256) returns (i256);
function {:inline} $smin.i1(i1: i1, i2: i1) returns (i1) { (if $slt.i1.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i5(i1: i5, i2: i5) returns (i5) { (if $slt.i5.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i6(i1: i6, i2: i6) returns (i6) { (if $slt.i6.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i8(i1: i8, i2: i8) returns (i8) { (if $slt.i8.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i16(i1: i16, i2: i16) returns (i16) { (if $slt.i16.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i24(i1: i24, i2: i24) returns (i24) { (if $slt.i24.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i32(i1: i32, i2: i32) returns (i32) { (if $slt.i32.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i40(i1: i40, i2: i40) returns (i40) { (if $slt.i40.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i48(i1: i48, i2: i48) returns (i48) { (if $slt.i48.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i56(i1: i56, i2: i56) returns (i56) { (if $slt.i56.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i64(i1: i64, i2: i64) returns (i64) { (if $slt.i64.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i80(i1: i80, i2: i80) returns (i80) { (if $slt.i80.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i88(i1: i88, i2: i88) returns (i88) { (if $slt.i88.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i96(i1: i96, i2: i96) returns (i96) { (if $slt.i96.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i128(i1: i128, i2: i128) returns (i128) { (if $slt.i128.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i160(i1: i160, i2: i160) returns (i160) { (if $slt.i160.bool(i1, i2) then i1 else i2) }
function {:inline} $smin.i256(i1: i256, i2: i256) returns (i256) { (if $slt.i256.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i1(i1: i1, i2: i1) returns (i1) { (if $sgt.i1.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i5(i1: i5, i2: i5) returns (i5) { (if $sgt.i5.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i6(i1: i6, i2: i6) returns (i6) { (if $sgt.i6.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i8(i1: i8, i2: i8) returns (i8) { (if $sgt.i8.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i16(i1: i16, i2: i16) returns (i16) { (if $sgt.i16.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i24(i1: i24, i2: i24) returns (i24) { (if $sgt.i24.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i32(i1: i32, i2: i32) returns (i32) { (if $sgt.i32.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i40(i1: i40, i2: i40) returns (i40) { (if $sgt.i40.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i48(i1: i48, i2: i48) returns (i48) { (if $sgt.i48.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i56(i1: i56, i2: i56) returns (i56) { (if $sgt.i56.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i64(i1: i64, i2: i64) returns (i64) { (if $sgt.i64.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i80(i1: i80, i2: i80) returns (i80) { (if $sgt.i80.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i88(i1: i88, i2: i88) returns (i88) { (if $sgt.i88.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i96(i1: i96, i2: i96) returns (i96) { (if $sgt.i96.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i128(i1: i128, i2: i128) returns (i128) { (if $sgt.i128.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i160(i1: i160, i2: i160) returns (i160) { (if $sgt.i160.bool(i1, i2) then i1 else i2) }
function {:inline} $smax.i256(i1: i256, i2: i256) returns (i256) { (if $sgt.i256.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i1(i1: i1, i2: i1) returns (i1) { (if $ult.i1.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i5(i1: i5, i2: i5) returns (i5) { (if $ult.i5.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i6(i1: i6, i2: i6) returns (i6) { (if $ult.i6.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i8(i1: i8, i2: i8) returns (i8) { (if $ult.i8.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i16(i1: i16, i2: i16) returns (i16) { (if $ult.i16.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i24(i1: i24, i2: i24) returns (i24) { (if $ult.i24.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i32(i1: i32, i2: i32) returns (i32) { (if $ult.i32.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i40(i1: i40, i2: i40) returns (i40) { (if $ult.i40.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i48(i1: i48, i2: i48) returns (i48) { (if $ult.i48.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i56(i1: i56, i2: i56) returns (i56) { (if $ult.i56.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i64(i1: i64, i2: i64) returns (i64) { (if $ult.i64.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i80(i1: i80, i2: i80) returns (i80) { (if $ult.i80.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i88(i1: i88, i2: i88) returns (i88) { (if $ult.i88.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i96(i1: i96, i2: i96) returns (i96) { (if $ult.i96.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i128(i1: i128, i2: i128) returns (i128) { (if $ult.i128.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i160(i1: i160, i2: i160) returns (i160) { (if $ult.i160.bool(i1, i2) then i1 else i2) }
function {:inline} $umin.i256(i1: i256, i2: i256) returns (i256) { (if $ult.i256.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i1(i1: i1, i2: i1) returns (i1) { (if $ugt.i1.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i5(i1: i5, i2: i5) returns (i5) { (if $ugt.i5.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i6(i1: i6, i2: i6) returns (i6) { (if $ugt.i6.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i8(i1: i8, i2: i8) returns (i8) { (if $ugt.i8.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i16(i1: i16, i2: i16) returns (i16) { (if $ugt.i16.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i24(i1: i24, i2: i24) returns (i24) { (if $ugt.i24.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i32(i1: i32, i2: i32) returns (i32) { (if $ugt.i32.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i40(i1: i40, i2: i40) returns (i40) { (if $ugt.i40.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i48(i1: i48, i2: i48) returns (i48) { (if $ugt.i48.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i56(i1: i56, i2: i56) returns (i56) { (if $ugt.i56.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i64(i1: i64, i2: i64) returns (i64) { (if $ugt.i64.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i80(i1: i80, i2: i80) returns (i80) { (if $ugt.i80.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i88(i1: i88, i2: i88) returns (i88) { (if $ugt.i88.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i96(i1: i96, i2: i96) returns (i96) { (if $ugt.i96.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i128(i1: i128, i2: i128) returns (i128) { (if $ugt.i128.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i160(i1: i160, i2: i160) returns (i160) { (if $ugt.i160.bool(i1, i2) then i1 else i2) }
function {:inline} $umax.i256(i1: i256, i2: i256) returns (i256) { (if $ugt.i256.bool(i1, i2) then i1 else i2) }
axiom ($and.i1(0, 0) == 0);
axiom ($or.i1(0, 0) == 0);
axiom ($xor.i1(0, 0) == 0);
axiom ($and.i1(0, 1) == 0);
axiom ($or.i1(0, 1) == 1);
axiom ($xor.i1(0, 1) == 1);
axiom ($and.i1(1, 0) == 0);
axiom ($or.i1(1, 0) == 1);
axiom ($xor.i1(1, 0) == 1);
axiom ($and.i1(1, 1) == 1);
axiom ($or.i1(1, 1) == 1);
axiom ($xor.i1(1, 1) == 0);
axiom ($and.i32(32, 16) == 0);
// Integer predicates
function {:inline} $ule.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i1(i1: i1, i2: i1) returns (i1) { (if $ule.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i5(i1: i5, i2: i5) returns (i1) { (if $ule.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i6(i1: i6, i2: i6) returns (i1) { (if $ule.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i8(i1: i8, i2: i8) returns (i1) { (if $ule.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i16(i1: i16, i2: i16) returns (i1) { (if $ule.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i24(i1: i24, i2: i24) returns (i1) { (if $ule.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i32(i1: i32, i2: i32) returns (i1) { (if $ule.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i40(i1: i40, i2: i40) returns (i1) { (if $ule.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i48(i1: i48, i2: i48) returns (i1) { (if $ule.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i56(i1: i56, i2: i56) returns (i1) { (if $ule.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i64(i1: i64, i2: i64) returns (i1) { (if $ule.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i80(i1: i80, i2: i80) returns (i1) { (if $ule.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i88(i1: i88, i2: i88) returns (i1) { (if $ule.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i96(i1: i96, i2: i96) returns (i1) { (if $ule.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i128(i1: i128, i2: i128) returns (i1) { (if $ule.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i160(i1: i160, i2: i160) returns (i1) { (if $ule.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $ule.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 <= i2) }
function {:inline} $ule.i256(i1: i256, i2: i256) returns (i1) { (if $ule.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 < i2) }
function {:inline} $ult.i1(i1: i1, i2: i1) returns (i1) { (if $ult.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 < i2) }
function {:inline} $ult.i5(i1: i5, i2: i5) returns (i1) { (if $ult.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 < i2) }
function {:inline} $ult.i6(i1: i6, i2: i6) returns (i1) { (if $ult.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 < i2) }
function {:inline} $ult.i8(i1: i8, i2: i8) returns (i1) { (if $ult.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 < i2) }
function {:inline} $ult.i16(i1: i16, i2: i16) returns (i1) { (if $ult.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 < i2) }
function {:inline} $ult.i24(i1: i24, i2: i24) returns (i1) { (if $ult.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 < i2) }
function {:inline} $ult.i32(i1: i32, i2: i32) returns (i1) { (if $ult.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 < i2) }
function {:inline} $ult.i40(i1: i40, i2: i40) returns (i1) { (if $ult.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 < i2) }
function {:inline} $ult.i48(i1: i48, i2: i48) returns (i1) { (if $ult.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 < i2) }
function {:inline} $ult.i56(i1: i56, i2: i56) returns (i1) { (if $ult.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 < i2) }
function {:inline} $ult.i64(i1: i64, i2: i64) returns (i1) { (if $ult.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 < i2) }
function {:inline} $ult.i80(i1: i80, i2: i80) returns (i1) { (if $ult.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 < i2) }
function {:inline} $ult.i88(i1: i88, i2: i88) returns (i1) { (if $ult.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 < i2) }
function {:inline} $ult.i96(i1: i96, i2: i96) returns (i1) { (if $ult.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 < i2) }
function {:inline} $ult.i128(i1: i128, i2: i128) returns (i1) { (if $ult.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 < i2) }
function {:inline} $ult.i160(i1: i160, i2: i160) returns (i1) { (if $ult.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $ult.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 < i2) }
function {:inline} $ult.i256(i1: i256, i2: i256) returns (i1) { (if $ult.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i1(i1: i1, i2: i1) returns (i1) { (if $uge.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i5(i1: i5, i2: i5) returns (i1) { (if $uge.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i6(i1: i6, i2: i6) returns (i1) { (if $uge.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i8(i1: i8, i2: i8) returns (i1) { (if $uge.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i16(i1: i16, i2: i16) returns (i1) { (if $uge.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i24(i1: i24, i2: i24) returns (i1) { (if $uge.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i32(i1: i32, i2: i32) returns (i1) { (if $uge.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i40(i1: i40, i2: i40) returns (i1) { (if $uge.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i48(i1: i48, i2: i48) returns (i1) { (if $uge.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i56(i1: i56, i2: i56) returns (i1) { (if $uge.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i64(i1: i64, i2: i64) returns (i1) { (if $uge.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i80(i1: i80, i2: i80) returns (i1) { (if $uge.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i88(i1: i88, i2: i88) returns (i1) { (if $uge.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i96(i1: i96, i2: i96) returns (i1) { (if $uge.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i128(i1: i128, i2: i128) returns (i1) { (if $uge.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i160(i1: i160, i2: i160) returns (i1) { (if $uge.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $uge.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 >= i2) }
function {:inline} $uge.i256(i1: i256, i2: i256) returns (i1) { (if $uge.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i1(i1: i1, i2: i1) returns (i1) { (if $ugt.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i5(i1: i5, i2: i5) returns (i1) { (if $ugt.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i6(i1: i6, i2: i6) returns (i1) { (if $ugt.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i8(i1: i8, i2: i8) returns (i1) { (if $ugt.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i16(i1: i16, i2: i16) returns (i1) { (if $ugt.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i24(i1: i24, i2: i24) returns (i1) { (if $ugt.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i32(i1: i32, i2: i32) returns (i1) { (if $ugt.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i40(i1: i40, i2: i40) returns (i1) { (if $ugt.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i48(i1: i48, i2: i48) returns (i1) { (if $ugt.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i56(i1: i56, i2: i56) returns (i1) { (if $ugt.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i64(i1: i64, i2: i64) returns (i1) { (if $ugt.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i80(i1: i80, i2: i80) returns (i1) { (if $ugt.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i88(i1: i88, i2: i88) returns (i1) { (if $ugt.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i96(i1: i96, i2: i96) returns (i1) { (if $ugt.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i128(i1: i128, i2: i128) returns (i1) { (if $ugt.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i160(i1: i160, i2: i160) returns (i1) { (if $ugt.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $ugt.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 > i2) }
function {:inline} $ugt.i256(i1: i256, i2: i256) returns (i1) { (if $ugt.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i1(i1: i1, i2: i1) returns (i1) { (if $sle.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i5(i1: i5, i2: i5) returns (i1) { (if $sle.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i6(i1: i6, i2: i6) returns (i1) { (if $sle.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i8(i1: i8, i2: i8) returns (i1) { (if $sle.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i16(i1: i16, i2: i16) returns (i1) { (if $sle.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i24(i1: i24, i2: i24) returns (i1) { (if $sle.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i32(i1: i32, i2: i32) returns (i1) { (if $sle.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i40(i1: i40, i2: i40) returns (i1) { (if $sle.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i48(i1: i48, i2: i48) returns (i1) { (if $sle.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i56(i1: i56, i2: i56) returns (i1) { (if $sle.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i64(i1: i64, i2: i64) returns (i1) { (if $sle.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i80(i1: i80, i2: i80) returns (i1) { (if $sle.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i88(i1: i88, i2: i88) returns (i1) { (if $sle.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i96(i1: i96, i2: i96) returns (i1) { (if $sle.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i128(i1: i128, i2: i128) returns (i1) { (if $sle.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i160(i1: i160, i2: i160) returns (i1) { (if $sle.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $sle.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 <= i2) }
function {:inline} $sle.i256(i1: i256, i2: i256) returns (i1) { (if $sle.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 < i2) }
function {:inline} $slt.i1(i1: i1, i2: i1) returns (i1) { (if $slt.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 < i2) }
function {:inline} $slt.i5(i1: i5, i2: i5) returns (i1) { (if $slt.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 < i2) }
function {:inline} $slt.i6(i1: i6, i2: i6) returns (i1) { (if $slt.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 < i2) }
function {:inline} $slt.i8(i1: i8, i2: i8) returns (i1) { (if $slt.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 < i2) }
function {:inline} $slt.i16(i1: i16, i2: i16) returns (i1) { (if $slt.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 < i2) }
function {:inline} $slt.i24(i1: i24, i2: i24) returns (i1) { (if $slt.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 < i2) }
function {:inline} $slt.i32(i1: i32, i2: i32) returns (i1) { (if $slt.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 < i2) }
function {:inline} $slt.i40(i1: i40, i2: i40) returns (i1) { (if $slt.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 < i2) }
function {:inline} $slt.i48(i1: i48, i2: i48) returns (i1) { (if $slt.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 < i2) }
function {:inline} $slt.i56(i1: i56, i2: i56) returns (i1) { (if $slt.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 < i2) }
function {:inline} $slt.i64(i1: i64, i2: i64) returns (i1) { (if $slt.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 < i2) }
function {:inline} $slt.i80(i1: i80, i2: i80) returns (i1) { (if $slt.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 < i2) }
function {:inline} $slt.i88(i1: i88, i2: i88) returns (i1) { (if $slt.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 < i2) }
function {:inline} $slt.i96(i1: i96, i2: i96) returns (i1) { (if $slt.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 < i2) }
function {:inline} $slt.i128(i1: i128, i2: i128) returns (i1) { (if $slt.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 < i2) }
function {:inline} $slt.i160(i1: i160, i2: i160) returns (i1) { (if $slt.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $slt.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 < i2) }
function {:inline} $slt.i256(i1: i256, i2: i256) returns (i1) { (if $slt.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i1(i1: i1, i2: i1) returns (i1) { (if $sge.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i5(i1: i5, i2: i5) returns (i1) { (if $sge.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i6(i1: i6, i2: i6) returns (i1) { (if $sge.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i8(i1: i8, i2: i8) returns (i1) { (if $sge.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i16(i1: i16, i2: i16) returns (i1) { (if $sge.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i24(i1: i24, i2: i24) returns (i1) { (if $sge.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i32(i1: i32, i2: i32) returns (i1) { (if $sge.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i40(i1: i40, i2: i40) returns (i1) { (if $sge.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i48(i1: i48, i2: i48) returns (i1) { (if $sge.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i56(i1: i56, i2: i56) returns (i1) { (if $sge.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i64(i1: i64, i2: i64) returns (i1) { (if $sge.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i80(i1: i80, i2: i80) returns (i1) { (if $sge.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i88(i1: i88, i2: i88) returns (i1) { (if $sge.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i96(i1: i96, i2: i96) returns (i1) { (if $sge.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i128(i1: i128, i2: i128) returns (i1) { (if $sge.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i160(i1: i160, i2: i160) returns (i1) { (if $sge.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $sge.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 >= i2) }
function {:inline} $sge.i256(i1: i256, i2: i256) returns (i1) { (if $sge.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i1(i1: i1, i2: i1) returns (i1) { (if $sgt.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i5(i1: i5, i2: i5) returns (i1) { (if $sgt.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i6(i1: i6, i2: i6) returns (i1) { (if $sgt.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i8(i1: i8, i2: i8) returns (i1) { (if $sgt.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i16(i1: i16, i2: i16) returns (i1) { (if $sgt.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i24(i1: i24, i2: i24) returns (i1) { (if $sgt.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i32(i1: i32, i2: i32) returns (i1) { (if $sgt.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i40(i1: i40, i2: i40) returns (i1) { (if $sgt.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i48(i1: i48, i2: i48) returns (i1) { (if $sgt.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i56(i1: i56, i2: i56) returns (i1) { (if $sgt.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i64(i1: i64, i2: i64) returns (i1) { (if $sgt.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i80(i1: i80, i2: i80) returns (i1) { (if $sgt.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i88(i1: i88, i2: i88) returns (i1) { (if $sgt.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i96(i1: i96, i2: i96) returns (i1) { (if $sgt.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i128(i1: i128, i2: i128) returns (i1) { (if $sgt.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i160(i1: i160, i2: i160) returns (i1) { (if $sgt.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $sgt.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 > i2) }
function {:inline} $sgt.i256(i1: i256, i2: i256) returns (i1) { (if $sgt.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 == i2) }
function {:inline} $eq.i1(i1: i1, i2: i1) returns (i1) { (if $eq.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 == i2) }
function {:inline} $eq.i5(i1: i5, i2: i5) returns (i1) { (if $eq.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 == i2) }
function {:inline} $eq.i6(i1: i6, i2: i6) returns (i1) { (if $eq.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 == i2) }
function {:inline} $eq.i8(i1: i8, i2: i8) returns (i1) { (if $eq.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 == i2) }
function {:inline} $eq.i16(i1: i16, i2: i16) returns (i1) { (if $eq.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 == i2) }
function {:inline} $eq.i24(i1: i24, i2: i24) returns (i1) { (if $eq.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 == i2) }
function {:inline} $eq.i32(i1: i32, i2: i32) returns (i1) { (if $eq.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 == i2) }
function {:inline} $eq.i40(i1: i40, i2: i40) returns (i1) { (if $eq.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 == i2) }
function {:inline} $eq.i48(i1: i48, i2: i48) returns (i1) { (if $eq.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 == i2) }
function {:inline} $eq.i56(i1: i56, i2: i56) returns (i1) { (if $eq.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 == i2) }
function {:inline} $eq.i64(i1: i64, i2: i64) returns (i1) { (if $eq.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 == i2) }
function {:inline} $eq.i80(i1: i80, i2: i80) returns (i1) { (if $eq.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 == i2) }
function {:inline} $eq.i88(i1: i88, i2: i88) returns (i1) { (if $eq.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 == i2) }
function {:inline} $eq.i96(i1: i96, i2: i96) returns (i1) { (if $eq.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 == i2) }
function {:inline} $eq.i128(i1: i128, i2: i128) returns (i1) { (if $eq.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 == i2) }
function {:inline} $eq.i160(i1: i160, i2: i160) returns (i1) { (if $eq.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $eq.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 == i2) }
function {:inline} $eq.i256(i1: i256, i2: i256) returns (i1) { (if $eq.i256.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i1.bool(i1: i1, i2: i1) returns (bool) { (i1 != i2) }
function {:inline} $ne.i1(i1: i1, i2: i1) returns (i1) { (if $ne.i1.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i5.bool(i1: i5, i2: i5) returns (bool) { (i1 != i2) }
function {:inline} $ne.i5(i1: i5, i2: i5) returns (i1) { (if $ne.i5.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i6.bool(i1: i6, i2: i6) returns (bool) { (i1 != i2) }
function {:inline} $ne.i6(i1: i6, i2: i6) returns (i1) { (if $ne.i6.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i8.bool(i1: i8, i2: i8) returns (bool) { (i1 != i2) }
function {:inline} $ne.i8(i1: i8, i2: i8) returns (i1) { (if $ne.i8.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i16.bool(i1: i16, i2: i16) returns (bool) { (i1 != i2) }
function {:inline} $ne.i16(i1: i16, i2: i16) returns (i1) { (if $ne.i16.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i24.bool(i1: i24, i2: i24) returns (bool) { (i1 != i2) }
function {:inline} $ne.i24(i1: i24, i2: i24) returns (i1) { (if $ne.i24.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i32.bool(i1: i32, i2: i32) returns (bool) { (i1 != i2) }
function {:inline} $ne.i32(i1: i32, i2: i32) returns (i1) { (if $ne.i32.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i40.bool(i1: i40, i2: i40) returns (bool) { (i1 != i2) }
function {:inline} $ne.i40(i1: i40, i2: i40) returns (i1) { (if $ne.i40.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i48.bool(i1: i48, i2: i48) returns (bool) { (i1 != i2) }
function {:inline} $ne.i48(i1: i48, i2: i48) returns (i1) { (if $ne.i48.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i56.bool(i1: i56, i2: i56) returns (bool) { (i1 != i2) }
function {:inline} $ne.i56(i1: i56, i2: i56) returns (i1) { (if $ne.i56.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i64.bool(i1: i64, i2: i64) returns (bool) { (i1 != i2) }
function {:inline} $ne.i64(i1: i64, i2: i64) returns (i1) { (if $ne.i64.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i80.bool(i1: i80, i2: i80) returns (bool) { (i1 != i2) }
function {:inline} $ne.i80(i1: i80, i2: i80) returns (i1) { (if $ne.i80.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i88.bool(i1: i88, i2: i88) returns (bool) { (i1 != i2) }
function {:inline} $ne.i88(i1: i88, i2: i88) returns (i1) { (if $ne.i88.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i96.bool(i1: i96, i2: i96) returns (bool) { (i1 != i2) }
function {:inline} $ne.i96(i1: i96, i2: i96) returns (i1) { (if $ne.i96.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i128.bool(i1: i128, i2: i128) returns (bool) { (i1 != i2) }
function {:inline} $ne.i128(i1: i128, i2: i128) returns (i1) { (if $ne.i128.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i160.bool(i1: i160, i2: i160) returns (bool) { (i1 != i2) }
function {:inline} $ne.i160(i1: i160, i2: i160) returns (i1) { (if $ne.i160.bool(i1, i2) then 1 else 0) }
function {:inline} $ne.i256.bool(i1: i256, i2: i256) returns (bool) { (i1 != i2) }
function {:inline} $ne.i256(i1: i256, i2: i256) returns (i1) { (if $ne.i256.bool(i1, i2) then 1 else 0) }
// Integer load/store operations
function {:inline} $load.i1(M: [ref] i1, p: ref) returns (i1) { M[p] }
function {:inline} $store.i1(M: [ref] i1, p: ref, i: i1) returns ([ref] i1) { M[p := i] }
function {:inline} $load.i5(M: [ref] i5, p: ref) returns (i5) { M[p] }
function {:inline} $store.i5(M: [ref] i5, p: ref, i: i5) returns ([ref] i5) { M[p := i] }
function {:inline} $load.i6(M: [ref] i6, p: ref) returns (i6) { M[p] }
function {:inline} $store.i6(M: [ref] i6, p: ref, i: i6) returns ([ref] i6) { M[p := i] }
function {:inline} $load.i8(M: [ref] i8, p: ref) returns (i8) { M[p] }
function {:inline} $store.i8(M: [ref] i8, p: ref, i: i8) returns ([ref] i8) { M[p := i] }
function {:inline} $load.i16(M: [ref] i16, p: ref) returns (i16) { M[p] }
function {:inline} $store.i16(M: [ref] i16, p: ref, i: i16) returns ([ref] i16) { M[p := i] }
function {:inline} $load.i24(M: [ref] i24, p: ref) returns (i24) { M[p] }
function {:inline} $store.i24(M: [ref] i24, p: ref, i: i24) returns ([ref] i24) { M[p := i] }
function {:inline} $load.i32(M: [ref] i32, p: ref) returns (i32) { M[p] }
function {:inline} $store.i32(M: [ref] i32, p: ref, i: i32) returns ([ref] i32) { M[p := i] }
function {:inline} $load.i40(M: [ref] i40, p: ref) returns (i40) { M[p] }
function {:inline} $store.i40(M: [ref] i40, p: ref, i: i40) returns ([ref] i40) { M[p := i] }
function {:inline} $load.i48(M: [ref] i48, p: ref) returns (i48) { M[p] }
function {:inline} $store.i48(M: [ref] i48, p: ref, i: i48) returns ([ref] i48) { M[p := i] }
function {:inline} $load.i56(M: [ref] i56, p: ref) returns (i56) { M[p] }
function {:inline} $store.i56(M: [ref] i56, p: ref, i: i56) returns ([ref] i56) { M[p := i] }
function {:inline} $load.i64(M: [ref] i64, p: ref) returns (i64) { M[p] }
function {:inline} $store.i64(M: [ref] i64, p: ref, i: i64) returns ([ref] i64) { M[p := i] }
function {:inline} $load.i80(M: [ref] i80, p: ref) returns (i80) { M[p] }
function {:inline} $store.i80(M: [ref] i80, p: ref, i: i80) returns ([ref] i80) { M[p := i] }
function {:inline} $load.i88(M: [ref] i88, p: ref) returns (i88) { M[p] }
function {:inline} $store.i88(M: [ref] i88, p: ref, i: i88) returns ([ref] i88) { M[p := i] }
function {:inline} $load.i96(M: [ref] i96, p: ref) returns (i96) { M[p] }
function {:inline} $store.i96(M: [ref] i96, p: ref, i: i96) returns ([ref] i96) { M[p := i] }
function {:inline} $load.i128(M: [ref] i128, p: ref) returns (i128) { M[p] }
function {:inline} $store.i128(M: [ref] i128, p: ref, i: i128) returns ([ref] i128) { M[p := i] }
function {:inline} $load.i160(M: [ref] i160, p: ref) returns (i160) { M[p] }
function {:inline} $store.i160(M: [ref] i160, p: ref, i: i160) returns ([ref] i160) { M[p := i] }
function {:inline} $load.i256(M: [ref] i256, p: ref) returns (i256) { M[p] }
function {:inline} $store.i256(M: [ref] i256, p: ref, i: i256) returns ([ref] i256) { M[p := i] }
// Conversion between integer types
function {:inline} $trunc.i5.i1(i: i5) returns (i1) { i }
function {:inline} $trunc.i6.i1(i: i6) returns (i1) { i }
function {:inline} $trunc.i8.i1(i: i8) returns (i1) { i }
function {:inline} $trunc.i16.i1(i: i16) returns (i1) { i }
function {:inline} $trunc.i24.i1(i: i24) returns (i1) { i }
function {:inline} $trunc.i32.i1(i: i32) returns (i1) { i }
function {:inline} $trunc.i40.i1(i: i40) returns (i1) { i }
function {:inline} $trunc.i48.i1(i: i48) returns (i1) { i }
function {:inline} $trunc.i56.i1(i: i56) returns (i1) { i }
function {:inline} $trunc.i64.i1(i: i64) returns (i1) { i }
function {:inline} $trunc.i80.i1(i: i80) returns (i1) { i }
function {:inline} $trunc.i88.i1(i: i88) returns (i1) { i }
function {:inline} $trunc.i96.i1(i: i96) returns (i1) { i }
function {:inline} $trunc.i128.i1(i: i128) returns (i1) { i }
function {:inline} $trunc.i160.i1(i: i160) returns (i1) { i }
function {:inline} $trunc.i256.i1(i: i256) returns (i1) { i }
function {:inline} $trunc.i6.i5(i: i6) returns (i5) { i }
function {:inline} $trunc.i8.i5(i: i8) returns (i5) { i }
function {:inline} $trunc.i16.i5(i: i16) returns (i5) { i }
function {:inline} $trunc.i24.i5(i: i24) returns (i5) { i }
function {:inline} $trunc.i32.i5(i: i32) returns (i5) { i }
function {:inline} $trunc.i40.i5(i: i40) returns (i5) { i }
function {:inline} $trunc.i48.i5(i: i48) returns (i5) { i }
function {:inline} $trunc.i56.i5(i: i56) returns (i5) { i }
function {:inline} $trunc.i64.i5(i: i64) returns (i5) { i }
function {:inline} $trunc.i80.i5(i: i80) returns (i5) { i }
function {:inline} $trunc.i88.i5(i: i88) returns (i5) { i }
function {:inline} $trunc.i96.i5(i: i96) returns (i5) { i }
function {:inline} $trunc.i128.i5(i: i128) returns (i5) { i }
function {:inline} $trunc.i160.i5(i: i160) returns (i5) { i }
function {:inline} $trunc.i256.i5(i: i256) returns (i5) { i }
function {:inline} $trunc.i8.i6(i: i8) returns (i6) { i }
function {:inline} $trunc.i16.i6(i: i16) returns (i6) { i }
function {:inline} $trunc.i24.i6(i: i24) returns (i6) { i }
function {:inline} $trunc.i32.i6(i: i32) returns (i6) { i }
function {:inline} $trunc.i40.i6(i: i40) returns (i6) { i }
function {:inline} $trunc.i48.i6(i: i48) returns (i6) { i }
function {:inline} $trunc.i56.i6(i: i56) returns (i6) { i }
function {:inline} $trunc.i64.i6(i: i64) returns (i6) { i }
function {:inline} $trunc.i80.i6(i: i80) returns (i6) { i }
function {:inline} $trunc.i88.i6(i: i88) returns (i6) { i }
function {:inline} $trunc.i96.i6(i: i96) returns (i6) { i }
function {:inline} $trunc.i128.i6(i: i128) returns (i6) { i }
function {:inline} $trunc.i160.i6(i: i160) returns (i6) { i }
function {:inline} $trunc.i256.i6(i: i256) returns (i6) { i }
function {:inline} $trunc.i16.i8(i: i16) returns (i8) { i }
function {:inline} $trunc.i24.i8(i: i24) returns (i8) { i }
function {:inline} $trunc.i32.i8(i: i32) returns (i8) { i }
function {:inline} $trunc.i40.i8(i: i40) returns (i8) { i }
function {:inline} $trunc.i48.i8(i: i48) returns (i8) { i }
function {:inline} $trunc.i56.i8(i: i56) returns (i8) { i }
function {:inline} $trunc.i64.i8(i: i64) returns (i8) { i }
function {:inline} $trunc.i80.i8(i: i80) returns (i8) { i }
function {:inline} $trunc.i88.i8(i: i88) returns (i8) { i }
function {:inline} $trunc.i96.i8(i: i96) returns (i8) { i }
function {:inline} $trunc.i128.i8(i: i128) returns (i8) { i }
function {:inline} $trunc.i160.i8(i: i160) returns (i8) { i }
function {:inline} $trunc.i256.i8(i: i256) returns (i8) { i }
function {:inline} $trunc.i24.i16(i: i24) returns (i16) { i }
function {:inline} $trunc.i32.i16(i: i32) returns (i16) { i }
function {:inline} $trunc.i40.i16(i: i40) returns (i16) { i }
function {:inline} $trunc.i48.i16(i: i48) returns (i16) { i }
function {:inline} $trunc.i56.i16(i: i56) returns (i16) { i }
function {:inline} $trunc.i64.i16(i: i64) returns (i16) { i }
function {:inline} $trunc.i80.i16(i: i80) returns (i16) { i }
function {:inline} $trunc.i88.i16(i: i88) returns (i16) { i }
function {:inline} $trunc.i96.i16(i: i96) returns (i16) { i }
function {:inline} $trunc.i128.i16(i: i128) returns (i16) { i }
function {:inline} $trunc.i160.i16(i: i160) returns (i16) { i }
function {:inline} $trunc.i256.i16(i: i256) returns (i16) { i }
function {:inline} $trunc.i32.i24(i: i32) returns (i24) { i }
function {:inline} $trunc.i40.i24(i: i40) returns (i24) { i }
function {:inline} $trunc.i48.i24(i: i48) returns (i24) { i }
function {:inline} $trunc.i56.i24(i: i56) returns (i24) { i }
function {:inline} $trunc.i64.i24(i: i64) returns (i24) { i }
function {:inline} $trunc.i80.i24(i: i80) returns (i24) { i }
function {:inline} $trunc.i88.i24(i: i88) returns (i24) { i }
function {:inline} $trunc.i96.i24(i: i96) returns (i24) { i }
function {:inline} $trunc.i128.i24(i: i128) returns (i24) { i }
function {:inline} $trunc.i160.i24(i: i160) returns (i24) { i }
function {:inline} $trunc.i256.i24(i: i256) returns (i24) { i }
function {:inline} $trunc.i40.i32(i: i40) returns (i32) { i }
function {:inline} $trunc.i48.i32(i: i48) returns (i32) { i }
function {:inline} $trunc.i56.i32(i: i56) returns (i32) { i }
function {:inline} $trunc.i64.i32(i: i64) returns (i32) { i }
function {:inline} $trunc.i80.i32(i: i80) returns (i32) { i }
function {:inline} $trunc.i88.i32(i: i88) returns (i32) { i }
function {:inline} $trunc.i96.i32(i: i96) returns (i32) { i }
function {:inline} $trunc.i128.i32(i: i128) returns (i32) { i }
function {:inline} $trunc.i160.i32(i: i160) returns (i32) { i }
function {:inline} $trunc.i256.i32(i: i256) returns (i32) { i }
function {:inline} $trunc.i48.i40(i: i48) returns (i40) { i }
function {:inline} $trunc.i56.i40(i: i56) returns (i40) { i }
function {:inline} $trunc.i64.i40(i: i64) returns (i40) { i }
function {:inline} $trunc.i80.i40(i: i80) returns (i40) { i }
function {:inline} $trunc.i88.i40(i: i88) returns (i40) { i }
function {:inline} $trunc.i96.i40(i: i96) returns (i40) { i }
function {:inline} $trunc.i128.i40(i: i128) returns (i40) { i }
function {:inline} $trunc.i160.i40(i: i160) returns (i40) { i }
function {:inline} $trunc.i256.i40(i: i256) returns (i40) { i }
function {:inline} $trunc.i56.i48(i: i56) returns (i48) { i }
function {:inline} $trunc.i64.i48(i: i64) returns (i48) { i }
function {:inline} $trunc.i80.i48(i: i80) returns (i48) { i }
function {:inline} $trunc.i88.i48(i: i88) returns (i48) { i }
function {:inline} $trunc.i96.i48(i: i96) returns (i48) { i }
function {:inline} $trunc.i128.i48(i: i128) returns (i48) { i }
function {:inline} $trunc.i160.i48(i: i160) returns (i48) { i }
function {:inline} $trunc.i256.i48(i: i256) returns (i48) { i }
function {:inline} $trunc.i64.i56(i: i64) returns (i56) { i }
function {:inline} $trunc.i80.i56(i: i80) returns (i56) { i }
function {:inline} $trunc.i88.i56(i: i88) returns (i56) { i }
function {:inline} $trunc.i96.i56(i: i96) returns (i56) { i }
function {:inline} $trunc.i128.i56(i: i128) returns (i56) { i }
function {:inline} $trunc.i160.i56(i: i160) returns (i56) { i }
function {:inline} $trunc.i256.i56(i: i256) returns (i56) { i }
function {:inline} $trunc.i80.i64(i: i80) returns (i64) { i }
function {:inline} $trunc.i88.i64(i: i88) returns (i64) { i }
function {:inline} $trunc.i96.i64(i: i96) returns (i64) { i }
function {:inline} $trunc.i128.i64(i: i128) returns (i64) { i }
function {:inline} $trunc.i160.i64(i: i160) returns (i64) { i }
function {:inline} $trunc.i256.i64(i: i256) returns (i64) { i }
function {:inline} $trunc.i88.i80(i: i88) returns (i80) { i }
function {:inline} $trunc.i96.i80(i: i96) returns (i80) { i }
function {:inline} $trunc.i128.i80(i: i128) returns (i80) { i }
function {:inline} $trunc.i160.i80(i: i160) returns (i80) { i }
function {:inline} $trunc.i256.i80(i: i256) returns (i80) { i }
function {:inline} $trunc.i96.i88(i: i96) returns (i88) { i }
function {:inline} $trunc.i128.i88(i: i128) returns (i88) { i }
function {:inline} $trunc.i160.i88(i: i160) returns (i88) { i }
function {:inline} $trunc.i256.i88(i: i256) returns (i88) { i }
function {:inline} $trunc.i128.i96(i: i128) returns (i96) { i }
function {:inline} $trunc.i160.i96(i: i160) returns (i96) { i }
function {:inline} $trunc.i256.i96(i: i256) returns (i96) { i }
function {:inline} $trunc.i160.i128(i: i160) returns (i128) { i }
function {:inline} $trunc.i256.i128(i: i256) returns (i128) { i }
function {:inline} $trunc.i256.i160(i: i256) returns (i160) { i }
function {:inline} $sext.i1.i5(i: i1) returns (i5) { i }
function {:inline} $sext.i1.i6(i: i1) returns (i6) { i }
function {:inline} $sext.i1.i8(i: i1) returns (i8) { i }
function {:inline} $sext.i1.i16(i: i1) returns (i16) { i }
function {:inline} $sext.i1.i24(i: i1) returns (i24) { i }
function {:inline} $sext.i1.i32(i: i1) returns (i32) { i }
function {:inline} $sext.i1.i40(i: i1) returns (i40) { i }
function {:inline} $sext.i1.i48(i: i1) returns (i48) { i }
function {:inline} $sext.i1.i56(i: i1) returns (i56) { i }
function {:inline} $sext.i1.i64(i: i1) returns (i64) { i }
function {:inline} $sext.i1.i80(i: i1) returns (i80) { i }
function {:inline} $sext.i1.i88(i: i1) returns (i88) { i }
function {:inline} $sext.i1.i96(i: i1) returns (i96) { i }
function {:inline} $sext.i1.i128(i: i1) returns (i128) { i }
function {:inline} $sext.i1.i160(i: i1) returns (i160) { i }
function {:inline} $sext.i1.i256(i: i1) returns (i256) { i }
function {:inline} $sext.i5.i6(i: i5) returns (i6) { i }
function {:inline} $sext.i5.i8(i: i5) returns (i8) { i }
function {:inline} $sext.i5.i16(i: i5) returns (i16) { i }
function {:inline} $sext.i5.i24(i: i5) returns (i24) { i }
function {:inline} $sext.i5.i32(i: i5) returns (i32) { i }
function {:inline} $sext.i5.i40(i: i5) returns (i40) { i }
function {:inline} $sext.i5.i48(i: i5) returns (i48) { i }
function {:inline} $sext.i5.i56(i: i5) returns (i56) { i }
function {:inline} $sext.i5.i64(i: i5) returns (i64) { i }
function {:inline} $sext.i5.i80(i: i5) returns (i80) { i }
function {:inline} $sext.i5.i88(i: i5) returns (i88) { i }
function {:inline} $sext.i5.i96(i: i5) returns (i96) { i }
function {:inline} $sext.i5.i128(i: i5) returns (i128) { i }
function {:inline} $sext.i5.i160(i: i5) returns (i160) { i }
function {:inline} $sext.i5.i256(i: i5) returns (i256) { i }
function {:inline} $sext.i6.i8(i: i6) returns (i8) { i }
function {:inline} $sext.i6.i16(i: i6) returns (i16) { i }
function {:inline} $sext.i6.i24(i: i6) returns (i24) { i }
function {:inline} $sext.i6.i32(i: i6) returns (i32) { i }
function {:inline} $sext.i6.i40(i: i6) returns (i40) { i }
function {:inline} $sext.i6.i48(i: i6) returns (i48) { i }
function {:inline} $sext.i6.i56(i: i6) returns (i56) { i }
function {:inline} $sext.i6.i64(i: i6) returns (i64) { i }
function {:inline} $sext.i6.i80(i: i6) returns (i80) { i }
function {:inline} $sext.i6.i88(i: i6) returns (i88) { i }
function {:inline} $sext.i6.i96(i: i6) returns (i96) { i }
function {:inline} $sext.i6.i128(i: i6) returns (i128) { i }
function {:inline} $sext.i6.i160(i: i6) returns (i160) { i }
function {:inline} $sext.i6.i256(i: i6) returns (i256) { i }
function {:inline} $sext.i8.i16(i: i8) returns (i16) { i }
function {:inline} $sext.i8.i24(i: i8) returns (i24) { i }
function {:inline} $sext.i8.i32(i: i8) returns (i32) { i }
function {:inline} $sext.i8.i40(i: i8) returns (i40) { i }
function {:inline} $sext.i8.i48(i: i8) returns (i48) { i }
function {:inline} $sext.i8.i56(i: i8) returns (i56) { i }
function {:inline} $sext.i8.i64(i: i8) returns (i64) { i }
function {:inline} $sext.i8.i80(i: i8) returns (i80) { i }
function {:inline} $sext.i8.i88(i: i8) returns (i88) { i }
function {:inline} $sext.i8.i96(i: i8) returns (i96) { i }
function {:inline} $sext.i8.i128(i: i8) returns (i128) { i }
function {:inline} $sext.i8.i160(i: i8) returns (i160) { i }
function {:inline} $sext.i8.i256(i: i8) returns (i256) { i }
function {:inline} $sext.i16.i24(i: i16) returns (i24) { i }
function {:inline} $sext.i16.i32(i: i16) returns (i32) { i }
function {:inline} $sext.i16.i40(i: i16) returns (i40) { i }
function {:inline} $sext.i16.i48(i: i16) returns (i48) { i }
function {:inline} $sext.i16.i56(i: i16) returns (i56) { i }
function {:inline} $sext.i16.i64(i: i16) returns (i64) { i }
function {:inline} $sext.i16.i80(i: i16) returns (i80) { i }
function {:inline} $sext.i16.i88(i: i16) returns (i88) { i }
function {:inline} $sext.i16.i96(i: i16) returns (i96) { i }
function {:inline} $sext.i16.i128(i: i16) returns (i128) { i }
function {:inline} $sext.i16.i160(i: i16) returns (i160) { i }
function {:inline} $sext.i16.i256(i: i16) returns (i256) { i }
function {:inline} $sext.i24.i32(i: i24) returns (i32) { i }
function {:inline} $sext.i24.i40(i: i24) returns (i40) { i }
function {:inline} $sext.i24.i48(i: i24) returns (i48) { i }
function {:inline} $sext.i24.i56(i: i24) returns (i56) { i }
function {:inline} $sext.i24.i64(i: i24) returns (i64) { i }
function {:inline} $sext.i24.i80(i: i24) returns (i80) { i }
function {:inline} $sext.i24.i88(i: i24) returns (i88) { i }
function {:inline} $sext.i24.i96(i: i24) returns (i96) { i }
function {:inline} $sext.i24.i128(i: i24) returns (i128) { i }
function {:inline} $sext.i24.i160(i: i24) returns (i160) { i }
function {:inline} $sext.i24.i256(i: i24) returns (i256) { i }
function {:inline} $sext.i32.i40(i: i32) returns (i40) { i }
function {:inline} $sext.i32.i48(i: i32) returns (i48) { i }
function {:inline} $sext.i32.i56(i: i32) returns (i56) { i }
function {:inline} $sext.i32.i64(i: i32) returns (i64) { i }
function {:inline} $sext.i32.i80(i: i32) returns (i80) { i }
function {:inline} $sext.i32.i88(i: i32) returns (i88) { i }
function {:inline} $sext.i32.i96(i: i32) returns (i96) { i }
function {:inline} $sext.i32.i128(i: i32) returns (i128) { i }
function {:inline} $sext.i32.i160(i: i32) returns (i160) { i }
function {:inline} $sext.i32.i256(i: i32) returns (i256) { i }
function {:inline} $sext.i40.i48(i: i40) returns (i48) { i }
function {:inline} $sext.i40.i56(i: i40) returns (i56) { i }
function {:inline} $sext.i40.i64(i: i40) returns (i64) { i }
function {:inline} $sext.i40.i80(i: i40) returns (i80) { i }
function {:inline} $sext.i40.i88(i: i40) returns (i88) { i }
function {:inline} $sext.i40.i96(i: i40) returns (i96) { i }
function {:inline} $sext.i40.i128(i: i40) returns (i128) { i }
function {:inline} $sext.i40.i160(i: i40) returns (i160) { i }
function {:inline} $sext.i40.i256(i: i40) returns (i256) { i }
function {:inline} $sext.i48.i56(i: i48) returns (i56) { i }
function {:inline} $sext.i48.i64(i: i48) returns (i64) { i }
function {:inline} $sext.i48.i80(i: i48) returns (i80) { i }
function {:inline} $sext.i48.i88(i: i48) returns (i88) { i }
function {:inline} $sext.i48.i96(i: i48) returns (i96) { i }
function {:inline} $sext.i48.i128(i: i48) returns (i128) { i }
function {:inline} $sext.i48.i160(i: i48) returns (i160) { i }
function {:inline} $sext.i48.i256(i: i48) returns (i256) { i }
function {:inline} $sext.i56.i64(i: i56) returns (i64) { i }
function {:inline} $sext.i56.i80(i: i56) returns (i80) { i }
function {:inline} $sext.i56.i88(i: i56) returns (i88) { i }
function {:inline} $sext.i56.i96(i: i56) returns (i96) { i }
function {:inline} $sext.i56.i128(i: i56) returns (i128) { i }
function {:inline} $sext.i56.i160(i: i56) returns (i160) { i }
function {:inline} $sext.i56.i256(i: i56) returns (i256) { i }
function {:inline} $sext.i64.i80(i: i64) returns (i80) { i }
function {:inline} $sext.i64.i88(i: i64) returns (i88) { i }
function {:inline} $sext.i64.i96(i: i64) returns (i96) { i }
function {:inline} $sext.i64.i128(i: i64) returns (i128) { i }
function {:inline} $sext.i64.i160(i: i64) returns (i160) { i }
function {:inline} $sext.i64.i256(i: i64) returns (i256) { i }
function {:inline} $sext.i80.i88(i: i80) returns (i88) { i }
function {:inline} $sext.i80.i96(i: i80) returns (i96) { i }
function {:inline} $sext.i80.i128(i: i80) returns (i128) { i }
function {:inline} $sext.i80.i160(i: i80) returns (i160) { i }
function {:inline} $sext.i80.i256(i: i80) returns (i256) { i }
function {:inline} $sext.i88.i96(i: i88) returns (i96) { i }
function {:inline} $sext.i88.i128(i: i88) returns (i128) { i }
function {:inline} $sext.i88.i160(i: i88) returns (i160) { i }
function {:inline} $sext.i88.i256(i: i88) returns (i256) { i }
function {:inline} $sext.i96.i128(i: i96) returns (i128) { i }
function {:inline} $sext.i96.i160(i: i96) returns (i160) { i }
function {:inline} $sext.i96.i256(i: i96) returns (i256) { i }
function {:inline} $sext.i128.i160(i: i128) returns (i160) { i }
function {:inline} $sext.i128.i256(i: i128) returns (i256) { i }
function {:inline} $sext.i160.i256(i: i160) returns (i256) { i }
function {:inline} $zext.i1.i5(i: i1) returns (i5) { i }
function {:inline} $zext.i1.i6(i: i1) returns (i6) { i }
function {:inline} $zext.i1.i8(i: i1) returns (i8) { i }
function {:inline} $zext.i1.i16(i: i1) returns (i16) { i }
function {:inline} $zext.i1.i24(i: i1) returns (i24) { i }
function {:inline} $zext.i1.i32(i: i1) returns (i32) { i }
function {:inline} $zext.i1.i40(i: i1) returns (i40) { i }
function {:inline} $zext.i1.i48(i: i1) returns (i48) { i }
function {:inline} $zext.i1.i56(i: i1) returns (i56) { i }
function {:inline} $zext.i1.i64(i: i1) returns (i64) { i }
function {:inline} $zext.i1.i80(i: i1) returns (i80) { i }
function {:inline} $zext.i1.i88(i: i1) returns (i88) { i }
function {:inline} $zext.i1.i96(i: i1) returns (i96) { i }
function {:inline} $zext.i1.i128(i: i1) returns (i128) { i }
function {:inline} $zext.i1.i160(i: i1) returns (i160) { i }
function {:inline} $zext.i1.i256(i: i1) returns (i256) { i }
function {:inline} $zext.i5.i6(i: i5) returns (i6) { i }
function {:inline} $zext.i5.i8(i: i5) returns (i8) { i }
function {:inline} $zext.i5.i16(i: i5) returns (i16) { i }
function {:inline} $zext.i5.i24(i: i5) returns (i24) { i }
function {:inline} $zext.i5.i32(i: i5) returns (i32) { i }
function {:inline} $zext.i5.i40(i: i5) returns (i40) { i }
function {:inline} $zext.i5.i48(i: i5) returns (i48) { i }
function {:inline} $zext.i5.i56(i: i5) returns (i56) { i }
function {:inline} $zext.i5.i64(i: i5) returns (i64) { i }
function {:inline} $zext.i5.i80(i: i5) returns (i80) { i }
function {:inline} $zext.i5.i88(i: i5) returns (i88) { i }
function {:inline} $zext.i5.i96(i: i5) returns (i96) { i }
function {:inline} $zext.i5.i128(i: i5) returns (i128) { i }
function {:inline} $zext.i5.i160(i: i5) returns (i160) { i }
function {:inline} $zext.i5.i256(i: i5) returns (i256) { i }
function {:inline} $zext.i6.i8(i: i6) returns (i8) { i }
function {:inline} $zext.i6.i16(i: i6) returns (i16) { i }
function {:inline} $zext.i6.i24(i: i6) returns (i24) { i }
function {:inline} $zext.i6.i32(i: i6) returns (i32) { i }
function {:inline} $zext.i6.i40(i: i6) returns (i40) { i }
function {:inline} $zext.i6.i48(i: i6) returns (i48) { i }
function {:inline} $zext.i6.i56(i: i6) returns (i56) { i }
function {:inline} $zext.i6.i64(i: i6) returns (i64) { i }
function {:inline} $zext.i6.i80(i: i6) returns (i80) { i }
function {:inline} $zext.i6.i88(i: i6) returns (i88) { i }
function {:inline} $zext.i6.i96(i: i6) returns (i96) { i }
function {:inline} $zext.i6.i128(i: i6) returns (i128) { i }
function {:inline} $zext.i6.i160(i: i6) returns (i160) { i }
function {:inline} $zext.i6.i256(i: i6) returns (i256) { i }
function {:inline} $zext.i8.i16(i: i8) returns (i16) { i }
function {:inline} $zext.i8.i24(i: i8) returns (i24) { i }
function {:inline} $zext.i8.i32(i: i8) returns (i32) { i }
function {:inline} $zext.i8.i40(i: i8) returns (i40) { i }
function {:inline} $zext.i8.i48(i: i8) returns (i48) { i }
function {:inline} $zext.i8.i56(i: i8) returns (i56) { i }
function {:inline} $zext.i8.i64(i: i8) returns (i64) { i }
function {:inline} $zext.i8.i80(i: i8) returns (i80) { i }
function {:inline} $zext.i8.i88(i: i8) returns (i88) { i }
function {:inline} $zext.i8.i96(i: i8) returns (i96) { i }
function {:inline} $zext.i8.i128(i: i8) returns (i128) { i }
function {:inline} $zext.i8.i160(i: i8) returns (i160) { i }
function {:inline} $zext.i8.i256(i: i8) returns (i256) { i }
function {:inline} $zext.i16.i24(i: i16) returns (i24) { i }
function {:inline} $zext.i16.i32(i: i16) returns (i32) { i }
function {:inline} $zext.i16.i40(i: i16) returns (i40) { i }
function {:inline} $zext.i16.i48(i: i16) returns (i48) { i }
function {:inline} $zext.i16.i56(i: i16) returns (i56) { i }
function {:inline} $zext.i16.i64(i: i16) returns (i64) { i }
function {:inline} $zext.i16.i80(i: i16) returns (i80) { i }
function {:inline} $zext.i16.i88(i: i16) returns (i88) { i }
function {:inline} $zext.i16.i96(i: i16) returns (i96) { i }
function {:inline} $zext.i16.i128(i: i16) returns (i128) { i }
function {:inline} $zext.i16.i160(i: i16) returns (i160) { i }
function {:inline} $zext.i16.i256(i: i16) returns (i256) { i }
function {:inline} $zext.i24.i32(i: i24) returns (i32) { i }
function {:inline} $zext.i24.i40(i: i24) returns (i40) { i }
function {:inline} $zext.i24.i48(i: i24) returns (i48) { i }
function {:inline} $zext.i24.i56(i: i24) returns (i56) { i }
function {:inline} $zext.i24.i64(i: i24) returns (i64) { i }
function {:inline} $zext.i24.i80(i: i24) returns (i80) { i }
function {:inline} $zext.i24.i88(i: i24) returns (i88) { i }
function {:inline} $zext.i24.i96(i: i24) returns (i96) { i }
function {:inline} $zext.i24.i128(i: i24) returns (i128) { i }
function {:inline} $zext.i24.i160(i: i24) returns (i160) { i }
function {:inline} $zext.i24.i256(i: i24) returns (i256) { i }
function {:inline} $zext.i32.i40(i: i32) returns (i40) { i }
function {:inline} $zext.i32.i48(i: i32) returns (i48) { i }
function {:inline} $zext.i32.i56(i: i32) returns (i56) { i }
function {:inline} $zext.i32.i64(i: i32) returns (i64) { i }
function {:inline} $zext.i32.i80(i: i32) returns (i80) { i }
function {:inline} $zext.i32.i88(i: i32) returns (i88) { i }
function {:inline} $zext.i32.i96(i: i32) returns (i96) { i }
function {:inline} $zext.i32.i128(i: i32) returns (i128) { i }
function {:inline} $zext.i32.i160(i: i32) returns (i160) { i }
function {:inline} $zext.i32.i256(i: i32) returns (i256) { i }
function {:inline} $zext.i40.i48(i: i40) returns (i48) { i }
function {:inline} $zext.i40.i56(i: i40) returns (i56) { i }
function {:inline} $zext.i40.i64(i: i40) returns (i64) { i }
function {:inline} $zext.i40.i80(i: i40) returns (i80) { i }
function {:inline} $zext.i40.i88(i: i40) returns (i88) { i }
function {:inline} $zext.i40.i96(i: i40) returns (i96) { i }
function {:inline} $zext.i40.i128(i: i40) returns (i128) { i }
function {:inline} $zext.i40.i160(i: i40) returns (i160) { i }
function {:inline} $zext.i40.i256(i: i40) returns (i256) { i }
function {:inline} $zext.i48.i56(i: i48) returns (i56) { i }
function {:inline} $zext.i48.i64(i: i48) returns (i64) { i }
function {:inline} $zext.i48.i80(i: i48) returns (i80) { i }
function {:inline} $zext.i48.i88(i: i48) returns (i88) { i }
function {:inline} $zext.i48.i96(i: i48) returns (i96) { i }
function {:inline} $zext.i48.i128(i: i48) returns (i128) { i }
function {:inline} $zext.i48.i160(i: i48) returns (i160) { i }
function {:inline} $zext.i48.i256(i: i48) returns (i256) { i }
function {:inline} $zext.i56.i64(i: i56) returns (i64) { i }
function {:inline} $zext.i56.i80(i: i56) returns (i80) { i }
function {:inline} $zext.i56.i88(i: i56) returns (i88) { i }
function {:inline} $zext.i56.i96(i: i56) returns (i96) { i }
function {:inline} $zext.i56.i128(i: i56) returns (i128) { i }
function {:inline} $zext.i56.i160(i: i56) returns (i160) { i }
function {:inline} $zext.i56.i256(i: i56) returns (i256) { i }
function {:inline} $zext.i64.i80(i: i64) returns (i80) { i }
function {:inline} $zext.i64.i88(i: i64) returns (i88) { i }
function {:inline} $zext.i64.i96(i: i64) returns (i96) { i }
function {:inline} $zext.i64.i128(i: i64) returns (i128) { i }
function {:inline} $zext.i64.i160(i: i64) returns (i160) { i }
function {:inline} $zext.i64.i256(i: i64) returns (i256) { i }
function {:inline} $zext.i80.i88(i: i80) returns (i88) { i }
function {:inline} $zext.i80.i96(i: i80) returns (i96) { i }
function {:inline} $zext.i80.i128(i: i80) returns (i128) { i }
function {:inline} $zext.i80.i160(i: i80) returns (i160) { i }
function {:inline} $zext.i80.i256(i: i80) returns (i256) { i }
function {:inline} $zext.i88.i96(i: i88) returns (i96) { i }
function {:inline} $zext.i88.i128(i: i88) returns (i128) { i }
function {:inline} $zext.i88.i160(i: i88) returns (i160) { i }
function {:inline} $zext.i88.i256(i: i88) returns (i256) { i }
function {:inline} $zext.i96.i128(i: i96) returns (i128) { i }
function {:inline} $zext.i96.i160(i: i96) returns (i160) { i }
function {:inline} $zext.i96.i256(i: i96) returns (i256) { i }
function {:inline} $zext.i128.i160(i: i128) returns (i160) { i }
function {:inline} $zext.i128.i256(i: i128) returns (i256) { i }
function {:inline} $zext.i160.i256(i: i160) returns (i256) { i }
function $extractvalue.i1(p: ref, i: int) returns (i1);
function $extractvalue.i5(p: ref, i: int) returns (i5);
function $extractvalue.i6(p: ref, i: int) returns (i6);
function $extractvalue.i8(p: ref, i: int) returns (i8);
function $extractvalue.i16(p: ref, i: int) returns (i16);
function $extractvalue.i24(p: ref, i: int) returns (i24);
function $extractvalue.i32(p: ref, i: int) returns (i32);
function $extractvalue.i40(p: ref, i: int) returns (i40);
function $extractvalue.i48(p: ref, i: int) returns (i48);
function $extractvalue.i56(p: ref, i: int) returns (i56);
function $extractvalue.i64(p: ref, i: int) returns (i64);
function $extractvalue.i80(p: ref, i: int) returns (i80);
function $extractvalue.i88(p: ref, i: int) returns (i88);
function $extractvalue.i96(p: ref, i: int) returns (i96);
function $extractvalue.i128(p: ref, i: int) returns (i128);
function $extractvalue.i160(p: ref, i: int) returns (i160);
function $extractvalue.i256(p: ref, i: int) returns (i256);
// Pointer arithmetic operations
function {:inline} $add.ref(p1: ref, p2: ref) returns (ref) { $add.i64(p1, p2) }
function {:inline} $sub.ref(p1: ref, p2: ref) returns (ref) { $sub.i64(p1, p2) }
function {:inline} $mul.ref(p1: ref, p2: ref) returns (ref) { $mul.i64(p1, p2) }

// Pointer predicates
function {:inline} $eq.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 == p2) }
function {:inline} $eq.ref(p1: ref, p2: ref) returns (i1) { (if $eq.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $ne.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 != p2) }
function {:inline} $ne.ref(p1: ref, p2: ref) returns (i1) { (if $ne.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $ugt.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 > p2) }
function {:inline} $ugt.ref(p1: ref, p2: ref) returns (i1) { (if $ugt.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $uge.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 >= p2) }
function {:inline} $uge.ref(p1: ref, p2: ref) returns (i1) { (if $uge.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $ult.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 < p2) }
function {:inline} $ult.ref(p1: ref, p2: ref) returns (i1) { (if $ult.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $ule.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 <= p2) }
function {:inline} $ule.ref(p1: ref, p2: ref) returns (i1) { (if $ule.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $sgt.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 > p2) }
function {:inline} $sgt.ref(p1: ref, p2: ref) returns (i1) { (if $sgt.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $sge.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 >= p2) }
function {:inline} $sge.ref(p1: ref, p2: ref) returns (i1) { (if $sge.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $slt.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 < p2) }
function {:inline} $slt.ref(p1: ref, p2: ref) returns (i1) { (if $slt.ref.bool(p1, p2) then 1 else 0) }
function {:inline} $sle.ref.bool(p1: ref, p2: ref) returns (bool) { (p1 <= p2) }
function {:inline} $sle.ref(p1: ref, p2: ref) returns (i1) { (if $sle.ref.bool(p1, p2) then 1 else 0) }

// Pointer load/store operations
function {:inline} $load.ref(M: [ref] ref, p: ref) returns (ref) { M[p] }
function {:inline} $store.ref(M: [ref] ref, p: ref, i: ref) returns ([ref] ref) { M[p := i] }

// Pointer conversion
function {:inline} $bitcast.ref.ref(p: ref) returns (ref) { p }
function $extractvalue.ref(p: ref, i: int) returns (ref);
// Pointer-number conversion
function {:inline} $p2i.ref.i8(p: ref) returns (i8) { $trunc.i64.i8(p) }
function {:inline} $i2p.i8.ref(i: i8) returns (ref) { $zext.i8.i64(i) }
function {:inline} $p2i.ref.i16(p: ref) returns (i16) { $trunc.i64.i16(p) }
function {:inline} $i2p.i16.ref(i: i16) returns (ref) { $zext.i16.i64(i) }
function {:inline} $p2i.ref.i32(p: ref) returns (i32) { $trunc.i64.i32(p) }
function {:inline} $i2p.i32.ref(i: i32) returns (ref) { $zext.i32.i64(i) }
function {:inline} $p2i.ref.i64(p: ref) returns (i64) { p }
function {:inline} $i2p.i64.ref(i: i64) returns (ref) { i }

function $fp(ipart: int, fpart: int, epart: int) returns (float);
// Floating-point arithmetic operations
function $abs.float(f: float) returns (float);
function $round.float(f: float) returns (float);
function $sqrt.float(f: float) returns (float);
function $fadd.float(f1: float, f2: float) returns (float);
function $fsub.float(f1: float, f2: float) returns (float);
function $fmul.float(f1: float, f2: float) returns (float);
function $fdiv.float(f1: float, f2: float) returns (float);
function $frem.float(f1: float, f2: float) returns (float);
function $min.float(f1: float, f2: float) returns (float);
function $max.float(f1: float, f2: float) returns (float);
function $fma.float(f1: float, f2: float, f3: float) returns (float);
// Floating-point predicates
function $foeq.float.bool(f1: float, f2: float) returns (bool);
function $fole.float.bool(f1: float, f2: float) returns (bool);
function $folt.float.bool(f1: float, f2: float) returns (bool);
function $foge.float.bool(f1: float, f2: float) returns (bool);
function $fogt.float.bool(f1: float, f2: float) returns (bool);
function $fone.float.bool(f1: float, f2: float) returns (bool);
function $ford.float.bool(f1: float, f2: float) returns (bool);
function $fueq.float.bool(f1: float, f2: float) returns (bool);
function $fugt.float.bool(f1: float, f2: float) returns (bool);
function $fuge.float.bool(f1: float, f2: float) returns (bool);
function $fult.float.bool(f1: float, f2: float) returns (bool);
function $fule.float.bool(f1: float, f2: float) returns (bool);
function $fune.float.bool(f1: float, f2: float) returns (bool);
function $funo.float.bool(f1: float, f2: float) returns (bool);
function $ffalse.float.bool(f1: float, f2: float) returns (bool);
function $ftrue.float.bool(f1: float, f2: float) returns (bool);
// Floating-point/integer conversion
function $bitcast.float.i8(f: float) returns (i8);
function $bitcast.float.i16(f: float) returns (i16);
function $bitcast.float.i32(f: float) returns (i32);
function $bitcast.float.i64(f: float) returns (i64);
function $bitcast.float.i80(f: float) returns (i80);
function $bitcast.i8.float(i: i8) returns (float);
function $bitcast.i16.float(i: i16) returns (float);
function $bitcast.i32.float(i: i32) returns (float);
function $bitcast.i64.float(i: i64) returns (float);
function $bitcast.i80.float(i: i80) returns (float);
function $fp2si.float.i1(f: float) returns (i1);
function $fp2si.float.i5(f: float) returns (i5);
function $fp2si.float.i6(f: float) returns (i6);
function $fp2si.float.i8(f: float) returns (i8);
function $fp2si.float.i16(f: float) returns (i16);
function $fp2si.float.i24(f: float) returns (i24);
function $fp2si.float.i32(f: float) returns (i32);
function $fp2si.float.i40(f: float) returns (i40);
function $fp2si.float.i48(f: float) returns (i48);
function $fp2si.float.i56(f: float) returns (i56);
function $fp2si.float.i64(f: float) returns (i64);
function $fp2si.float.i80(f: float) returns (i80);
function $fp2si.float.i88(f: float) returns (i88);
function $fp2si.float.i96(f: float) returns (i96);
function $fp2si.float.i128(f: float) returns (i128);
function $fp2si.float.i160(f: float) returns (i160);
function $fp2si.float.i256(f: float) returns (i256);
function $fp2ui.float.i1(f: float) returns (i1);
function $fp2ui.float.i5(f: float) returns (i5);
function $fp2ui.float.i6(f: float) returns (i6);
function $fp2ui.float.i8(f: float) returns (i8);
function $fp2ui.float.i16(f: float) returns (i16);
function $fp2ui.float.i24(f: float) returns (i24);
function $fp2ui.float.i32(f: float) returns (i32);
function $fp2ui.float.i40(f: float) returns (i40);
function $fp2ui.float.i48(f: float) returns (i48);
function $fp2ui.float.i56(f: float) returns (i56);
function $fp2ui.float.i64(f: float) returns (i64);
function $fp2ui.float.i80(f: float) returns (i80);
function $fp2ui.float.i88(f: float) returns (i88);
function $fp2ui.float.i96(f: float) returns (i96);
function $fp2ui.float.i128(f: float) returns (i128);
function $fp2ui.float.i160(f: float) returns (i160);
function $fp2ui.float.i256(f: float) returns (i256);
function $si2fp.i1.float(i: i1) returns (float);
function $si2fp.i5.float(i: i5) returns (float);
function $si2fp.i6.float(i: i6) returns (float);
function $si2fp.i8.float(i: i8) returns (float);
function $si2fp.i16.float(i: i16) returns (float);
function $si2fp.i24.float(i: i24) returns (float);
function $si2fp.i32.float(i: i32) returns (float);
function $si2fp.i40.float(i: i40) returns (float);
function $si2fp.i48.float(i: i48) returns (float);
function $si2fp.i56.float(i: i56) returns (float);
function $si2fp.i64.float(i: i64) returns (float);
function $si2fp.i80.float(i: i80) returns (float);
function $si2fp.i88.float(i: i88) returns (float);
function $si2fp.i96.float(i: i96) returns (float);
function $si2fp.i128.float(i: i128) returns (float);
function $si2fp.i160.float(i: i160) returns (float);
function $si2fp.i256.float(i: i256) returns (float);
function $ui2fp.i1.float(i: i1) returns (float);
function $ui2fp.i5.float(i: i5) returns (float);
function $ui2fp.i6.float(i: i6) returns (float);
function $ui2fp.i8.float(i: i8) returns (float);
function $ui2fp.i16.float(i: i16) returns (float);
function $ui2fp.i24.float(i: i24) returns (float);
function $ui2fp.i32.float(i: i32) returns (float);
function $ui2fp.i40.float(i: i40) returns (float);
function $ui2fp.i48.float(i: i48) returns (float);
function $ui2fp.i56.float(i: i56) returns (float);
function $ui2fp.i64.float(i: i64) returns (float);
function $ui2fp.i80.float(i: i80) returns (float);
function $ui2fp.i88.float(i: i88) returns (float);
function $ui2fp.i96.float(i: i96) returns (float);
function $ui2fp.i128.float(i: i128) returns (float);
function $ui2fp.i160.float(i: i160) returns (float);
function $ui2fp.i256.float(i: i256) returns (float);
// Floating-point conversion
function $fpext.float.float(f: float) returns (float);
function $fptrunc.float.float(f: float) returns (float);
// Floating-point load/store operations
function {:inline} $load.float(M: [ref] float, p: ref) returns (float) { M[p] }
function {:inline} $store.float(M: [ref] float, p: ref, f: float) returns ([ref] float) { M[p := f] }
function {:inline} $load.unsafe.float(M: [ref] i8, p: ref) returns (float) { $bitcast.i8.float(M[p]) }
function {:inline} $store.unsafe.float(M: [ref] i8, p: ref, f: float) returns ([ref] i8) { M[p := $bitcast.float.i8(f)] }
function $extractvalue.float(p: ref, i: int) returns (float);
const __unbuffered_cnt: ref;
axiom (__unbuffered_cnt == $sub.ref(0, 1028));
const __unbuffered_p0_EAX: ref;
axiom (__unbuffered_p0_EAX == $sub.ref(0, 2056));
const __unbuffered_p1_EAX: ref;
axiom (__unbuffered_p1_EAX == $sub.ref(0, 3084));
const __unbuffered_p1_EBX: ref;
axiom (__unbuffered_p1_EBX == $sub.ref(0, 4112));
const x: ref;
axiom (x == $sub.ref(0, 5140));
const y: ref;
axiom (y == $sub.ref(0, 6168));
const x$w_buff0_used: ref;
axiom (x$w_buff0_used == $sub.ref(0, 7193));
const x$r_buff0_thd1: ref;
axiom (x$r_buff0_thd1 == $sub.ref(0, 8218));
const x$w_buff0: ref;
axiom (x$w_buff0 == $sub.ref(0, 9246));
const x$w_buff1_used: ref;
axiom (x$w_buff1_used == $sub.ref(0, 10271));
const x$r_buff1_thd1: ref;
axiom (x$r_buff1_thd1 == $sub.ref(0, 11296));
const x$w_buff1: ref;
axiom (x$w_buff1 == $sub.ref(0, 12324));
const weak$$choice0: ref;
axiom (weak$$choice0 == $sub.ref(0, 13349));
const weak$$choice2: ref;
axiom (weak$$choice2 == $sub.ref(0, 14374));
const x$flush_delayed: ref;
axiom (x$flush_delayed == $sub.ref(0, 15399));
const x$mem_tmp: ref;
axiom (x$mem_tmp == $sub.ref(0, 16427));
const x$r_buff0_thd0: ref;
axiom (x$r_buff0_thd0 == $sub.ref(0, 17452));
const x$r_buff1_thd0: ref;
axiom (x$r_buff1_thd0 == $sub.ref(0, 18477));
const x$r_buff0_thd2: ref;
axiom (x$r_buff0_thd2 == $sub.ref(0, 19502));
const x$r_buff1_thd2: ref;
axiom (x$r_buff1_thd2 == $sub.ref(0, 20527));
const y$w_buff0: ref;
axiom (y$w_buff0 == $sub.ref(0, 21555));
const y$w_buff1: ref;
axiom (y$w_buff1 == $sub.ref(0, 22583));
const y$w_buff0_used: ref;
axiom (y$w_buff0_used == $sub.ref(0, 23608));
const y$w_buff1_used: ref;
axiom (y$w_buff1_used == $sub.ref(0, 24633));
const y$r_buff0_thd0: ref;
axiom (y$r_buff0_thd0 == $sub.ref(0, 25658));
const y$r_buff1_thd0: ref;
axiom (y$r_buff1_thd0 == $sub.ref(0, 26683));
const y$r_buff0_thd1: ref;
axiom (y$r_buff0_thd1 == $sub.ref(0, 27708));
const y$r_buff1_thd1: ref;
axiom (y$r_buff1_thd1 == $sub.ref(0, 28733));
const y$r_buff0_thd2: ref;
axiom (y$r_buff0_thd2 == $sub.ref(0, 29758));
const y$r_buff1_thd2: ref;
axiom (y$r_buff1_thd2 == $sub.ref(0, 30783));
const y$flush_delayed: ref;
axiom (y$flush_delayed == $sub.ref(0, 31808));
const y$mem_tmp: ref;
axiom (y$mem_tmp == $sub.ref(0, 32836));
const main$tmp_guard0: ref;
axiom (main$tmp_guard0 == $sub.ref(0, 33861));
const main$tmp_guard1: ref;
axiom (main$tmp_guard1 == $sub.ref(0, 34886));
const x$read_delayed: ref;
axiom (x$read_delayed == $sub.ref(0, 35911));
const x$read_delayed_var: ref;
axiom (x$read_delayed_var == $sub.ref(0, 36943));
const y$read_delayed: ref;
axiom (y$read_delayed == $sub.ref(0, 37968));
const y$read_delayed_var: ref;
axiom (y$read_delayed_var == $sub.ref(0, 39000));
const .str: ref;
axiom (.str == $sub.ref(0, 40026));
const {:count 17} .str.1: ref;
axiom (.str.1 == $sub.ref(0, 41067));
const {:count 19} __PRETTY_FUNCTION__.reach_error: ref;
axiom (__PRETTY_FUNCTION__.reach_error == $sub.ref(0, 42110));
const env_value_str: ref;
axiom (env_value_str == $sub.ref(0, 43142));
const {:count 3} .str.1.12: ref;
axiom (.str.1.12 == $sub.ref(0, 44169));
const errno_global: ref;
axiom (errno_global == $sub.ref(0, 45197));
const assume_abort_if_not: ref;
axiom (assume_abort_if_not == $sub.ref(0, 46229));
procedure  assume_abort_if_not($i0: i32)
{
  var $i1: i1;
$bb0:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 4, 7} true;
  assume {:verifier.code 0} true;
  call {:cexpr "assume_abort_if_not:arg:cond"} boogie_si_record_i32($i0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 4, 7} true;
  assume {:verifier.code 0} true;
  $i1 := $ne.i32($i0, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 4, 6} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i1} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i1 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 5, 1} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
$bb2:
  assume !(($i1 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 4, 14} true;
  assume {:verifier.code 0} true;
  call abort();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 4, 14} true;
  assume {:verifier.code 0} true;
  assume false;
}
const llvm.dbg.declare: ref;
axiom (llvm.dbg.declare == $sub.ref(0, 47261));
procedure  llvm.dbg.declare($p0: ref, $p1: ref, $p2: ref);
const reach_error: ref;
axiom (reach_error == $sub.ref(0, 48293));
procedure  reach_error()
{
$bb0:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 18, 83} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 18, 83} true;
  assume {:verifier.code 0} true;
  call __assert_fail(.str, .str.1, 9, __PRETTY_FUNCTION__.reach_error);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 18, 83} true;
  assume {:verifier.code 0} true;
  assume false;
}
const __assert_fail: ref;
axiom (__assert_fail == $sub.ref(0, 49325));
procedure  __assert_fail($p0: ref, $p1: ref, $i2: i32, $p3: ref);
const __VERIFIER_assert: ref;
axiom (__VERIFIER_assert == $sub.ref(0, 50357));
procedure  __VERIFIER_assert($i0: i32)
{
  var $i1: i1;
$bb0:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 47} true;
  assume {:verifier.code 0} true;
  call {:cexpr "__VERIFIER_assert:arg:expression"} boogie_si_record_i32($i0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 47} true;
  assume {:verifier.code 0} true;
  $i1 := $ne.i32($i0, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 46} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i1} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i1 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 96} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
$bb2:
  assume !(($i1 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 59} true;
  assume {:verifier.code 0} true;
  goto $bb3;
$bb3:
  assume true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 69} true;
  assume {:verifier.code 0} true;
  call reach_error();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 83} true;
  assume {:verifier.code 0} true;
  call abort();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 19, 83} true;
  assume {:verifier.code 0} true;
  assume false;
}
const llvm.dbg.label: ref;
axiom (llvm.dbg.label == $sub.ref(0, 51389));
procedure  llvm.dbg.label($p0: ref);
const P0: ref;
axiom (P0 == $sub.ref(0, 52421));
procedure  P0($p0: ref)
  returns ($r: ref)
{
  var $i1: i8;
  var $i2: i1;
  var $i3: i8;
  var $i4: i1;
  var $i5: i32;
  var $i7: i8;
  var $i8: i1;
  var $i9: i8;
  var $i10: i1;
  var $i11: i32;
  var $i13: i32;
  var $i12: i32;
  var $i6: i32;
  var $i14: i8;
  var $i15: i1;
  var $i16: i8;
  var $i17: i1;
  var $i19: i8;
  var $i20: i1;
  var $i21: i32;
  var $i18: i32;
  var $i22: i1;
  var $i23: i8;
  var $i24: i8;
  var $i25: i1;
  var $i26: i8;
  var $i27: i1;
  var $i28: i8;
  var $i29: i1;
  var $i30: i8;
  var $i31: i1;
  var $i33: i8;
  var $i34: i1;
  var $i35: i32;
  var $i32: i32;
  var $i36: i1;
  var $i37: i8;
  var $i38: i8;
  var $i39: i1;
  var $i40: i8;
  var $i41: i1;
  var $i43: i8;
  var $i44: i1;
  var $i45: i32;
  var $i42: i32;
  var $i46: i1;
  var $i47: i8;
  var $i48: i8;
  var $i49: i1;
  var $i50: i8;
  var $i51: i1;
  var $i52: i8;
  var $i53: i1;
  var $i54: i8;
  var $i55: i1;
  var $i57: i8;
  var $i58: i1;
  var $i59: i32;
  var $i56: i32;
  var $i60: i1;
  var $i61: i8;
  var $i62: i1;
  var $i63: i8;
  var $i64: i1;
  var $i65: i8;
  var $i66: i8;
  var $i67: i1;
  var $i68: i8;
  var $i69: i32;
  var $i70: i8;
  var $i71: i1;
  var $i72: i8;
  var $i73: i1;
  var $i74: i8;
  var $i75: i1;
  var $i76: i8;
  var $i77: i1;
  var $i78: i8;
  var $i79: i1;
  var $i80: i32;
  var $i82: i8;
  var $i83: i1;
  var $i84: i8;
  var $i85: i1;
  var $i86: i32;
  var $i88: i32;
  var $i87: i32;
  var $i81: i32;
  var $i89: i8;
  var $i90: i1;
  var $i91: i32;
  var $i93: i8;
  var $i94: i1;
  var $i95: i8;
  var $i96: i1;
  var $i97: i8;
  var $i98: i1;
  var $i99: i8;
  var $i100: i1;
  var $i101: i8;
  var $i102: i1;
  var $i103: i32;
  var $i105: i8;
  var $i106: i1;
  var $i107: i8;
  var $i108: i1;
  var $i109: i32;
  var $i111: i32;
  var $i110: i32;
  var $i104: i32;
  var $i92: i32;
  var $i112: i8;
  var $i113: i1;
  var $i114: i32;
  var $i116: i8;
  var $i117: i1;
  var $i118: i8;
  var $i119: i1;
  var $i120: i8;
  var $i121: i1;
  var $i122: i8;
  var $i123: i1;
  var $i124: i8;
  var $i125: i1;
  var $i126: i32;
  var $i128: i8;
  var $i129: i1;
  var $i130: i8;
  var $i131: i1;
  var $i132: i32;
  var $i134: i32;
  var $i133: i32;
  var $i127: i32;
  var $i115: i32;
  var $i135: i8;
  var $i136: i1;
  var $i137: i8;
  var $i138: i1;
  var $i139: i32;
  var $i141: i8;
  var $i142: i1;
  var $i143: i8;
  var $i144: i1;
  var $i145: i8;
  var $i146: i1;
  var $i147: i8;
  var $i148: i1;
  var $i149: i8;
  var $i150: i1;
  var $i151: i8;
  var $i152: i1;
  var $i153: i32;
  var $i155: i8;
  var $i156: i1;
  var $i157: i8;
  var $i158: i1;
  var $i160: i8;
  var $i161: i1;
  var $i162: i32;
  var $i159: i32;
  var $i154: i32;
  var $i140: i32;
  var $i163: i1;
  var $i164: i8;
  var $i165: i8;
  var $i166: i1;
  var $i167: i8;
  var $i168: i1;
  var $i169: i32;
  var $i171: i8;
  var $i172: i1;
  var $i173: i8;
  var $i174: i1;
  var $i175: i8;
  var $i176: i1;
  var $i177: i8;
  var $i178: i1;
  var $i179: i8;
  var $i180: i1;
  var $i181: i8;
  var $i182: i1;
  var $i183: i32;
  var $i185: i8;
  var $i186: i1;
  var $i188: i8;
  var $i189: i1;
  var $i187: i1;
  var $i190: i32;
  var $i184: i32;
  var $i170: i32;
  var $i191: i1;
  var $i192: i8;
  var $i193: i8;
  var $i194: i1;
  var $i195: i8;
  var $i196: i1;
  var $i197: i32;
  var $i199: i8;
  var $i200: i1;
  var $i201: i8;
  var $i202: i1;
  var $i203: i8;
  var $i204: i1;
  var $i205: i8;
  var $i206: i1;
  var $i207: i8;
  var $i208: i1;
  var $i209: i8;
  var $i210: i1;
  var $i211: i32;
  var $i213: i8;
  var $i214: i1;
  var $i215: i8;
  var $i216: i1;
  var $i218: i8;
  var $i219: i1;
  var $i220: i32;
  var $i217: i32;
  var $i212: i32;
  var $i198: i32;
  var $i221: i1;
  var $i222: i8;
  var $i223: i8;
  var $i224: i1;
  var $i225: i8;
  var $i226: i1;
  var $i227: i32;
  var $i229: i8;
  var $i230: i1;
  var $i231: i8;
  var $i232: i1;
  var $i233: i8;
  var $i234: i1;
  var $i235: i8;
  var $i236: i1;
  var $i237: i8;
  var $i238: i1;
  var $i239: i8;
  var $i240: i1;
  var $i241: i32;
  var $i243: i8;
  var $i244: i1;
  var $i246: i8;
  var $i247: i1;
  var $i245: i1;
  var $i248: i32;
  var $i242: i32;
  var $i228: i32;
  var $i249: i1;
  var $i250: i8;
  var $i251: i32;
  var $i252: i8;
  var $i253: i1;
  var $i254: i32;
  var $i256: i32;
  var $i255: i32;
  var $i257: i8;
  var $i258: i1;
  var $i259: i8;
  var $i260: i1;
  var $i261: i32;
  var $i263: i8;
  var $i264: i1;
  var $i265: i8;
  var $i266: i1;
  var $i267: i32;
  var $i269: i32;
  var $i268: i32;
  var $i262: i32;
  var $i270: i8;
  var $i271: i1;
  var $i272: i8;
  var $i273: i1;
  var $i275: i8;
  var $i276: i1;
  var $i277: i32;
  var $i274: i32;
  var $i278: i1;
  var $i279: i8;
  var $i280: i8;
  var $i281: i1;
  var $i282: i8;
  var $i283: i1;
  var $i284: i8;
  var $i285: i1;
  var $i286: i8;
  var $i287: i1;
  var $i289: i8;
  var $i290: i1;
  var $i291: i32;
  var $i288: i32;
  var $i292: i1;
  var $i293: i8;
  var $i294: i8;
  var $i295: i1;
  var $i296: i8;
  var $i297: i1;
  var $i299: i8;
  var $i300: i1;
  var $i301: i32;
  var $i298: i32;
  var $i302: i1;
  var $i303: i8;
  var $i304: i8;
  var $i305: i1;
  var $i306: i8;
  var $i307: i1;
  var $i308: i8;
  var $i309: i1;
  var $i310: i8;
  var $i311: i1;
  var $i313: i8;
  var $i314: i1;
  var $i315: i32;
  var $i312: i32;
  var $i316: i1;
  var $i317: i8;
  var $i318: i32;
  var $i319: i32;
$bb0:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 754, 3} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 754, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 755, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, 1);
  call {:cexpr "x"} boogie_si_record_i32(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 756, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 757, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  $i1 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  $i2 := $trunc.i8.i1($i1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i2} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i2 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 25} true;
  assume {:verifier.code 0} true;
  $i3 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 25} true;
  assume {:verifier.code 0} true;
  $i4 := $trunc.i8.i1($i3);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i4} true;
  goto $bb4, $bb5;
$bb2:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i2 == 1));
  goto $bb3;
$bb3:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  $i7 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  $i8 := $trunc.i8.i1($i7);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i8} true;
  goto $bb7, $bb8;
$bb4:
  assume ($i4 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 42} true;
  assume {:verifier.code 0} true;
  $i5 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  $i6 := $i5;
  goto $bb6;
$bb5:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i4 == 1));
  goto $bb3;
$bb6:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i6);
  call {:cexpr "x"} boogie_si_record_i32($i6);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  $i14 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  $i15 := $trunc.i8.i1($i14);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i15} true;
  goto $bb13, $bb14;
$bb7:
  assume ($i8 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 73} true;
  assume {:verifier.code 0} true;
  $i9 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 73} true;
  assume {:verifier.code 0} true;
  $i10 := $trunc.i8.i1($i9);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i10} true;
  goto $bb10, $bb11;
$bb8:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i8 == 1));
  goto $bb9;
$bb9:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 102} true;
  assume {:verifier.code 0} true;
  $i13 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  $i12 := $i13;
  goto $bb12;
$bb10:
  assume ($i10 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 90} true;
  assume {:verifier.code 0} true;
  $i11 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  $i12 := $i11;
  goto $bb12;
$bb11:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i10 == 1));
  goto $bb9;
$bb12:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 758, 7} true;
  assume {:verifier.code 0} true;
  $i6 := $i12;
  goto $bb6;
$bb13:
  assume ($i15 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 38} true;
  assume {:verifier.code 0} true;
  $i16 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 38} true;
  assume {:verifier.code 0} true;
  $i17 := $trunc.i8.i1($i16);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i17} true;
  goto $bb16, $bb17;
$bb14:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i15 == 1));
  goto $bb15;
$bb15:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 66} true;
  assume {:verifier.code 0} true;
  $i19 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 66} true;
  assume {:verifier.code 0} true;
  $i20 := $trunc.i8.i1($i19);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 66} true;
  assume {:verifier.code 0} true;
  $i21 := $zext.i1.i32($i20);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  $i18 := $i21;
  goto $bb18;
$bb16:
  assume ($i17 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  $i18 := 0;
  goto $bb18;
$bb17:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i17 == 1));
  goto $bb15;
$bb18:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 20} true;
  assume {:verifier.code 0} true;
  $i22 := $ne.i32($i18, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 18} true;
  assume {:verifier.code 0} true;
  $i23 := $zext.i1.i8($i22);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 759, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i23);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i23);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  $i24 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  $i25 := $trunc.i8.i1($i24);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i25} true;
  goto $bb19, $bb20;
$bb19:
  assume ($i25 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 38} true;
  assume {:verifier.code 0} true;
  $i26 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 38} true;
  assume {:verifier.code 0} true;
  $i27 := $trunc.i8.i1($i26);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i27} true;
  goto $bb22, $bb24;
$bb20:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i25 == 1));
  goto $bb21;
$bb21:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 56} true;
  assume {:verifier.code 0} true;
  $i28 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 56} true;
  assume {:verifier.code 0} true;
  $i29 := $trunc.i8.i1($i28);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i29} true;
  goto $bb25, $bb26;
$bb22:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 53} true;
  assume {:verifier.code 0} true;
  assume ($i27 == 1);
  goto $bb23;
$bb23:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  $i32 := 0;
  goto $bb30;
$bb24:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i27 == 1));
  goto $bb21;
$bb25:
  assume ($i29 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 74} true;
  assume {:verifier.code 0} true;
  $i30 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 74} true;
  assume {:verifier.code 0} true;
  $i31 := $trunc.i8.i1($i30);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i31} true;
  goto $bb28, $bb29;
$bb26:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i29 == 1));
  goto $bb27;
$bb27:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 102} true;
  assume {:verifier.code 0} true;
  $i33 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 102} true;
  assume {:verifier.code 0} true;
  $i34 := $trunc.i8.i1($i33);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 102} true;
  assume {:verifier.code 0} true;
  $i35 := $zext.i1.i32($i34);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  $i32 := $i35;
  goto $bb30;
$bb28:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  assume ($i31 == 1);
  goto $bb23;
$bb29:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i31 == 1));
  goto $bb27;
$bb30:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 20} true;
  assume {:verifier.code 0} true;
  $i36 := $ne.i32($i32, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 18} true;
  assume {:verifier.code 0} true;
  $i37 := $zext.i1.i8($i36);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 760, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i37);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i37);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  $i38 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  $i39 := $trunc.i8.i1($i38);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i39} true;
  goto $bb31, $bb32;
$bb31:
  assume ($i39 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 38} true;
  assume {:verifier.code 0} true;
  $i40 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 38} true;
  assume {:verifier.code 0} true;
  $i41 := $trunc.i8.i1($i40);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i41} true;
  goto $bb34, $bb35;
$bb32:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i39 == 1));
  goto $bb33;
$bb33:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 66} true;
  assume {:verifier.code 0} true;
  $i43 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 66} true;
  assume {:verifier.code 0} true;
  $i44 := $trunc.i8.i1($i43);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 66} true;
  assume {:verifier.code 0} true;
  $i45 := $zext.i1.i32($i44);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  $i42 := $i45;
  goto $bb36;
$bb34:
  assume ($i41 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  $i42 := 0;
  goto $bb36;
$bb35:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i41 == 1));
  goto $bb33;
$bb36:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 20} true;
  assume {:verifier.code 0} true;
  $i46 := $ne.i32($i42, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 18} true;
  assume {:verifier.code 0} true;
  $i47 := $zext.i1.i8($i46);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 761, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd1, $i47);
  call {:cexpr "x$r_buff0_thd1"} boogie_si_record_i8($i47);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  $i48 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  $i49 := $trunc.i8.i1($i48);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i49} true;
  goto $bb37, $bb38;
$bb37:
  assume ($i49 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 38} true;
  assume {:verifier.code 0} true;
  $i50 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 38} true;
  assume {:verifier.code 0} true;
  $i51 := $trunc.i8.i1($i50);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i51} true;
  goto $bb40, $bb42;
$bb38:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i49 == 1));
  goto $bb39;
$bb39:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 56} true;
  assume {:verifier.code 0} true;
  $i52 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 56} true;
  assume {:verifier.code 0} true;
  $i53 := $trunc.i8.i1($i52);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i53} true;
  goto $bb43, $bb44;
$bb40:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 53} true;
  assume {:verifier.code 0} true;
  assume ($i51 == 1);
  goto $bb41;
$bb41:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  $i56 := 0;
  goto $bb48;
$bb42:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i51 == 1));
  goto $bb39;
$bb43:
  assume ($i53 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 74} true;
  assume {:verifier.code 0} true;
  $i54 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 74} true;
  assume {:verifier.code 0} true;
  $i55 := $trunc.i8.i1($i54);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i55} true;
  goto $bb46, $bb47;
$bb44:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i53 == 1));
  goto $bb45;
$bb45:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 102} true;
  assume {:verifier.code 0} true;
  $i57 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 102} true;
  assume {:verifier.code 0} true;
  $i58 := $trunc.i8.i1($i57);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 102} true;
  assume {:verifier.code 0} true;
  $i59 := $zext.i1.i32($i58);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  $i56 := $i59;
  goto $bb48;
$bb46:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  assume ($i55 == 1);
  goto $bb41;
$bb47:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i55 == 1));
  goto $bb45;
$bb48:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 20} true;
  assume {:verifier.code 0} true;
  $i60 := $ne.i32($i56, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 18} true;
  assume {:verifier.code 0} true;
  $i61 := $zext.i1.i8($i60);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 762, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd1, $i61);
  call {:cexpr "x$r_buff1_thd1"} boogie_si_record_i8($i61);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 763, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 764, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 765, 19} true;
  assume {:verifier.code 1} true;
  call $i62 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i62);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 765, 17} true;
  assume {:verifier.code 0} true;
  $i63 := $zext.i1.i8($i62);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 765, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice0, $i63);
  call {:cexpr "weak$$choice0"} boogie_si_record_i8($i63);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 766, 19} true;
  assume {:verifier.code 1} true;
  call $i64 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i64);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 766, 17} true;
  assume {:verifier.code 0} true;
  $i65 := $zext.i1.i8($i64);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 766, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice2, $i65);
  call {:cexpr "weak$$choice2"} boogie_si_record_i8($i65);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 767, 21} true;
  assume {:verifier.code 0} true;
  $i66 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 767, 21} true;
  assume {:verifier.code 0} true;
  $i67 := $trunc.i8.i1($i66);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 767, 19} true;
  assume {:verifier.code 0} true;
  $i68 := $zext.i1.i8($i67);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 767, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, $i68);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8($i68);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 768, 15} true;
  assume {:verifier.code 0} true;
  $i69 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 768, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$mem_tmp, $i69);
  call {:cexpr "x$mem_tmp"} boogie_si_record_i32($i69);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 8} true;
  assume {:verifier.code 0} true;
  $i70 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 8} true;
  assume {:verifier.code 0} true;
  $i71 := $trunc.i8.i1($i70);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 23} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i71} true;
  goto $bb49, $bb50;
$bb49:
  assume ($i71 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 27} true;
  assume {:verifier.code 0} true;
  $i72 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 27} true;
  assume {:verifier.code 0} true;
  $i73 := $trunc.i8.i1($i72);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 42} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i73} true;
  goto $bb52, $bb54;
$bb50:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 23} true;
  assume {:verifier.code 0} true;
  assume !(($i71 == 1));
  goto $bb51;
$bb51:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 101} true;
  assume {:verifier.code 0} true;
  $i80 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  $i81 := $i80;
  goto $bb62;
$bb52:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 42} true;
  assume {:verifier.code 0} true;
  assume ($i73 == 1);
  goto $bb53;
$bb53:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 65} true;
  assume {:verifier.code 0} true;
  $i76 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 65} true;
  assume {:verifier.code 0} true;
  $i77 := $trunc.i8.i1($i76);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 80} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i77} true;
  goto $bb57, $bb59;
$bb54:
  assume !(($i73 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 46} true;
  assume {:verifier.code 0} true;
  $i74 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 46} true;
  assume {:verifier.code 0} true;
  $i75 := $trunc.i8.i1($i74);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 61} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i75} true;
  goto $bb55, $bb56;
$bb55:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 61} true;
  assume {:verifier.code 0} true;
  assume ($i75 == 1);
  goto $bb53;
$bb56:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 61} true;
  assume {:verifier.code 0} true;
  assume !(($i75 == 1));
  goto $bb51;
$bb57:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 80} true;
  assume {:verifier.code 0} true;
  assume ($i77 == 1);
  goto $bb58;
$bb58:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  $i82 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  $i83 := $trunc.i8.i1($i82);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 121} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i83} true;
  goto $bb63, $bb64;
$bb59:
  assume !(($i77 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 84} true;
  assume {:verifier.code 0} true;
  $i78 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 84} true;
  assume {:verifier.code 0} true;
  $i79 := $trunc.i8.i1($i78);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i79} true;
  goto $bb60, $bb61;
$bb60:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  assume ($i79 == 1);
  goto $bb58;
$bb61:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i79 == 1));
  goto $bb51;
$bb62:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i81);
  call {:cexpr "x"} boogie_si_record_i32($i81);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  $i89 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  $i90 := $trunc.i8.i1($i89);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i90} true;
  goto $bb69, $bb70;
$bb63:
  assume ($i83 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 124} true;
  assume {:verifier.code 0} true;
  $i84 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 124} true;
  assume {:verifier.code 0} true;
  $i85 := $trunc.i8.i1($i84);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i85} true;
  goto $bb66, $bb67;
$bb64:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 121} true;
  assume {:verifier.code 0} true;
  assume !(($i83 == 1));
  goto $bb65;
$bb65:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 153} true;
  assume {:verifier.code 0} true;
  $i88 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  $i87 := $i88;
  goto $bb68;
$bb66:
  assume ($i85 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 141} true;
  assume {:verifier.code 0} true;
  $i86 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  $i87 := $i86;
  goto $bb68;
$bb67:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  assume !(($i85 == 1));
  goto $bb65;
$bb68:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 106} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 769, 7} true;
  assume {:verifier.code 0} true;
  $i81 := $i87;
  goto $bb62;
$bb69:
  assume ($i90 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 31} true;
  assume {:verifier.code 0} true;
  $i91 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  $i92 := $i91;
  goto $bb71;
$bb70:
  assume !(($i90 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 45} true;
  assume {:verifier.code 0} true;
  $i93 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 45} true;
  assume {:verifier.code 0} true;
  $i94 := $trunc.i8.i1($i93);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i94} true;
  goto $bb72, $bb73;
$bb71:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff0, $i92);
  call {:cexpr "x$w_buff0"} boogie_si_record_i32($i92);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  $i112 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  $i113 := $trunc.i8.i1($i112);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i113} true;
  goto $bb92, $bb93;
$bb72:
  assume ($i94 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 64} true;
  assume {:verifier.code 0} true;
  $i95 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 64} true;
  assume {:verifier.code 0} true;
  $i96 := $trunc.i8.i1($i95);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i96} true;
  goto $bb75, $bb77;
$bb73:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i94 == 1));
  goto $bb74;
$bb74:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 138} true;
  assume {:verifier.code 0} true;
  $i103 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  $i104 := $i103;
  goto $bb85;
$bb75:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 79} true;
  assume {:verifier.code 0} true;
  assume ($i96 == 1);
  goto $bb76;
$bb76:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 102} true;
  assume {:verifier.code 0} true;
  $i99 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 102} true;
  assume {:verifier.code 0} true;
  $i100 := $trunc.i8.i1($i99);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i100} true;
  goto $bb80, $bb82;
$bb77:
  assume !(($i96 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 83} true;
  assume {:verifier.code 0} true;
  $i97 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 83} true;
  assume {:verifier.code 0} true;
  $i98 := $trunc.i8.i1($i97);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i98} true;
  goto $bb78, $bb79;
$bb78:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 98} true;
  assume {:verifier.code 0} true;
  assume ($i98 == 1);
  goto $bb76;
$bb79:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i98 == 1));
  goto $bb74;
$bb80:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 117} true;
  assume {:verifier.code 0} true;
  assume ($i100 == 1);
  goto $bb81;
$bb81:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  $i105 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  $i106 := $trunc.i8.i1($i105);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i106} true;
  goto $bb86, $bb87;
$bb82:
  assume !(($i100 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 121} true;
  assume {:verifier.code 0} true;
  $i101 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 121} true;
  assume {:verifier.code 0} true;
  $i102 := $trunc.i8.i1($i101);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i102} true;
  goto $bb83, $bb84;
$bb83:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  assume ($i102 == 1);
  goto $bb81;
$bb84:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i102 == 1));
  goto $bb74;
$bb85:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 15} true;
  assume {:verifier.code 0} true;
  $i92 := $i104;
  goto $bb71;
$bb86:
  assume ($i106 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 169} true;
  assume {:verifier.code 0} true;
  $i107 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 169} true;
  assume {:verifier.code 0} true;
  $i108 := $trunc.i8.i1($i107);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i108} true;
  goto $bb89, $bb90;
$bb87:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i106 == 1));
  goto $bb88;
$bb88:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 198} true;
  assume {:verifier.code 0} true;
  $i111 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  $i110 := $i111;
  goto $bb91;
$bb89:
  assume ($i108 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 186} true;
  assume {:verifier.code 0} true;
  $i109 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  $i110 := $i109;
  goto $bb91;
$bb90:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i108 == 1));
  goto $bb88;
$bb91:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 770, 44} true;
  assume {:verifier.code 0} true;
  $i104 := $i110;
  goto $bb85;
$bb92:
  assume ($i113 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 31} true;
  assume {:verifier.code 0} true;
  $i114 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  $i115 := $i114;
  goto $bb94;
$bb93:
  assume !(($i113 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 45} true;
  assume {:verifier.code 0} true;
  $i116 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 45} true;
  assume {:verifier.code 0} true;
  $i117 := $trunc.i8.i1($i116);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i117} true;
  goto $bb95, $bb96;
$bb94:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff1, $i115);
  call {:cexpr "x$w_buff1"} boogie_si_record_i32($i115);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  $i135 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  $i136 := $trunc.i8.i1($i135);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i136} true;
  goto $bb115, $bb116;
$bb95:
  assume ($i117 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 64} true;
  assume {:verifier.code 0} true;
  $i118 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 64} true;
  assume {:verifier.code 0} true;
  $i119 := $trunc.i8.i1($i118);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i119} true;
  goto $bb98, $bb100;
$bb96:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i117 == 1));
  goto $bb97;
$bb97:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 138} true;
  assume {:verifier.code 0} true;
  $i126 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  $i127 := $i126;
  goto $bb108;
$bb98:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 79} true;
  assume {:verifier.code 0} true;
  assume ($i119 == 1);
  goto $bb99;
$bb99:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 102} true;
  assume {:verifier.code 0} true;
  $i122 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 102} true;
  assume {:verifier.code 0} true;
  $i123 := $trunc.i8.i1($i122);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i123} true;
  goto $bb103, $bb105;
$bb100:
  assume !(($i119 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 83} true;
  assume {:verifier.code 0} true;
  $i120 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 83} true;
  assume {:verifier.code 0} true;
  $i121 := $trunc.i8.i1($i120);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i121} true;
  goto $bb101, $bb102;
$bb101:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 98} true;
  assume {:verifier.code 0} true;
  assume ($i121 == 1);
  goto $bb99;
$bb102:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i121 == 1));
  goto $bb97;
$bb103:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 117} true;
  assume {:verifier.code 0} true;
  assume ($i123 == 1);
  goto $bb104;
$bb104:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  $i128 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  $i129 := $trunc.i8.i1($i128);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i129} true;
  goto $bb109, $bb110;
$bb105:
  assume !(($i123 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 121} true;
  assume {:verifier.code 0} true;
  $i124 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 121} true;
  assume {:verifier.code 0} true;
  $i125 := $trunc.i8.i1($i124);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i125} true;
  goto $bb106, $bb107;
$bb106:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  assume ($i125 == 1);
  goto $bb104;
$bb107:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i125 == 1));
  goto $bb97;
$bb108:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 15} true;
  assume {:verifier.code 0} true;
  $i115 := $i127;
  goto $bb94;
$bb109:
  assume ($i129 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 169} true;
  assume {:verifier.code 0} true;
  $i130 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 169} true;
  assume {:verifier.code 0} true;
  $i131 := $trunc.i8.i1($i130);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i131} true;
  goto $bb112, $bb113;
$bb110:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i129 == 1));
  goto $bb111;
$bb111:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 198} true;
  assume {:verifier.code 0} true;
  $i134 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  $i133 := $i134;
  goto $bb114;
$bb112:
  assume ($i131 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 186} true;
  assume {:verifier.code 0} true;
  $i132 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  $i133 := $i132;
  goto $bb114;
$bb113:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i131 == 1));
  goto $bb111;
$bb114:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 771, 44} true;
  assume {:verifier.code 0} true;
  $i127 := $i133;
  goto $bb108;
$bb115:
  assume ($i136 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 36} true;
  assume {:verifier.code 0} true;
  $i137 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 36} true;
  assume {:verifier.code 0} true;
  $i138 := $trunc.i8.i1($i137);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 36} true;
  assume {:verifier.code 0} true;
  $i139 := $zext.i1.i32($i138);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  $i140 := $i139;
  goto $bb117;
$bb116:
  assume !(($i136 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 55} true;
  assume {:verifier.code 0} true;
  $i141 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 55} true;
  assume {:verifier.code 0} true;
  $i142 := $trunc.i8.i1($i141);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i142} true;
  goto $bb118, $bb119;
$bb117:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  $i163 := $ne.i32($i140, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 18} true;
  assume {:verifier.code 0} true;
  $i164 := $zext.i1.i8($i163);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i164);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i164);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  $i165 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  $i166 := $trunc.i8.i1($i165);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i166} true;
  goto $bb138, $bb139;
$bb118:
  assume ($i142 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 74} true;
  assume {:verifier.code 0} true;
  $i143 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 74} true;
  assume {:verifier.code 0} true;
  $i144 := $trunc.i8.i1($i143);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i144} true;
  goto $bb121, $bb123;
$bb119:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i142 == 1));
  goto $bb120;
$bb120:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 148} true;
  assume {:verifier.code 0} true;
  $i151 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 148} true;
  assume {:verifier.code 0} true;
  $i152 := $trunc.i8.i1($i151);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 148} true;
  assume {:verifier.code 0} true;
  $i153 := $zext.i1.i32($i152);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  $i154 := $i153;
  goto $bb131;
$bb121:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 89} true;
  assume {:verifier.code 0} true;
  assume ($i144 == 1);
  goto $bb122;
$bb122:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 112} true;
  assume {:verifier.code 0} true;
  $i147 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 112} true;
  assume {:verifier.code 0} true;
  $i148 := $trunc.i8.i1($i147);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i148} true;
  goto $bb126, $bb128;
$bb123:
  assume !(($i144 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 93} true;
  assume {:verifier.code 0} true;
  $i145 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 93} true;
  assume {:verifier.code 0} true;
  $i146 := $trunc.i8.i1($i145);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i146} true;
  goto $bb124, $bb125;
$bb124:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 108} true;
  assume {:verifier.code 0} true;
  assume ($i146 == 1);
  goto $bb122;
$bb125:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i146 == 1));
  goto $bb120;
$bb126:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 127} true;
  assume {:verifier.code 0} true;
  assume ($i148 == 1);
  goto $bb127;
$bb127:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  $i155 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  $i156 := $trunc.i8.i1($i155);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i156} true;
  goto $bb132, $bb133;
$bb128:
  assume !(($i148 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 131} true;
  assume {:verifier.code 0} true;
  $i149 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 131} true;
  assume {:verifier.code 0} true;
  $i150 := $trunc.i8.i1($i149);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i150} true;
  goto $bb129, $bb130;
$bb129:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  assume ($i150 == 1);
  goto $bb127;
$bb130:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i150 == 1));
  goto $bb120;
$bb131:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 20} true;
  assume {:verifier.code 0} true;
  $i140 := $i154;
  goto $bb117;
$bb132:
  assume ($i156 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 184} true;
  assume {:verifier.code 0} true;
  $i157 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 184} true;
  assume {:verifier.code 0} true;
  $i158 := $trunc.i8.i1($i157);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i158} true;
  goto $bb135, $bb136;
$bb133:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i156 == 1));
  goto $bb134;
$bb134:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 212} true;
  assume {:verifier.code 0} true;
  $i160 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 212} true;
  assume {:verifier.code 0} true;
  $i161 := $trunc.i8.i1($i160);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 212} true;
  assume {:verifier.code 0} true;
  $i162 := $zext.i1.i32($i161);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  $i159 := $i162;
  goto $bb137;
$bb135:
  assume ($i158 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  $i159 := 0;
  goto $bb137;
$bb136:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i158 == 1));
  goto $bb134;
$bb137:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 772, 54} true;
  assume {:verifier.code 0} true;
  $i154 := $i159;
  goto $bb131;
$bb138:
  assume ($i166 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 36} true;
  assume {:verifier.code 0} true;
  $i167 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 36} true;
  assume {:verifier.code 0} true;
  $i168 := $trunc.i8.i1($i167);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 36} true;
  assume {:verifier.code 0} true;
  $i169 := $zext.i1.i32($i168);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  $i170 := $i169;
  goto $bb140;
$bb139:
  assume !(($i166 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 55} true;
  assume {:verifier.code 0} true;
  $i171 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 55} true;
  assume {:verifier.code 0} true;
  $i172 := $trunc.i8.i1($i171);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i172} true;
  goto $bb141, $bb142;
$bb140:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  $i191 := $ne.i32($i170, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 18} true;
  assume {:verifier.code 0} true;
  $i192 := $zext.i1.i8($i191);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i192);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i192);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  $i193 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  $i194 := $trunc.i8.i1($i193);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i194} true;
  goto $bb158, $bb159;
$bb141:
  assume ($i172 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 74} true;
  assume {:verifier.code 0} true;
  $i173 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 74} true;
  assume {:verifier.code 0} true;
  $i174 := $trunc.i8.i1($i173);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i174} true;
  goto $bb144, $bb146;
$bb142:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i172 == 1));
  goto $bb143;
$bb143:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 148} true;
  assume {:verifier.code 0} true;
  $i181 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 148} true;
  assume {:verifier.code 0} true;
  $i182 := $trunc.i8.i1($i181);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 148} true;
  assume {:verifier.code 0} true;
  $i183 := $zext.i1.i32($i182);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  $i184 := $i183;
  goto $bb154;
$bb144:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 89} true;
  assume {:verifier.code 0} true;
  assume ($i174 == 1);
  goto $bb145;
$bb145:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 112} true;
  assume {:verifier.code 0} true;
  $i177 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 112} true;
  assume {:verifier.code 0} true;
  $i178 := $trunc.i8.i1($i177);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i178} true;
  goto $bb149, $bb151;
$bb146:
  assume !(($i174 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 93} true;
  assume {:verifier.code 0} true;
  $i175 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 93} true;
  assume {:verifier.code 0} true;
  $i176 := $trunc.i8.i1($i175);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i176} true;
  goto $bb147, $bb148;
$bb147:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 108} true;
  assume {:verifier.code 0} true;
  assume ($i176 == 1);
  goto $bb145;
$bb148:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i176 == 1));
  goto $bb143;
$bb149:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 127} true;
  assume {:verifier.code 0} true;
  assume ($i178 == 1);
  goto $bb150;
$bb150:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 166} true;
  assume {:verifier.code 0} true;
  $i185 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 166} true;
  assume {:verifier.code 0} true;
  $i186 := $trunc.i8.i1($i185);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 181} true;
  assume {:verifier.code 0} true;
  $i187 := 0;
  assume {:branchcond $i186} true;
  goto $bb155, $bb156;
$bb151:
  assume !(($i178 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 131} true;
  assume {:verifier.code 0} true;
  $i179 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 131} true;
  assume {:verifier.code 0} true;
  $i180 := $trunc.i8.i1($i179);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i180} true;
  goto $bb152, $bb153;
$bb152:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  assume ($i180 == 1);
  goto $bb150;
$bb153:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i180 == 1));
  goto $bb143;
$bb154:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 20} true;
  assume {:verifier.code 0} true;
  $i170 := $i184;
  goto $bb140;
$bb155:
  assume ($i186 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 184} true;
  assume {:verifier.code 0} true;
  $i188 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 184} true;
  assume {:verifier.code 0} true;
  $i189 := $trunc.i8.i1($i188);
  assume {:verifier.code 0} true;
  $i187 := $i189;
  goto $bb157;
$bb156:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i186 == 1));
  goto $bb157;
$bb157:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 166} true;
  assume {:verifier.code 0} true;
  $i190 := (if ($i187 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 773, 54} true;
  assume {:verifier.code 0} true;
  $i184 := $i190;
  goto $bb154;
$bb158:
  assume ($i194 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 36} true;
  assume {:verifier.code 0} true;
  $i195 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 36} true;
  assume {:verifier.code 0} true;
  $i196 := $trunc.i8.i1($i195);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 36} true;
  assume {:verifier.code 0} true;
  $i197 := $zext.i1.i32($i196);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  $i198 := $i197;
  goto $bb160;
$bb159:
  assume !(($i194 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 55} true;
  assume {:verifier.code 0} true;
  $i199 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 55} true;
  assume {:verifier.code 0} true;
  $i200 := $trunc.i8.i1($i199);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i200} true;
  goto $bb161, $bb162;
$bb160:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  $i221 := $ne.i32($i198, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 18} true;
  assume {:verifier.code 0} true;
  $i222 := $zext.i1.i8($i221);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd1, $i222);
  call {:cexpr "x$r_buff0_thd1"} boogie_si_record_i8($i222);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  $i223 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  $i224 := $trunc.i8.i1($i223);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i224} true;
  goto $bb181, $bb182;
$bb161:
  assume ($i200 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 74} true;
  assume {:verifier.code 0} true;
  $i201 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 74} true;
  assume {:verifier.code 0} true;
  $i202 := $trunc.i8.i1($i201);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i202} true;
  goto $bb164, $bb166;
$bb162:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i200 == 1));
  goto $bb163;
$bb163:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 148} true;
  assume {:verifier.code 0} true;
  $i209 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 148} true;
  assume {:verifier.code 0} true;
  $i210 := $trunc.i8.i1($i209);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 148} true;
  assume {:verifier.code 0} true;
  $i211 := $zext.i1.i32($i210);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  $i212 := $i211;
  goto $bb174;
$bb164:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 89} true;
  assume {:verifier.code 0} true;
  assume ($i202 == 1);
  goto $bb165;
$bb165:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 112} true;
  assume {:verifier.code 0} true;
  $i205 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 112} true;
  assume {:verifier.code 0} true;
  $i206 := $trunc.i8.i1($i205);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i206} true;
  goto $bb169, $bb171;
$bb166:
  assume !(($i202 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 93} true;
  assume {:verifier.code 0} true;
  $i203 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 93} true;
  assume {:verifier.code 0} true;
  $i204 := $trunc.i8.i1($i203);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i204} true;
  goto $bb167, $bb168;
$bb167:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 108} true;
  assume {:verifier.code 0} true;
  assume ($i204 == 1);
  goto $bb165;
$bb168:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i204 == 1));
  goto $bb163;
$bb169:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 127} true;
  assume {:verifier.code 0} true;
  assume ($i206 == 1);
  goto $bb170;
$bb170:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  $i213 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  $i214 := $trunc.i8.i1($i213);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i214} true;
  goto $bb175, $bb176;
$bb171:
  assume !(($i206 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 131} true;
  assume {:verifier.code 0} true;
  $i207 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 131} true;
  assume {:verifier.code 0} true;
  $i208 := $trunc.i8.i1($i207);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i208} true;
  goto $bb172, $bb173;
$bb172:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  assume ($i208 == 1);
  goto $bb170;
$bb173:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i208 == 1));
  goto $bb163;
$bb174:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 20} true;
  assume {:verifier.code 0} true;
  $i198 := $i212;
  goto $bb160;
$bb175:
  assume ($i214 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 184} true;
  assume {:verifier.code 0} true;
  $i215 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 184} true;
  assume {:verifier.code 0} true;
  $i216 := $trunc.i8.i1($i215);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i216} true;
  goto $bb178, $bb179;
$bb176:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i214 == 1));
  goto $bb177;
$bb177:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 212} true;
  assume {:verifier.code 0} true;
  $i218 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 212} true;
  assume {:verifier.code 0} true;
  $i219 := $trunc.i8.i1($i218);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 212} true;
  assume {:verifier.code 0} true;
  $i220 := $zext.i1.i32($i219);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  $i217 := $i220;
  goto $bb180;
$bb178:
  assume ($i216 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  $i217 := 0;
  goto $bb180;
$bb179:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i216 == 1));
  goto $bb177;
$bb180:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 774, 54} true;
  assume {:verifier.code 0} true;
  $i212 := $i217;
  goto $bb174;
$bb181:
  assume ($i224 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 36} true;
  assume {:verifier.code 0} true;
  $i225 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 36} true;
  assume {:verifier.code 0} true;
  $i226 := $trunc.i8.i1($i225);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 36} true;
  assume {:verifier.code 0} true;
  $i227 := $zext.i1.i32($i226);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  $i228 := $i227;
  goto $bb183;
$bb182:
  assume !(($i224 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 55} true;
  assume {:verifier.code 0} true;
  $i229 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 55} true;
  assume {:verifier.code 0} true;
  $i230 := $trunc.i8.i1($i229);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i230} true;
  goto $bb184, $bb185;
$bb183:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  $i249 := $ne.i32($i228, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 18} true;
  assume {:verifier.code 0} true;
  $i250 := $zext.i1.i8($i249);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd1, $i250);
  call {:cexpr "x$r_buff1_thd1"} boogie_si_record_i8($i250);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 776, 25} true;
  assume {:verifier.code 0} true;
  $i251 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 776, 23} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, __unbuffered_p0_EAX, $i251);
  call {:cexpr "__unbuffered_p0_EAX"} boogie_si_record_i32($i251);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  $i252 := $load.i8($M.0, x$flush_delayed);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  $i253 := $trunc.i8.i1($i252);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i253} true;
  goto $bb201, $bb202;
$bb184:
  assume ($i230 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 74} true;
  assume {:verifier.code 0} true;
  $i231 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 74} true;
  assume {:verifier.code 0} true;
  $i232 := $trunc.i8.i1($i231);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i232} true;
  goto $bb187, $bb189;
$bb185:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i230 == 1));
  goto $bb186;
$bb186:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 148} true;
  assume {:verifier.code 0} true;
  $i239 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 148} true;
  assume {:verifier.code 0} true;
  $i240 := $trunc.i8.i1($i239);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 148} true;
  assume {:verifier.code 0} true;
  $i241 := $zext.i1.i32($i240);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  $i242 := $i241;
  goto $bb197;
$bb187:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 89} true;
  assume {:verifier.code 0} true;
  assume ($i232 == 1);
  goto $bb188;
$bb188:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 112} true;
  assume {:verifier.code 0} true;
  $i235 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 112} true;
  assume {:verifier.code 0} true;
  $i236 := $trunc.i8.i1($i235);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i236} true;
  goto $bb192, $bb194;
$bb189:
  assume !(($i232 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 93} true;
  assume {:verifier.code 0} true;
  $i233 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 93} true;
  assume {:verifier.code 0} true;
  $i234 := $trunc.i8.i1($i233);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i234} true;
  goto $bb190, $bb191;
$bb190:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 108} true;
  assume {:verifier.code 0} true;
  assume ($i234 == 1);
  goto $bb188;
$bb191:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i234 == 1));
  goto $bb186;
$bb192:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 127} true;
  assume {:verifier.code 0} true;
  assume ($i236 == 1);
  goto $bb193;
$bb193:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 166} true;
  assume {:verifier.code 0} true;
  $i243 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 166} true;
  assume {:verifier.code 0} true;
  $i244 := $trunc.i8.i1($i243);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 181} true;
  assume {:verifier.code 0} true;
  $i245 := 0;
  assume {:branchcond $i244} true;
  goto $bb198, $bb199;
$bb194:
  assume !(($i236 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 131} true;
  assume {:verifier.code 0} true;
  $i237 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 131} true;
  assume {:verifier.code 0} true;
  $i238 := $trunc.i8.i1($i237);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i238} true;
  goto $bb195, $bb196;
$bb195:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  assume ($i238 == 1);
  goto $bb193;
$bb196:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i238 == 1));
  goto $bb186;
$bb197:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 20} true;
  assume {:verifier.code 0} true;
  $i228 := $i242;
  goto $bb183;
$bb198:
  assume ($i244 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 184} true;
  assume {:verifier.code 0} true;
  $i246 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 184} true;
  assume {:verifier.code 0} true;
  $i247 := $trunc.i8.i1($i246);
  assume {:verifier.code 0} true;
  $i245 := $i247;
  goto $bb200;
$bb199:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i244 == 1));
  goto $bb200;
$bb200:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 166} true;
  assume {:verifier.code 0} true;
  $i248 := (if ($i245 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 775, 54} true;
  assume {:verifier.code 0} true;
  $i242 := $i248;
  goto $bb197;
$bb201:
  assume ($i253 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 25} true;
  assume {:verifier.code 0} true;
  $i254 := $load.i32($M.0, x$mem_tmp);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  $i255 := $i254;
  goto $bb203;
$bb202:
  assume !(($i253 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 37} true;
  assume {:verifier.code 0} true;
  $i256 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  $i255 := $i256;
  goto $bb203;
$bb203:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 777, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i255);
  call {:cexpr "x"} boogie_si_record_i32($i255);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 778, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, 0);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8(0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 779, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 780, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  $i257 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  $i258 := $trunc.i8.i1($i257);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i258} true;
  goto $bb204, $bb205;
$bb204:
  assume ($i258 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 25} true;
  assume {:verifier.code 0} true;
  $i259 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 25} true;
  assume {:verifier.code 0} true;
  $i260 := $trunc.i8.i1($i259);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i260} true;
  goto $bb207, $bb208;
$bb205:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i258 == 1));
  goto $bb206;
$bb206:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  $i263 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  $i264 := $trunc.i8.i1($i263);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i264} true;
  goto $bb210, $bb211;
$bb207:
  assume ($i260 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 42} true;
  assume {:verifier.code 0} true;
  $i261 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  $i262 := $i261;
  goto $bb209;
$bb208:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i260 == 1));
  goto $bb206;
$bb209:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i262);
  call {:cexpr "x"} boogie_si_record_i32($i262);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  $i270 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  $i271 := $trunc.i8.i1($i270);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i271} true;
  goto $bb216, $bb217;
$bb210:
  assume ($i264 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 73} true;
  assume {:verifier.code 0} true;
  $i265 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 73} true;
  assume {:verifier.code 0} true;
  $i266 := $trunc.i8.i1($i265);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i266} true;
  goto $bb213, $bb214;
$bb211:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i264 == 1));
  goto $bb212;
$bb212:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 102} true;
  assume {:verifier.code 0} true;
  $i269 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  $i268 := $i269;
  goto $bb215;
$bb213:
  assume ($i266 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 90} true;
  assume {:verifier.code 0} true;
  $i267 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  $i268 := $i267;
  goto $bb215;
$bb214:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i266 == 1));
  goto $bb212;
$bb215:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 781, 7} true;
  assume {:verifier.code 0} true;
  $i262 := $i268;
  goto $bb209;
$bb216:
  assume ($i271 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 38} true;
  assume {:verifier.code 0} true;
  $i272 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 38} true;
  assume {:verifier.code 0} true;
  $i273 := $trunc.i8.i1($i272);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i273} true;
  goto $bb219, $bb220;
$bb217:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i271 == 1));
  goto $bb218;
$bb218:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 66} true;
  assume {:verifier.code 0} true;
  $i275 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 66} true;
  assume {:verifier.code 0} true;
  $i276 := $trunc.i8.i1($i275);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 66} true;
  assume {:verifier.code 0} true;
  $i277 := $zext.i1.i32($i276);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  $i274 := $i277;
  goto $bb221;
$bb219:
  assume ($i273 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  $i274 := 0;
  goto $bb221;
$bb220:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i273 == 1));
  goto $bb218;
$bb221:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 20} true;
  assume {:verifier.code 0} true;
  $i278 := $ne.i32($i274, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 18} true;
  assume {:verifier.code 0} true;
  $i279 := $zext.i1.i8($i278);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 782, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i279);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i279);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  $i280 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  $i281 := $trunc.i8.i1($i280);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i281} true;
  goto $bb222, $bb223;
$bb222:
  assume ($i281 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 38} true;
  assume {:verifier.code 0} true;
  $i282 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 38} true;
  assume {:verifier.code 0} true;
  $i283 := $trunc.i8.i1($i282);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i283} true;
  goto $bb225, $bb227;
$bb223:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i281 == 1));
  goto $bb224;
$bb224:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 56} true;
  assume {:verifier.code 0} true;
  $i284 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 56} true;
  assume {:verifier.code 0} true;
  $i285 := $trunc.i8.i1($i284);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i285} true;
  goto $bb228, $bb229;
$bb225:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 53} true;
  assume {:verifier.code 0} true;
  assume ($i283 == 1);
  goto $bb226;
$bb226:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  $i288 := 0;
  goto $bb233;
$bb227:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i283 == 1));
  goto $bb224;
$bb228:
  assume ($i285 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 74} true;
  assume {:verifier.code 0} true;
  $i286 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 74} true;
  assume {:verifier.code 0} true;
  $i287 := $trunc.i8.i1($i286);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i287} true;
  goto $bb231, $bb232;
$bb229:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i285 == 1));
  goto $bb230;
$bb230:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 102} true;
  assume {:verifier.code 0} true;
  $i289 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 102} true;
  assume {:verifier.code 0} true;
  $i290 := $trunc.i8.i1($i289);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 102} true;
  assume {:verifier.code 0} true;
  $i291 := $zext.i1.i32($i290);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  $i288 := $i291;
  goto $bb233;
$bb231:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  assume ($i287 == 1);
  goto $bb226;
$bb232:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i287 == 1));
  goto $bb230;
$bb233:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 20} true;
  assume {:verifier.code 0} true;
  $i292 := $ne.i32($i288, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 18} true;
  assume {:verifier.code 0} true;
  $i293 := $zext.i1.i8($i292);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 783, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i293);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i293);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  $i294 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  $i295 := $trunc.i8.i1($i294);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i295} true;
  goto $bb234, $bb235;
$bb234:
  assume ($i295 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 38} true;
  assume {:verifier.code 0} true;
  $i296 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 38} true;
  assume {:verifier.code 0} true;
  $i297 := $trunc.i8.i1($i296);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i297} true;
  goto $bb237, $bb238;
$bb235:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i295 == 1));
  goto $bb236;
$bb236:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 66} true;
  assume {:verifier.code 0} true;
  $i299 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 66} true;
  assume {:verifier.code 0} true;
  $i300 := $trunc.i8.i1($i299);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 66} true;
  assume {:verifier.code 0} true;
  $i301 := $zext.i1.i32($i300);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  $i298 := $i301;
  goto $bb239;
$bb237:
  assume ($i297 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  $i298 := 0;
  goto $bb239;
$bb238:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i297 == 1));
  goto $bb236;
$bb239:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 20} true;
  assume {:verifier.code 0} true;
  $i302 := $ne.i32($i298, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 18} true;
  assume {:verifier.code 0} true;
  $i303 := $zext.i1.i8($i302);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 784, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd1, $i303);
  call {:cexpr "x$r_buff0_thd1"} boogie_si_record_i8($i303);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  $i304 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  $i305 := $trunc.i8.i1($i304);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i305} true;
  goto $bb240, $bb241;
$bb240:
  assume ($i305 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 38} true;
  assume {:verifier.code 0} true;
  $i306 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 38} true;
  assume {:verifier.code 0} true;
  $i307 := $trunc.i8.i1($i306);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i307} true;
  goto $bb243, $bb245;
$bb241:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i305 == 1));
  goto $bb242;
$bb242:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 56} true;
  assume {:verifier.code 0} true;
  $i308 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 56} true;
  assume {:verifier.code 0} true;
  $i309 := $trunc.i8.i1($i308);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i309} true;
  goto $bb246, $bb247;
$bb243:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 53} true;
  assume {:verifier.code 0} true;
  assume ($i307 == 1);
  goto $bb244;
$bb244:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  $i312 := 0;
  goto $bb251;
$bb245:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i307 == 1));
  goto $bb242;
$bb246:
  assume ($i309 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 74} true;
  assume {:verifier.code 0} true;
  $i310 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 74} true;
  assume {:verifier.code 0} true;
  $i311 := $trunc.i8.i1($i310);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i311} true;
  goto $bb249, $bb250;
$bb247:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i309 == 1));
  goto $bb248;
$bb248:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 102} true;
  assume {:verifier.code 0} true;
  $i313 := $load.i8($M.0, x$r_buff1_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 102} true;
  assume {:verifier.code 0} true;
  $i314 := $trunc.i8.i1($i313);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 102} true;
  assume {:verifier.code 0} true;
  $i315 := $zext.i1.i32($i314);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  $i312 := $i315;
  goto $bb251;
$bb249:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  assume ($i311 == 1);
  goto $bb244;
$bb250:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i311 == 1));
  goto $bb248;
$bb251:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 20} true;
  assume {:verifier.code 0} true;
  $i316 := $ne.i32($i312, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 18} true;
  assume {:verifier.code 0} true;
  $i317 := $zext.i1.i8($i316);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 785, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd1, $i317);
  call {:cexpr "x$r_buff1_thd1"} boogie_si_record_i8($i317);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 786, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 787, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 788, 22} true;
  assume {:verifier.code 0} true;
  $i318 := $load.i32($M.0, __unbuffered_cnt);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 788, 39} true;
  assume {:verifier.code 0} true;
  $i319 := $add.i32($i318, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 788, 20} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, __unbuffered_cnt, $i319);
  call {:cexpr "__unbuffered_cnt"} boogie_si_record_i32($i319);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 789, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 790, 3} true;
  assume {:verifier.code 0} true;
  $r := $0.ref;
  $exn := false;
  return;
}
const P1: ref;
axiom (P1 == $sub.ref(0, 53453));
procedure  P1($p0: ref)
  returns ($r: ref)
{
  var $i1: i32;
  var $i2: i8;
  var $i3: i1;
  var $i4: i8;
  var $i5: i8;
  var $i6: i1;
  var $i8: i8;
  var $i9: i1;
  var $i7: i1;
  var $i10: i1;
  var $i11: i32;
  var $i12: i8;
  var $i13: i1;
  var $i14: i8;
  var $i15: i8;
  var $i16: i1;
  var $i17: i8;
  var $i18: i8;
  var $i19: i1;
  var $i20: i8;
  var $i21: i32;
  var $i22: i8;
  var $i23: i1;
  var $i24: i8;
  var $i25: i8;
  var $i26: i1;
  var $i28: i8;
  var $i29: i1;
  var $i27: i1;
  var $i30: i1;
  var $i31: i32;
  var $i32: i8;
  var $i33: i1;
  var $i34: i8;
  var $i35: i8;
  var $i36: i1;
  var $i37: i8;
  var $i38: i8;
  var $i39: i1;
  var $i40: i8;
  var $i41: i1;
  var $i42: i8;
  var $i43: i1;
  var $i44: i8;
  var $i45: i8;
  var $i46: i1;
  var $i47: i8;
  var $i48: i32;
  var $i49: i8;
  var $i50: i1;
  var $i51: i8;
  var $i52: i1;
  var $i53: i8;
  var $i54: i1;
  var $i55: i8;
  var $i56: i1;
  var $i57: i8;
  var $i58: i1;
  var $i59: i32;
  var $i61: i8;
  var $i62: i1;
  var $i63: i8;
  var $i64: i1;
  var $i65: i32;
  var $i67: i32;
  var $i66: i32;
  var $i60: i32;
  var $i68: i8;
  var $i69: i1;
  var $i70: i32;
  var $i72: i8;
  var $i73: i1;
  var $i74: i8;
  var $i75: i1;
  var $i76: i8;
  var $i77: i1;
  var $i78: i8;
  var $i79: i1;
  var $i80: i8;
  var $i81: i1;
  var $i82: i32;
  var $i84: i8;
  var $i85: i1;
  var $i86: i8;
  var $i87: i1;
  var $i88: i32;
  var $i90: i32;
  var $i89: i32;
  var $i83: i32;
  var $i71: i32;
  var $i91: i8;
  var $i92: i1;
  var $i93: i32;
  var $i95: i8;
  var $i96: i1;
  var $i97: i8;
  var $i98: i1;
  var $i99: i8;
  var $i100: i1;
  var $i101: i8;
  var $i102: i1;
  var $i103: i8;
  var $i104: i1;
  var $i105: i32;
  var $i107: i8;
  var $i108: i1;
  var $i109: i8;
  var $i110: i1;
  var $i111: i32;
  var $i113: i32;
  var $i112: i32;
  var $i106: i32;
  var $i94: i32;
  var $i114: i8;
  var $i115: i1;
  var $i116: i8;
  var $i117: i1;
  var $i118: i32;
  var $i120: i8;
  var $i121: i1;
  var $i122: i8;
  var $i123: i1;
  var $i124: i8;
  var $i125: i1;
  var $i126: i8;
  var $i127: i1;
  var $i128: i8;
  var $i129: i1;
  var $i130: i8;
  var $i131: i1;
  var $i132: i32;
  var $i134: i8;
  var $i135: i1;
  var $i136: i8;
  var $i137: i1;
  var $i139: i8;
  var $i140: i1;
  var $i141: i32;
  var $i138: i32;
  var $i133: i32;
  var $i119: i32;
  var $i142: i1;
  var $i143: i8;
  var $i144: i8;
  var $i145: i1;
  var $i146: i8;
  var $i147: i1;
  var $i148: i32;
  var $i150: i8;
  var $i151: i1;
  var $i152: i8;
  var $i153: i1;
  var $i154: i8;
  var $i155: i1;
  var $i156: i8;
  var $i157: i1;
  var $i158: i8;
  var $i159: i1;
  var $i160: i8;
  var $i161: i1;
  var $i162: i32;
  var $i164: i8;
  var $i165: i1;
  var $i167: i8;
  var $i168: i1;
  var $i166: i1;
  var $i169: i32;
  var $i163: i32;
  var $i149: i32;
  var $i170: i1;
  var $i171: i8;
  var $i172: i8;
  var $i173: i1;
  var $i174: i8;
  var $i175: i1;
  var $i176: i32;
  var $i178: i8;
  var $i179: i1;
  var $i180: i8;
  var $i181: i1;
  var $i182: i8;
  var $i183: i1;
  var $i184: i8;
  var $i185: i1;
  var $i186: i8;
  var $i187: i1;
  var $i188: i8;
  var $i189: i1;
  var $i190: i32;
  var $i192: i8;
  var $i193: i1;
  var $i194: i8;
  var $i195: i1;
  var $i197: i8;
  var $i198: i1;
  var $i199: i32;
  var $i196: i32;
  var $i191: i32;
  var $i177: i32;
  var $i200: i1;
  var $i201: i8;
  var $i202: i8;
  var $i203: i1;
  var $i204: i8;
  var $i205: i1;
  var $i206: i32;
  var $i208: i8;
  var $i209: i1;
  var $i210: i8;
  var $i211: i1;
  var $i212: i8;
  var $i213: i1;
  var $i214: i8;
  var $i215: i1;
  var $i216: i8;
  var $i217: i1;
  var $i218: i8;
  var $i219: i1;
  var $i220: i32;
  var $i222: i8;
  var $i223: i1;
  var $i225: i8;
  var $i226: i1;
  var $i224: i1;
  var $i227: i32;
  var $i221: i32;
  var $i207: i32;
  var $i228: i1;
  var $i229: i8;
  var $i230: i32;
  var $i231: i8;
  var $i232: i1;
  var $i233: i32;
  var $i235: i32;
  var $i234: i32;
  var $i236: i1;
  var $i237: i8;
  var $i238: i1;
  var $i239: i8;
  var $i240: i8;
  var $i241: i1;
  var $i242: i8;
  var $i243: i32;
  var $i244: i8;
  var $i245: i1;
  var $i246: i8;
  var $i247: i1;
  var $i248: i8;
  var $i249: i1;
  var $i250: i8;
  var $i251: i1;
  var $i252: i8;
  var $i253: i1;
  var $i254: i32;
  var $i256: i8;
  var $i257: i1;
  var $i258: i8;
  var $i259: i1;
  var $i260: i32;
  var $i262: i32;
  var $i261: i32;
  var $i255: i32;
  var $i263: i8;
  var $i264: i1;
  var $i265: i32;
  var $i267: i8;
  var $i268: i1;
  var $i269: i8;
  var $i270: i1;
  var $i271: i8;
  var $i272: i1;
  var $i273: i8;
  var $i274: i1;
  var $i275: i8;
  var $i276: i1;
  var $i277: i32;
  var $i279: i8;
  var $i280: i1;
  var $i281: i8;
  var $i282: i1;
  var $i283: i32;
  var $i285: i32;
  var $i284: i32;
  var $i278: i32;
  var $i266: i32;
  var $i286: i8;
  var $i287: i1;
  var $i288: i32;
  var $i290: i8;
  var $i291: i1;
  var $i292: i8;
  var $i293: i1;
  var $i294: i8;
  var $i295: i1;
  var $i296: i8;
  var $i297: i1;
  var $i298: i8;
  var $i299: i1;
  var $i300: i32;
  var $i302: i8;
  var $i303: i1;
  var $i304: i8;
  var $i305: i1;
  var $i306: i32;
  var $i308: i32;
  var $i307: i32;
  var $i301: i32;
  var $i289: i32;
  var $i309: i8;
  var $i310: i1;
  var $i311: i8;
  var $i312: i1;
  var $i313: i32;
  var $i315: i8;
  var $i316: i1;
  var $i317: i8;
  var $i318: i1;
  var $i319: i8;
  var $i320: i1;
  var $i321: i8;
  var $i322: i1;
  var $i323: i8;
  var $i324: i1;
  var $i325: i8;
  var $i326: i1;
  var $i327: i32;
  var $i329: i8;
  var $i330: i1;
  var $i331: i8;
  var $i332: i1;
  var $i334: i8;
  var $i335: i1;
  var $i336: i32;
  var $i333: i32;
  var $i328: i32;
  var $i314: i32;
  var $i337: i1;
  var $i338: i8;
  var $i339: i8;
  var $i340: i1;
  var $i341: i8;
  var $i342: i1;
  var $i343: i32;
  var $i345: i8;
  var $i346: i1;
  var $i347: i8;
  var $i348: i1;
  var $i349: i8;
  var $i350: i1;
  var $i351: i8;
  var $i352: i1;
  var $i353: i8;
  var $i354: i1;
  var $i355: i8;
  var $i356: i1;
  var $i357: i32;
  var $i359: i8;
  var $i360: i1;
  var $i362: i8;
  var $i363: i1;
  var $i361: i1;
  var $i364: i32;
  var $i358: i32;
  var $i344: i32;
  var $i365: i1;
  var $i366: i8;
  var $i367: i8;
  var $i368: i1;
  var $i369: i8;
  var $i370: i1;
  var $i371: i32;
  var $i373: i8;
  var $i374: i1;
  var $i375: i8;
  var $i376: i1;
  var $i377: i8;
  var $i378: i1;
  var $i379: i8;
  var $i380: i1;
  var $i381: i8;
  var $i382: i1;
  var $i383: i8;
  var $i384: i1;
  var $i385: i32;
  var $i387: i8;
  var $i388: i1;
  var $i389: i8;
  var $i390: i1;
  var $i392: i8;
  var $i393: i1;
  var $i394: i32;
  var $i391: i32;
  var $i386: i32;
  var $i372: i32;
  var $i395: i1;
  var $i396: i8;
  var $i397: i8;
  var $i398: i1;
  var $i399: i8;
  var $i400: i1;
  var $i401: i32;
  var $i403: i8;
  var $i404: i1;
  var $i405: i8;
  var $i406: i1;
  var $i407: i8;
  var $i408: i1;
  var $i409: i8;
  var $i410: i1;
  var $i411: i8;
  var $i412: i1;
  var $i413: i8;
  var $i414: i1;
  var $i415: i32;
  var $i417: i8;
  var $i418: i1;
  var $i420: i8;
  var $i421: i1;
  var $i419: i1;
  var $i422: i32;
  var $i416: i32;
  var $i402: i32;
  var $i423: i1;
  var $i424: i8;
  var $i425: i32;
  var $i426: i8;
  var $i427: i1;
  var $i428: i32;
  var $i430: i32;
  var $i429: i32;
  var $i431: i8;
  var $i432: i1;
  var $i433: i8;
  var $i434: i1;
  var $i435: i32;
  var $i437: i8;
  var $i438: i1;
  var $i439: i8;
  var $i440: i1;
  var $i441: i32;
  var $i443: i32;
  var $i442: i32;
  var $i436: i32;
  var $i444: i8;
  var $i445: i1;
  var $i446: i8;
  var $i447: i1;
  var $i449: i8;
  var $i450: i1;
  var $i451: i32;
  var $i448: i32;
  var $i452: i1;
  var $i453: i8;
  var $i454: i8;
  var $i455: i1;
  var $i456: i8;
  var $i457: i1;
  var $i458: i8;
  var $i459: i1;
  var $i460: i8;
  var $i461: i1;
  var $i463: i8;
  var $i464: i1;
  var $i465: i32;
  var $i462: i32;
  var $i466: i1;
  var $i467: i8;
  var $i468: i8;
  var $i469: i1;
  var $i470: i8;
  var $i471: i1;
  var $i473: i8;
  var $i474: i1;
  var $i475: i32;
  var $i472: i32;
  var $i476: i1;
  var $i477: i8;
  var $i478: i8;
  var $i479: i1;
  var $i480: i8;
  var $i481: i1;
  var $i482: i8;
  var $i483: i1;
  var $i484: i8;
  var $i485: i1;
  var $i487: i8;
  var $i488: i1;
  var $i489: i32;
  var $i486: i32;
  var $i490: i1;
  var $i491: i8;
  var $i492: i8;
  var $i493: i1;
  var $i494: i8;
  var $i495: i1;
  var $i496: i32;
  var $i498: i8;
  var $i499: i1;
  var $i500: i8;
  var $i501: i1;
  var $i502: i32;
  var $i504: i32;
  var $i503: i32;
  var $i497: i32;
  var $i505: i8;
  var $i506: i1;
  var $i507: i8;
  var $i508: i1;
  var $i510: i8;
  var $i511: i1;
  var $i512: i32;
  var $i509: i32;
  var $i513: i1;
  var $i514: i8;
  var $i515: i8;
  var $i516: i1;
  var $i517: i8;
  var $i518: i1;
  var $i519: i8;
  var $i520: i1;
  var $i521: i8;
  var $i522: i1;
  var $i524: i8;
  var $i525: i1;
  var $i526: i32;
  var $i523: i32;
  var $i527: i1;
  var $i528: i8;
  var $i529: i8;
  var $i530: i1;
  var $i531: i8;
  var $i532: i1;
  var $i534: i8;
  var $i535: i1;
  var $i536: i32;
  var $i533: i32;
  var $i537: i1;
  var $i538: i8;
  var $i539: i8;
  var $i540: i1;
  var $i541: i8;
  var $i542: i1;
  var $i543: i8;
  var $i544: i1;
  var $i545: i8;
  var $i546: i1;
  var $i548: i8;
  var $i549: i1;
  var $i550: i32;
  var $i547: i32;
  var $i551: i1;
  var $i552: i8;
  var $i553: i32;
  var $i554: i32;
$bb0:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 794, 3} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 794, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 795, 15} true;
  assume {:verifier.code 0} true;
  $i1 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 795, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff1, $i1);
  call {:cexpr "x$w_buff1"} boogie_si_record_i32($i1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 796, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff0, 2);
  call {:cexpr "x$w_buff0"} boogie_si_record_i32(2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 797, 20} true;
  assume {:verifier.code 0} true;
  $i2 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 797, 20} true;
  assume {:verifier.code 0} true;
  $i3 := $trunc.i8.i1($i2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 797, 18} true;
  assume {:verifier.code 0} true;
  $i4 := $zext.i1.i8($i3);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 797, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i4);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i4);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 798, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, 1);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 23} true;
  assume {:verifier.code 0} true;
  $i5 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 23} true;
  assume {:verifier.code 0} true;
  $i6 := $trunc.i8.i1($i5);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 38} true;
  assume {:verifier.code 0} true;
  $i7 := 0;
  assume {:branchcond $i6} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i6 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 41} true;
  assume {:verifier.code 1} true;
  $i8 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 41} true;
  assume {:verifier.code 1} true;
  $i9 := $trunc.i8.i1($i8);
  assume {:verifier.code 0} true;
  $i7 := $i9;
  goto $bb3;
$bb2:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 38} true;
  assume {:verifier.code 0} true;
  assume !(($i6 == 1));
  goto $bb3;
$bb3:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 21} true;
  assume {:verifier.code 1} true;
  // llvm2bpl: output/rfi002_tso.opt_tmp.c:799:21: warning: over-approximating bitwise operation xor;
  $i10 := $xor.i1($i7, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 21} true;
  assume {:verifier.code 1} true;
  $i11 := $zext.i1.i32($i10);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 799, 3} true;
  assume {:verifier.code 1} true;
  call __VERIFIER_assert($i11);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 800, 20} true;
  assume {:verifier.code 0} true;
  $i12 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 800, 20} true;
  assume {:verifier.code 0} true;
  $i13 := $trunc.i8.i1($i12);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 800, 18} true;
  assume {:verifier.code 0} true;
  $i14 := $zext.i1.i8($i13);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 800, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd0, $i14);
  call {:cexpr "x$r_buff1_thd0"} boogie_si_record_i8($i14);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 801, 20} true;
  assume {:verifier.code 0} true;
  $i15 := $load.i8($M.0, x$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 801, 20} true;
  assume {:verifier.code 0} true;
  $i16 := $trunc.i8.i1($i15);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 801, 18} true;
  assume {:verifier.code 0} true;
  $i17 := $zext.i1.i8($i16);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 801, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd1, $i17);
  call {:cexpr "x$r_buff1_thd1"} boogie_si_record_i8($i17);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 802, 20} true;
  assume {:verifier.code 0} true;
  $i18 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 802, 20} true;
  assume {:verifier.code 0} true;
  $i19 := $trunc.i8.i1($i18);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 802, 18} true;
  assume {:verifier.code 0} true;
  $i20 := $zext.i1.i8($i19);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 802, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd2, $i20);
  call {:cexpr "x$r_buff1_thd2"} boogie_si_record_i8($i20);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 803, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd2, 1);
  call {:cexpr "x$r_buff0_thd2"} boogie_si_record_i8(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 804, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 805, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 806, 15} true;
  assume {:verifier.code 0} true;
  $i21 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 806, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y$w_buff1, $i21);
  call {:cexpr "y$w_buff1"} boogie_si_record_i32($i21);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 807, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y$w_buff0, 1);
  call {:cexpr "y$w_buff0"} boogie_si_record_i32(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 808, 20} true;
  assume {:verifier.code 0} true;
  $i22 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 808, 20} true;
  assume {:verifier.code 0} true;
  $i23 := $trunc.i8.i1($i22);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 808, 18} true;
  assume {:verifier.code 0} true;
  $i24 := $zext.i1.i8($i23);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 808, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff1_used, $i24);
  call {:cexpr "y$w_buff1_used"} boogie_si_record_i8($i24);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 809, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff0_used, 1);
  call {:cexpr "y$w_buff0_used"} boogie_si_record_i8(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 23} true;
  assume {:verifier.code 0} true;
  $i25 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 23} true;
  assume {:verifier.code 0} true;
  $i26 := $trunc.i8.i1($i25);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 38} true;
  assume {:verifier.code 0} true;
  $i27 := 0;
  assume {:branchcond $i26} true;
  goto $bb4, $bb5;
$bb4:
  assume ($i26 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 41} true;
  assume {:verifier.code 1} true;
  $i28 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 41} true;
  assume {:verifier.code 1} true;
  $i29 := $trunc.i8.i1($i28);
  assume {:verifier.code 0} true;
  $i27 := $i29;
  goto $bb6;
$bb5:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 38} true;
  assume {:verifier.code 0} true;
  assume !(($i26 == 1));
  goto $bb6;
$bb6:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 21} true;
  assume {:verifier.code 1} true;
  // llvm2bpl: output/rfi002_tso.opt_tmp.c:810:21: warning: over-approximating bitwise operation xor;
  $i30 := $xor.i1($i27, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 21} true;
  assume {:verifier.code 1} true;
  $i31 := $zext.i1.i32($i30);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 810, 3} true;
  assume {:verifier.code 1} true;
  call __VERIFIER_assert($i31);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 811, 20} true;
  assume {:verifier.code 0} true;
  $i32 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 811, 20} true;
  assume {:verifier.code 0} true;
  $i33 := $trunc.i8.i1($i32);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 811, 18} true;
  assume {:verifier.code 0} true;
  $i34 := $zext.i1.i8($i33);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 811, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd0, $i34);
  call {:cexpr "y$r_buff1_thd0"} boogie_si_record_i8($i34);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 812, 20} true;
  assume {:verifier.code 0} true;
  $i35 := $load.i8($M.0, y$r_buff0_thd1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 812, 20} true;
  assume {:verifier.code 0} true;
  $i36 := $trunc.i8.i1($i35);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 812, 18} true;
  assume {:verifier.code 0} true;
  $i37 := $zext.i1.i8($i36);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 812, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd1, $i37);
  call {:cexpr "y$r_buff1_thd1"} boogie_si_record_i8($i37);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 813, 20} true;
  assume {:verifier.code 0} true;
  $i38 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 813, 20} true;
  assume {:verifier.code 0} true;
  $i39 := $trunc.i8.i1($i38);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 813, 18} true;
  assume {:verifier.code 0} true;
  $i40 := $zext.i1.i8($i39);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 813, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd2, $i40);
  call {:cexpr "y$r_buff1_thd2"} boogie_si_record_i8($i40);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 814, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff0_thd2, 1);
  call {:cexpr "y$r_buff0_thd2"} boogie_si_record_i8(1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 815, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 816, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 817, 19} true;
  assume {:verifier.code 1} true;
  call $i41 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i41);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 817, 17} true;
  assume {:verifier.code 0} true;
  $i42 := $zext.i1.i8($i41);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 817, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice0, $i42);
  call {:cexpr "weak$$choice0"} boogie_si_record_i8($i42);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 818, 19} true;
  assume {:verifier.code 1} true;
  call $i43 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i43);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 818, 17} true;
  assume {:verifier.code 0} true;
  $i44 := $zext.i1.i8($i43);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 818, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice2, $i44);
  call {:cexpr "weak$$choice2"} boogie_si_record_i8($i44);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 819, 21} true;
  assume {:verifier.code 0} true;
  $i45 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 819, 21} true;
  assume {:verifier.code 0} true;
  $i46 := $trunc.i8.i1($i45);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 819, 19} true;
  assume {:verifier.code 0} true;
  $i47 := $zext.i1.i8($i46);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 819, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$flush_delayed, $i47);
  call {:cexpr "y$flush_delayed"} boogie_si_record_i8($i47);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 820, 15} true;
  assume {:verifier.code 0} true;
  $i48 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 820, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y$mem_tmp, $i48);
  call {:cexpr "y$mem_tmp"} boogie_si_record_i32($i48);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 8} true;
  assume {:verifier.code 0} true;
  $i49 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 8} true;
  assume {:verifier.code 0} true;
  $i50 := $trunc.i8.i1($i49);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 23} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i50} true;
  goto $bb7, $bb8;
$bb7:
  assume ($i50 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 27} true;
  assume {:verifier.code 0} true;
  $i51 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 27} true;
  assume {:verifier.code 0} true;
  $i52 := $trunc.i8.i1($i51);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 42} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i52} true;
  goto $bb10, $bb12;
$bb8:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 23} true;
  assume {:verifier.code 0} true;
  assume !(($i50 == 1));
  goto $bb9;
$bb9:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 101} true;
  assume {:verifier.code 0} true;
  $i59 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  $i60 := $i59;
  goto $bb20;
$bb10:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 42} true;
  assume {:verifier.code 0} true;
  assume ($i52 == 1);
  goto $bb11;
$bb11:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 65} true;
  assume {:verifier.code 0} true;
  $i55 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 65} true;
  assume {:verifier.code 0} true;
  $i56 := $trunc.i8.i1($i55);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 80} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i56} true;
  goto $bb15, $bb17;
$bb12:
  assume !(($i52 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 46} true;
  assume {:verifier.code 0} true;
  $i53 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 46} true;
  assume {:verifier.code 0} true;
  $i54 := $trunc.i8.i1($i53);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 61} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i54} true;
  goto $bb13, $bb14;
$bb13:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 61} true;
  assume {:verifier.code 0} true;
  assume ($i54 == 1);
  goto $bb11;
$bb14:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 61} true;
  assume {:verifier.code 0} true;
  assume !(($i54 == 1));
  goto $bb9;
$bb15:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 80} true;
  assume {:verifier.code 0} true;
  assume ($i56 == 1);
  goto $bb16;
$bb16:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  $i61 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  $i62 := $trunc.i8.i1($i61);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 121} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i62} true;
  goto $bb21, $bb22;
$bb17:
  assume !(($i56 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 84} true;
  assume {:verifier.code 0} true;
  $i57 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 84} true;
  assume {:verifier.code 0} true;
  $i58 := $trunc.i8.i1($i57);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i58} true;
  goto $bb18, $bb19;
$bb18:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  assume ($i58 == 1);
  goto $bb16;
$bb19:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i58 == 1));
  goto $bb9;
$bb20:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y, $i60);
  call {:cexpr "y"} boogie_si_record_i32($i60);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  $i68 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  $i69 := $trunc.i8.i1($i68);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i69} true;
  goto $bb27, $bb28;
$bb21:
  assume ($i62 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 124} true;
  assume {:verifier.code 0} true;
  $i63 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 124} true;
  assume {:verifier.code 0} true;
  $i64 := $trunc.i8.i1($i63);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i64} true;
  goto $bb24, $bb25;
$bb22:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 121} true;
  assume {:verifier.code 0} true;
  assume !(($i62 == 1));
  goto $bb23;
$bb23:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 153} true;
  assume {:verifier.code 0} true;
  $i67 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  $i66 := $i67;
  goto $bb26;
$bb24:
  assume ($i64 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 141} true;
  assume {:verifier.code 0} true;
  $i65 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  $i66 := $i65;
  goto $bb26;
$bb25:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  assume !(($i64 == 1));
  goto $bb23;
$bb26:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 106} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 821, 7} true;
  assume {:verifier.code 0} true;
  $i60 := $i66;
  goto $bb20;
$bb27:
  assume ($i69 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 31} true;
  assume {:verifier.code 0} true;
  $i70 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  $i71 := $i70;
  goto $bb29;
$bb28:
  assume !(($i69 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 45} true;
  assume {:verifier.code 0} true;
  $i72 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 45} true;
  assume {:verifier.code 0} true;
  $i73 := $trunc.i8.i1($i72);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i73} true;
  goto $bb30, $bb31;
$bb29:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y$w_buff0, $i71);
  call {:cexpr "y$w_buff0"} boogie_si_record_i32($i71);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  $i91 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  $i92 := $trunc.i8.i1($i91);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i92} true;
  goto $bb50, $bb51;
$bb30:
  assume ($i73 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 64} true;
  assume {:verifier.code 0} true;
  $i74 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 64} true;
  assume {:verifier.code 0} true;
  $i75 := $trunc.i8.i1($i74);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i75} true;
  goto $bb33, $bb35;
$bb31:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i73 == 1));
  goto $bb32;
$bb32:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 138} true;
  assume {:verifier.code 0} true;
  $i82 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  $i83 := $i82;
  goto $bb43;
$bb33:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 79} true;
  assume {:verifier.code 0} true;
  assume ($i75 == 1);
  goto $bb34;
$bb34:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 102} true;
  assume {:verifier.code 0} true;
  $i78 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 102} true;
  assume {:verifier.code 0} true;
  $i79 := $trunc.i8.i1($i78);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i79} true;
  goto $bb38, $bb40;
$bb35:
  assume !(($i75 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 83} true;
  assume {:verifier.code 0} true;
  $i76 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 83} true;
  assume {:verifier.code 0} true;
  $i77 := $trunc.i8.i1($i76);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i77} true;
  goto $bb36, $bb37;
$bb36:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 98} true;
  assume {:verifier.code 0} true;
  assume ($i77 == 1);
  goto $bb34;
$bb37:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i77 == 1));
  goto $bb32;
$bb38:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 117} true;
  assume {:verifier.code 0} true;
  assume ($i79 == 1);
  goto $bb39;
$bb39:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  $i84 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  $i85 := $trunc.i8.i1($i84);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i85} true;
  goto $bb44, $bb45;
$bb40:
  assume !(($i79 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 121} true;
  assume {:verifier.code 0} true;
  $i80 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 121} true;
  assume {:verifier.code 0} true;
  $i81 := $trunc.i8.i1($i80);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i81} true;
  goto $bb41, $bb42;
$bb41:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  assume ($i81 == 1);
  goto $bb39;
$bb42:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i81 == 1));
  goto $bb32;
$bb43:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 15} true;
  assume {:verifier.code 0} true;
  $i71 := $i83;
  goto $bb29;
$bb44:
  assume ($i85 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 169} true;
  assume {:verifier.code 0} true;
  $i86 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 169} true;
  assume {:verifier.code 0} true;
  $i87 := $trunc.i8.i1($i86);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i87} true;
  goto $bb47, $bb48;
$bb45:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i85 == 1));
  goto $bb46;
$bb46:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 198} true;
  assume {:verifier.code 0} true;
  $i90 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  $i89 := $i90;
  goto $bb49;
$bb47:
  assume ($i87 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 186} true;
  assume {:verifier.code 0} true;
  $i88 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  $i89 := $i88;
  goto $bb49;
$bb48:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i87 == 1));
  goto $bb46;
$bb49:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 822, 44} true;
  assume {:verifier.code 0} true;
  $i83 := $i89;
  goto $bb43;
$bb50:
  assume ($i92 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 31} true;
  assume {:verifier.code 0} true;
  $i93 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  $i94 := $i93;
  goto $bb52;
$bb51:
  assume !(($i92 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 45} true;
  assume {:verifier.code 0} true;
  $i95 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 45} true;
  assume {:verifier.code 0} true;
  $i96 := $trunc.i8.i1($i95);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i96} true;
  goto $bb53, $bb54;
$bb52:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y$w_buff1, $i94);
  call {:cexpr "y$w_buff1"} boogie_si_record_i32($i94);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  $i114 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  $i115 := $trunc.i8.i1($i114);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i115} true;
  goto $bb73, $bb74;
$bb53:
  assume ($i96 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 64} true;
  assume {:verifier.code 0} true;
  $i97 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 64} true;
  assume {:verifier.code 0} true;
  $i98 := $trunc.i8.i1($i97);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i98} true;
  goto $bb56, $bb58;
$bb54:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i96 == 1));
  goto $bb55;
$bb55:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 138} true;
  assume {:verifier.code 0} true;
  $i105 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  $i106 := $i105;
  goto $bb66;
$bb56:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 79} true;
  assume {:verifier.code 0} true;
  assume ($i98 == 1);
  goto $bb57;
$bb57:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 102} true;
  assume {:verifier.code 0} true;
  $i101 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 102} true;
  assume {:verifier.code 0} true;
  $i102 := $trunc.i8.i1($i101);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i102} true;
  goto $bb61, $bb63;
$bb58:
  assume !(($i98 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 83} true;
  assume {:verifier.code 0} true;
  $i99 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 83} true;
  assume {:verifier.code 0} true;
  $i100 := $trunc.i8.i1($i99);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i100} true;
  goto $bb59, $bb60;
$bb59:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 98} true;
  assume {:verifier.code 0} true;
  assume ($i100 == 1);
  goto $bb57;
$bb60:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i100 == 1));
  goto $bb55;
$bb61:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 117} true;
  assume {:verifier.code 0} true;
  assume ($i102 == 1);
  goto $bb62;
$bb62:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  $i107 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  $i108 := $trunc.i8.i1($i107);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i108} true;
  goto $bb67, $bb68;
$bb63:
  assume !(($i102 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 121} true;
  assume {:verifier.code 0} true;
  $i103 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 121} true;
  assume {:verifier.code 0} true;
  $i104 := $trunc.i8.i1($i103);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i104} true;
  goto $bb64, $bb65;
$bb64:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  assume ($i104 == 1);
  goto $bb62;
$bb65:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i104 == 1));
  goto $bb55;
$bb66:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 15} true;
  assume {:verifier.code 0} true;
  $i94 := $i106;
  goto $bb52;
$bb67:
  assume ($i108 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 169} true;
  assume {:verifier.code 0} true;
  $i109 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 169} true;
  assume {:verifier.code 0} true;
  $i110 := $trunc.i8.i1($i109);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i110} true;
  goto $bb70, $bb71;
$bb68:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i108 == 1));
  goto $bb69;
$bb69:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 198} true;
  assume {:verifier.code 0} true;
  $i113 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  $i112 := $i113;
  goto $bb72;
$bb70:
  assume ($i110 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 186} true;
  assume {:verifier.code 0} true;
  $i111 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  $i112 := $i111;
  goto $bb72;
$bb71:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i110 == 1));
  goto $bb69;
$bb72:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 823, 44} true;
  assume {:verifier.code 0} true;
  $i106 := $i112;
  goto $bb66;
$bb73:
  assume ($i115 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 36} true;
  assume {:verifier.code 0} true;
  $i116 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 36} true;
  assume {:verifier.code 0} true;
  $i117 := $trunc.i8.i1($i116);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 36} true;
  assume {:verifier.code 0} true;
  $i118 := $zext.i1.i32($i117);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  $i119 := $i118;
  goto $bb75;
$bb74:
  assume !(($i115 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 55} true;
  assume {:verifier.code 0} true;
  $i120 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 55} true;
  assume {:verifier.code 0} true;
  $i121 := $trunc.i8.i1($i120);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i121} true;
  goto $bb76, $bb77;
$bb75:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  $i142 := $ne.i32($i119, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 18} true;
  assume {:verifier.code 0} true;
  $i143 := $zext.i1.i8($i142);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff0_used, $i143);
  call {:cexpr "y$w_buff0_used"} boogie_si_record_i8($i143);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  $i144 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  $i145 := $trunc.i8.i1($i144);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i145} true;
  goto $bb96, $bb97;
$bb76:
  assume ($i121 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 74} true;
  assume {:verifier.code 0} true;
  $i122 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 74} true;
  assume {:verifier.code 0} true;
  $i123 := $trunc.i8.i1($i122);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i123} true;
  goto $bb79, $bb81;
$bb77:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i121 == 1));
  goto $bb78;
$bb78:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 148} true;
  assume {:verifier.code 0} true;
  $i130 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 148} true;
  assume {:verifier.code 0} true;
  $i131 := $trunc.i8.i1($i130);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 148} true;
  assume {:verifier.code 0} true;
  $i132 := $zext.i1.i32($i131);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  $i133 := $i132;
  goto $bb89;
$bb79:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 89} true;
  assume {:verifier.code 0} true;
  assume ($i123 == 1);
  goto $bb80;
$bb80:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 112} true;
  assume {:verifier.code 0} true;
  $i126 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 112} true;
  assume {:verifier.code 0} true;
  $i127 := $trunc.i8.i1($i126);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i127} true;
  goto $bb84, $bb86;
$bb81:
  assume !(($i123 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 93} true;
  assume {:verifier.code 0} true;
  $i124 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 93} true;
  assume {:verifier.code 0} true;
  $i125 := $trunc.i8.i1($i124);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i125} true;
  goto $bb82, $bb83;
$bb82:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 108} true;
  assume {:verifier.code 0} true;
  assume ($i125 == 1);
  goto $bb80;
$bb83:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i125 == 1));
  goto $bb78;
$bb84:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 127} true;
  assume {:verifier.code 0} true;
  assume ($i127 == 1);
  goto $bb85;
$bb85:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  $i134 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  $i135 := $trunc.i8.i1($i134);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i135} true;
  goto $bb90, $bb91;
$bb86:
  assume !(($i127 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 131} true;
  assume {:verifier.code 0} true;
  $i128 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 131} true;
  assume {:verifier.code 0} true;
  $i129 := $trunc.i8.i1($i128);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i129} true;
  goto $bb87, $bb88;
$bb87:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  assume ($i129 == 1);
  goto $bb85;
$bb88:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i129 == 1));
  goto $bb78;
$bb89:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 20} true;
  assume {:verifier.code 0} true;
  $i119 := $i133;
  goto $bb75;
$bb90:
  assume ($i135 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 184} true;
  assume {:verifier.code 0} true;
  $i136 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 184} true;
  assume {:verifier.code 0} true;
  $i137 := $trunc.i8.i1($i136);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i137} true;
  goto $bb93, $bb94;
$bb91:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i135 == 1));
  goto $bb92;
$bb92:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 212} true;
  assume {:verifier.code 0} true;
  $i139 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 212} true;
  assume {:verifier.code 0} true;
  $i140 := $trunc.i8.i1($i139);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 212} true;
  assume {:verifier.code 0} true;
  $i141 := $zext.i1.i32($i140);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  $i138 := $i141;
  goto $bb95;
$bb93:
  assume ($i137 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  $i138 := 0;
  goto $bb95;
$bb94:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i137 == 1));
  goto $bb92;
$bb95:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 824, 54} true;
  assume {:verifier.code 0} true;
  $i133 := $i138;
  goto $bb89;
$bb96:
  assume ($i145 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 36} true;
  assume {:verifier.code 0} true;
  $i146 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 36} true;
  assume {:verifier.code 0} true;
  $i147 := $trunc.i8.i1($i146);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 36} true;
  assume {:verifier.code 0} true;
  $i148 := $zext.i1.i32($i147);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  $i149 := $i148;
  goto $bb98;
$bb97:
  assume !(($i145 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 55} true;
  assume {:verifier.code 0} true;
  $i150 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 55} true;
  assume {:verifier.code 0} true;
  $i151 := $trunc.i8.i1($i150);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i151} true;
  goto $bb99, $bb100;
$bb98:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  $i170 := $ne.i32($i149, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 18} true;
  assume {:verifier.code 0} true;
  $i171 := $zext.i1.i8($i170);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff1_used, $i171);
  call {:cexpr "y$w_buff1_used"} boogie_si_record_i8($i171);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  $i172 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  $i173 := $trunc.i8.i1($i172);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i173} true;
  goto $bb116, $bb117;
$bb99:
  assume ($i151 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 74} true;
  assume {:verifier.code 0} true;
  $i152 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 74} true;
  assume {:verifier.code 0} true;
  $i153 := $trunc.i8.i1($i152);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i153} true;
  goto $bb102, $bb104;
$bb100:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i151 == 1));
  goto $bb101;
$bb101:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 148} true;
  assume {:verifier.code 0} true;
  $i160 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 148} true;
  assume {:verifier.code 0} true;
  $i161 := $trunc.i8.i1($i160);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 148} true;
  assume {:verifier.code 0} true;
  $i162 := $zext.i1.i32($i161);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  $i163 := $i162;
  goto $bb112;
$bb102:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 89} true;
  assume {:verifier.code 0} true;
  assume ($i153 == 1);
  goto $bb103;
$bb103:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 112} true;
  assume {:verifier.code 0} true;
  $i156 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 112} true;
  assume {:verifier.code 0} true;
  $i157 := $trunc.i8.i1($i156);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i157} true;
  goto $bb107, $bb109;
$bb104:
  assume !(($i153 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 93} true;
  assume {:verifier.code 0} true;
  $i154 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 93} true;
  assume {:verifier.code 0} true;
  $i155 := $trunc.i8.i1($i154);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i155} true;
  goto $bb105, $bb106;
$bb105:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 108} true;
  assume {:verifier.code 0} true;
  assume ($i155 == 1);
  goto $bb103;
$bb106:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i155 == 1));
  goto $bb101;
$bb107:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 127} true;
  assume {:verifier.code 0} true;
  assume ($i157 == 1);
  goto $bb108;
$bb108:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 166} true;
  assume {:verifier.code 0} true;
  $i164 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 166} true;
  assume {:verifier.code 0} true;
  $i165 := $trunc.i8.i1($i164);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 181} true;
  assume {:verifier.code 0} true;
  $i166 := 0;
  assume {:branchcond $i165} true;
  goto $bb113, $bb114;
$bb109:
  assume !(($i157 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 131} true;
  assume {:verifier.code 0} true;
  $i158 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 131} true;
  assume {:verifier.code 0} true;
  $i159 := $trunc.i8.i1($i158);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i159} true;
  goto $bb110, $bb111;
$bb110:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  assume ($i159 == 1);
  goto $bb108;
$bb111:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i159 == 1));
  goto $bb101;
$bb112:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 20} true;
  assume {:verifier.code 0} true;
  $i149 := $i163;
  goto $bb98;
$bb113:
  assume ($i165 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 184} true;
  assume {:verifier.code 0} true;
  $i167 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 184} true;
  assume {:verifier.code 0} true;
  $i168 := $trunc.i8.i1($i167);
  assume {:verifier.code 0} true;
  $i166 := $i168;
  goto $bb115;
$bb114:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i165 == 1));
  goto $bb115;
$bb115:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 166} true;
  assume {:verifier.code 0} true;
  $i169 := (if ($i166 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 825, 54} true;
  assume {:verifier.code 0} true;
  $i163 := $i169;
  goto $bb112;
$bb116:
  assume ($i173 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 36} true;
  assume {:verifier.code 0} true;
  $i174 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 36} true;
  assume {:verifier.code 0} true;
  $i175 := $trunc.i8.i1($i174);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 36} true;
  assume {:verifier.code 0} true;
  $i176 := $zext.i1.i32($i175);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  $i177 := $i176;
  goto $bb118;
$bb117:
  assume !(($i173 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 55} true;
  assume {:verifier.code 0} true;
  $i178 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 55} true;
  assume {:verifier.code 0} true;
  $i179 := $trunc.i8.i1($i178);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i179} true;
  goto $bb119, $bb120;
$bb118:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  $i200 := $ne.i32($i177, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 18} true;
  assume {:verifier.code 0} true;
  $i201 := $zext.i1.i8($i200);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff0_thd2, $i201);
  call {:cexpr "y$r_buff0_thd2"} boogie_si_record_i8($i201);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  $i202 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  $i203 := $trunc.i8.i1($i202);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i203} true;
  goto $bb139, $bb140;
$bb119:
  assume ($i179 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 74} true;
  assume {:verifier.code 0} true;
  $i180 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 74} true;
  assume {:verifier.code 0} true;
  $i181 := $trunc.i8.i1($i180);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i181} true;
  goto $bb122, $bb124;
$bb120:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i179 == 1));
  goto $bb121;
$bb121:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 148} true;
  assume {:verifier.code 0} true;
  $i188 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 148} true;
  assume {:verifier.code 0} true;
  $i189 := $trunc.i8.i1($i188);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 148} true;
  assume {:verifier.code 0} true;
  $i190 := $zext.i1.i32($i189);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  $i191 := $i190;
  goto $bb132;
$bb122:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 89} true;
  assume {:verifier.code 0} true;
  assume ($i181 == 1);
  goto $bb123;
$bb123:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 112} true;
  assume {:verifier.code 0} true;
  $i184 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 112} true;
  assume {:verifier.code 0} true;
  $i185 := $trunc.i8.i1($i184);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i185} true;
  goto $bb127, $bb129;
$bb124:
  assume !(($i181 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 93} true;
  assume {:verifier.code 0} true;
  $i182 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 93} true;
  assume {:verifier.code 0} true;
  $i183 := $trunc.i8.i1($i182);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i183} true;
  goto $bb125, $bb126;
$bb125:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 108} true;
  assume {:verifier.code 0} true;
  assume ($i183 == 1);
  goto $bb123;
$bb126:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i183 == 1));
  goto $bb121;
$bb127:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 127} true;
  assume {:verifier.code 0} true;
  assume ($i185 == 1);
  goto $bb128;
$bb128:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  $i192 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  $i193 := $trunc.i8.i1($i192);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i193} true;
  goto $bb133, $bb134;
$bb129:
  assume !(($i185 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 131} true;
  assume {:verifier.code 0} true;
  $i186 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 131} true;
  assume {:verifier.code 0} true;
  $i187 := $trunc.i8.i1($i186);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i187} true;
  goto $bb130, $bb131;
$bb130:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  assume ($i187 == 1);
  goto $bb128;
$bb131:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i187 == 1));
  goto $bb121;
$bb132:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 20} true;
  assume {:verifier.code 0} true;
  $i177 := $i191;
  goto $bb118;
$bb133:
  assume ($i193 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 184} true;
  assume {:verifier.code 0} true;
  $i194 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 184} true;
  assume {:verifier.code 0} true;
  $i195 := $trunc.i8.i1($i194);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i195} true;
  goto $bb136, $bb137;
$bb134:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i193 == 1));
  goto $bb135;
$bb135:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 212} true;
  assume {:verifier.code 0} true;
  $i197 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 212} true;
  assume {:verifier.code 0} true;
  $i198 := $trunc.i8.i1($i197);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 212} true;
  assume {:verifier.code 0} true;
  $i199 := $zext.i1.i32($i198);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  $i196 := $i199;
  goto $bb138;
$bb136:
  assume ($i195 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  $i196 := 0;
  goto $bb138;
$bb137:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i195 == 1));
  goto $bb135;
$bb138:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 826, 54} true;
  assume {:verifier.code 0} true;
  $i191 := $i196;
  goto $bb132;
$bb139:
  assume ($i203 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 36} true;
  assume {:verifier.code 0} true;
  $i204 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 36} true;
  assume {:verifier.code 0} true;
  $i205 := $trunc.i8.i1($i204);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 36} true;
  assume {:verifier.code 0} true;
  $i206 := $zext.i1.i32($i205);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  $i207 := $i206;
  goto $bb141;
$bb140:
  assume !(($i203 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 55} true;
  assume {:verifier.code 0} true;
  $i208 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 55} true;
  assume {:verifier.code 0} true;
  $i209 := $trunc.i8.i1($i208);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i209} true;
  goto $bb142, $bb143;
$bb141:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  $i228 := $ne.i32($i207, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 18} true;
  assume {:verifier.code 0} true;
  $i229 := $zext.i1.i8($i228);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd2, $i229);
  call {:cexpr "y$r_buff1_thd2"} boogie_si_record_i8($i229);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 828, 25} true;
  assume {:verifier.code 0} true;
  $i230 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 828, 23} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, __unbuffered_p1_EAX, $i230);
  call {:cexpr "__unbuffered_p1_EAX"} boogie_si_record_i32($i230);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  $i231 := $load.i8($M.0, y$flush_delayed);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  $i232 := $trunc.i8.i1($i231);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i232} true;
  goto $bb159, $bb160;
$bb142:
  assume ($i209 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 74} true;
  assume {:verifier.code 0} true;
  $i210 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 74} true;
  assume {:verifier.code 0} true;
  $i211 := $trunc.i8.i1($i210);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i211} true;
  goto $bb145, $bb147;
$bb143:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i209 == 1));
  goto $bb144;
$bb144:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 148} true;
  assume {:verifier.code 0} true;
  $i218 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 148} true;
  assume {:verifier.code 0} true;
  $i219 := $trunc.i8.i1($i218);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 148} true;
  assume {:verifier.code 0} true;
  $i220 := $zext.i1.i32($i219);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  $i221 := $i220;
  goto $bb155;
$bb145:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 89} true;
  assume {:verifier.code 0} true;
  assume ($i211 == 1);
  goto $bb146;
$bb146:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 112} true;
  assume {:verifier.code 0} true;
  $i214 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 112} true;
  assume {:verifier.code 0} true;
  $i215 := $trunc.i8.i1($i214);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i215} true;
  goto $bb150, $bb152;
$bb147:
  assume !(($i211 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 93} true;
  assume {:verifier.code 0} true;
  $i212 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 93} true;
  assume {:verifier.code 0} true;
  $i213 := $trunc.i8.i1($i212);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i213} true;
  goto $bb148, $bb149;
$bb148:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 108} true;
  assume {:verifier.code 0} true;
  assume ($i213 == 1);
  goto $bb146;
$bb149:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i213 == 1));
  goto $bb144;
$bb150:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 127} true;
  assume {:verifier.code 0} true;
  assume ($i215 == 1);
  goto $bb151;
$bb151:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 166} true;
  assume {:verifier.code 0} true;
  $i222 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 166} true;
  assume {:verifier.code 0} true;
  $i223 := $trunc.i8.i1($i222);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 181} true;
  assume {:verifier.code 0} true;
  $i224 := 0;
  assume {:branchcond $i223} true;
  goto $bb156, $bb157;
$bb152:
  assume !(($i215 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 131} true;
  assume {:verifier.code 0} true;
  $i216 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 131} true;
  assume {:verifier.code 0} true;
  $i217 := $trunc.i8.i1($i216);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i217} true;
  goto $bb153, $bb154;
$bb153:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  assume ($i217 == 1);
  goto $bb151;
$bb154:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i217 == 1));
  goto $bb144;
$bb155:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 20} true;
  assume {:verifier.code 0} true;
  $i207 := $i221;
  goto $bb141;
$bb156:
  assume ($i223 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 184} true;
  assume {:verifier.code 0} true;
  $i225 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 184} true;
  assume {:verifier.code 0} true;
  $i226 := $trunc.i8.i1($i225);
  assume {:verifier.code 0} true;
  $i224 := $i226;
  goto $bb158;
$bb157:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i223 == 1));
  goto $bb158;
$bb158:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 166} true;
  assume {:verifier.code 0} true;
  $i227 := (if ($i224 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 827, 54} true;
  assume {:verifier.code 0} true;
  $i221 := $i227;
  goto $bb155;
$bb159:
  assume ($i232 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 25} true;
  assume {:verifier.code 0} true;
  $i233 := $load.i32($M.0, y$mem_tmp);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  $i234 := $i233;
  goto $bb161;
$bb160:
  assume !(($i232 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 37} true;
  assume {:verifier.code 0} true;
  $i235 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  $i234 := $i235;
  goto $bb161;
$bb161:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 829, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y, $i234);
  call {:cexpr "y"} boogie_si_record_i32($i234);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 830, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$flush_delayed, 0);
  call {:cexpr "y$flush_delayed"} boogie_si_record_i8(0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 831, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 832, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 833, 19} true;
  assume {:verifier.code 1} true;
  call $i236 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i236);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 833, 17} true;
  assume {:verifier.code 0} true;
  $i237 := $zext.i1.i8($i236);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 833, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice0, $i237);
  call {:cexpr "weak$$choice0"} boogie_si_record_i8($i237);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 834, 19} true;
  assume {:verifier.code 1} true;
  call $i238 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i238);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 834, 17} true;
  assume {:verifier.code 0} true;
  $i239 := $zext.i1.i8($i238);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 834, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice2, $i239);
  call {:cexpr "weak$$choice2"} boogie_si_record_i8($i239);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 835, 21} true;
  assume {:verifier.code 0} true;
  $i240 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 835, 21} true;
  assume {:verifier.code 0} true;
  $i241 := $trunc.i8.i1($i240);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 835, 19} true;
  assume {:verifier.code 0} true;
  $i242 := $zext.i1.i8($i241);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 835, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, $i242);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8($i242);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 836, 15} true;
  assume {:verifier.code 0} true;
  $i243 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 836, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$mem_tmp, $i243);
  call {:cexpr "x$mem_tmp"} boogie_si_record_i32($i243);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 8} true;
  assume {:verifier.code 0} true;
  $i244 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 8} true;
  assume {:verifier.code 0} true;
  $i245 := $trunc.i8.i1($i244);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 23} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i245} true;
  goto $bb162, $bb163;
$bb162:
  assume ($i245 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 27} true;
  assume {:verifier.code 0} true;
  $i246 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 27} true;
  assume {:verifier.code 0} true;
  $i247 := $trunc.i8.i1($i246);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 42} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i247} true;
  goto $bb165, $bb167;
$bb163:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 23} true;
  assume {:verifier.code 0} true;
  assume !(($i245 == 1));
  goto $bb164;
$bb164:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 101} true;
  assume {:verifier.code 0} true;
  $i254 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  $i255 := $i254;
  goto $bb175;
$bb165:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 42} true;
  assume {:verifier.code 0} true;
  assume ($i247 == 1);
  goto $bb166;
$bb166:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 65} true;
  assume {:verifier.code 0} true;
  $i250 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 65} true;
  assume {:verifier.code 0} true;
  $i251 := $trunc.i8.i1($i250);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 80} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i251} true;
  goto $bb170, $bb172;
$bb167:
  assume !(($i247 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 46} true;
  assume {:verifier.code 0} true;
  $i248 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 46} true;
  assume {:verifier.code 0} true;
  $i249 := $trunc.i8.i1($i248);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 61} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i249} true;
  goto $bb168, $bb169;
$bb168:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 61} true;
  assume {:verifier.code 0} true;
  assume ($i249 == 1);
  goto $bb166;
$bb169:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 61} true;
  assume {:verifier.code 0} true;
  assume !(($i249 == 1));
  goto $bb164;
$bb170:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 80} true;
  assume {:verifier.code 0} true;
  assume ($i251 == 1);
  goto $bb171;
$bb171:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  $i256 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  $i257 := $trunc.i8.i1($i256);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 121} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i257} true;
  goto $bb176, $bb177;
$bb172:
  assume !(($i251 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 84} true;
  assume {:verifier.code 0} true;
  $i252 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 84} true;
  assume {:verifier.code 0} true;
  $i253 := $trunc.i8.i1($i252);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i253} true;
  goto $bb173, $bb174;
$bb173:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  assume ($i253 == 1);
  goto $bb171;
$bb174:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i253 == 1));
  goto $bb164;
$bb175:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i255);
  call {:cexpr "x"} boogie_si_record_i32($i255);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  $i263 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  $i264 := $trunc.i8.i1($i263);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i264} true;
  goto $bb182, $bb183;
$bb176:
  assume ($i257 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 124} true;
  assume {:verifier.code 0} true;
  $i258 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 124} true;
  assume {:verifier.code 0} true;
  $i259 := $trunc.i8.i1($i258);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i259} true;
  goto $bb179, $bb180;
$bb177:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 121} true;
  assume {:verifier.code 0} true;
  assume !(($i257 == 1));
  goto $bb178;
$bb178:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 153} true;
  assume {:verifier.code 0} true;
  $i262 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  $i261 := $i262;
  goto $bb181;
$bb179:
  assume ($i259 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 141} true;
  assume {:verifier.code 0} true;
  $i260 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  $i261 := $i260;
  goto $bb181;
$bb180:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  assume !(($i259 == 1));
  goto $bb178;
$bb181:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 106} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 837, 7} true;
  assume {:verifier.code 0} true;
  $i255 := $i261;
  goto $bb175;
$bb182:
  assume ($i264 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 31} true;
  assume {:verifier.code 0} true;
  $i265 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  $i266 := $i265;
  goto $bb184;
$bb183:
  assume !(($i264 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 45} true;
  assume {:verifier.code 0} true;
  $i267 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 45} true;
  assume {:verifier.code 0} true;
  $i268 := $trunc.i8.i1($i267);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i268} true;
  goto $bb185, $bb186;
$bb184:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff0, $i266);
  call {:cexpr "x$w_buff0"} boogie_si_record_i32($i266);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  $i286 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  $i287 := $trunc.i8.i1($i286);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i287} true;
  goto $bb205, $bb206;
$bb185:
  assume ($i268 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 64} true;
  assume {:verifier.code 0} true;
  $i269 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 64} true;
  assume {:verifier.code 0} true;
  $i270 := $trunc.i8.i1($i269);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i270} true;
  goto $bb188, $bb190;
$bb186:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i268 == 1));
  goto $bb187;
$bb187:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 138} true;
  assume {:verifier.code 0} true;
  $i277 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  $i278 := $i277;
  goto $bb198;
$bb188:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 79} true;
  assume {:verifier.code 0} true;
  assume ($i270 == 1);
  goto $bb189;
$bb189:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 102} true;
  assume {:verifier.code 0} true;
  $i273 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 102} true;
  assume {:verifier.code 0} true;
  $i274 := $trunc.i8.i1($i273);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i274} true;
  goto $bb193, $bb195;
$bb190:
  assume !(($i270 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 83} true;
  assume {:verifier.code 0} true;
  $i271 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 83} true;
  assume {:verifier.code 0} true;
  $i272 := $trunc.i8.i1($i271);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i272} true;
  goto $bb191, $bb192;
$bb191:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 98} true;
  assume {:verifier.code 0} true;
  assume ($i272 == 1);
  goto $bb189;
$bb192:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i272 == 1));
  goto $bb187;
$bb193:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 117} true;
  assume {:verifier.code 0} true;
  assume ($i274 == 1);
  goto $bb194;
$bb194:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  $i279 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  $i280 := $trunc.i8.i1($i279);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i280} true;
  goto $bb199, $bb200;
$bb195:
  assume !(($i274 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 121} true;
  assume {:verifier.code 0} true;
  $i275 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 121} true;
  assume {:verifier.code 0} true;
  $i276 := $trunc.i8.i1($i275);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i276} true;
  goto $bb196, $bb197;
$bb196:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  assume ($i276 == 1);
  goto $bb194;
$bb197:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i276 == 1));
  goto $bb187;
$bb198:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 15} true;
  assume {:verifier.code 0} true;
  $i266 := $i278;
  goto $bb184;
$bb199:
  assume ($i280 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 169} true;
  assume {:verifier.code 0} true;
  $i281 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 169} true;
  assume {:verifier.code 0} true;
  $i282 := $trunc.i8.i1($i281);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i282} true;
  goto $bb202, $bb203;
$bb200:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i280 == 1));
  goto $bb201;
$bb201:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 198} true;
  assume {:verifier.code 0} true;
  $i285 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  $i284 := $i285;
  goto $bb204;
$bb202:
  assume ($i282 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 186} true;
  assume {:verifier.code 0} true;
  $i283 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  $i284 := $i283;
  goto $bb204;
$bb203:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i282 == 1));
  goto $bb201;
$bb204:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 838, 44} true;
  assume {:verifier.code 0} true;
  $i278 := $i284;
  goto $bb198;
$bb205:
  assume ($i287 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 31} true;
  assume {:verifier.code 0} true;
  $i288 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  $i289 := $i288;
  goto $bb207;
$bb206:
  assume !(($i287 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 45} true;
  assume {:verifier.code 0} true;
  $i290 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 45} true;
  assume {:verifier.code 0} true;
  $i291 := $trunc.i8.i1($i290);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i291} true;
  goto $bb208, $bb209;
$bb207:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff1, $i289);
  call {:cexpr "x$w_buff1"} boogie_si_record_i32($i289);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  $i309 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  $i310 := $trunc.i8.i1($i309);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i310} true;
  goto $bb228, $bb229;
$bb208:
  assume ($i291 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 64} true;
  assume {:verifier.code 0} true;
  $i292 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 64} true;
  assume {:verifier.code 0} true;
  $i293 := $trunc.i8.i1($i292);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i293} true;
  goto $bb211, $bb213;
$bb209:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i291 == 1));
  goto $bb210;
$bb210:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 138} true;
  assume {:verifier.code 0} true;
  $i300 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  $i301 := $i300;
  goto $bb221;
$bb211:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 79} true;
  assume {:verifier.code 0} true;
  assume ($i293 == 1);
  goto $bb212;
$bb212:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 102} true;
  assume {:verifier.code 0} true;
  $i296 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 102} true;
  assume {:verifier.code 0} true;
  $i297 := $trunc.i8.i1($i296);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i297} true;
  goto $bb216, $bb218;
$bb213:
  assume !(($i293 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 83} true;
  assume {:verifier.code 0} true;
  $i294 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 83} true;
  assume {:verifier.code 0} true;
  $i295 := $trunc.i8.i1($i294);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i295} true;
  goto $bb214, $bb215;
$bb214:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 98} true;
  assume {:verifier.code 0} true;
  assume ($i295 == 1);
  goto $bb212;
$bb215:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i295 == 1));
  goto $bb210;
$bb216:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 117} true;
  assume {:verifier.code 0} true;
  assume ($i297 == 1);
  goto $bb217;
$bb217:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  $i302 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  $i303 := $trunc.i8.i1($i302);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i303} true;
  goto $bb222, $bb223;
$bb218:
  assume !(($i297 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 121} true;
  assume {:verifier.code 0} true;
  $i298 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 121} true;
  assume {:verifier.code 0} true;
  $i299 := $trunc.i8.i1($i298);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i299} true;
  goto $bb219, $bb220;
$bb219:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  assume ($i299 == 1);
  goto $bb217;
$bb220:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i299 == 1));
  goto $bb210;
$bb221:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 15} true;
  assume {:verifier.code 0} true;
  $i289 := $i301;
  goto $bb207;
$bb222:
  assume ($i303 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 169} true;
  assume {:verifier.code 0} true;
  $i304 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 169} true;
  assume {:verifier.code 0} true;
  $i305 := $trunc.i8.i1($i304);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i305} true;
  goto $bb225, $bb226;
$bb223:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i303 == 1));
  goto $bb224;
$bb224:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 198} true;
  assume {:verifier.code 0} true;
  $i308 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  $i307 := $i308;
  goto $bb227;
$bb225:
  assume ($i305 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 186} true;
  assume {:verifier.code 0} true;
  $i306 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  $i307 := $i306;
  goto $bb227;
$bb226:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i305 == 1));
  goto $bb224;
$bb227:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 839, 44} true;
  assume {:verifier.code 0} true;
  $i301 := $i307;
  goto $bb221;
$bb228:
  assume ($i310 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 36} true;
  assume {:verifier.code 0} true;
  $i311 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 36} true;
  assume {:verifier.code 0} true;
  $i312 := $trunc.i8.i1($i311);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 36} true;
  assume {:verifier.code 0} true;
  $i313 := $zext.i1.i32($i312);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  $i314 := $i313;
  goto $bb230;
$bb229:
  assume !(($i310 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 55} true;
  assume {:verifier.code 0} true;
  $i315 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 55} true;
  assume {:verifier.code 0} true;
  $i316 := $trunc.i8.i1($i315);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i316} true;
  goto $bb231, $bb232;
$bb230:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  $i337 := $ne.i32($i314, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 18} true;
  assume {:verifier.code 0} true;
  $i338 := $zext.i1.i8($i337);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i338);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i338);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  $i339 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  $i340 := $trunc.i8.i1($i339);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i340} true;
  goto $bb251, $bb252;
$bb231:
  assume ($i316 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 74} true;
  assume {:verifier.code 0} true;
  $i317 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 74} true;
  assume {:verifier.code 0} true;
  $i318 := $trunc.i8.i1($i317);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i318} true;
  goto $bb234, $bb236;
$bb232:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i316 == 1));
  goto $bb233;
$bb233:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 148} true;
  assume {:verifier.code 0} true;
  $i325 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 148} true;
  assume {:verifier.code 0} true;
  $i326 := $trunc.i8.i1($i325);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 148} true;
  assume {:verifier.code 0} true;
  $i327 := $zext.i1.i32($i326);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  $i328 := $i327;
  goto $bb244;
$bb234:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 89} true;
  assume {:verifier.code 0} true;
  assume ($i318 == 1);
  goto $bb235;
$bb235:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 112} true;
  assume {:verifier.code 0} true;
  $i321 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 112} true;
  assume {:verifier.code 0} true;
  $i322 := $trunc.i8.i1($i321);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i322} true;
  goto $bb239, $bb241;
$bb236:
  assume !(($i318 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 93} true;
  assume {:verifier.code 0} true;
  $i319 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 93} true;
  assume {:verifier.code 0} true;
  $i320 := $trunc.i8.i1($i319);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i320} true;
  goto $bb237, $bb238;
$bb237:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 108} true;
  assume {:verifier.code 0} true;
  assume ($i320 == 1);
  goto $bb235;
$bb238:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i320 == 1));
  goto $bb233;
$bb239:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 127} true;
  assume {:verifier.code 0} true;
  assume ($i322 == 1);
  goto $bb240;
$bb240:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  $i329 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  $i330 := $trunc.i8.i1($i329);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i330} true;
  goto $bb245, $bb246;
$bb241:
  assume !(($i322 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 131} true;
  assume {:verifier.code 0} true;
  $i323 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 131} true;
  assume {:verifier.code 0} true;
  $i324 := $trunc.i8.i1($i323);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i324} true;
  goto $bb242, $bb243;
$bb242:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  assume ($i324 == 1);
  goto $bb240;
$bb243:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i324 == 1));
  goto $bb233;
$bb244:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 20} true;
  assume {:verifier.code 0} true;
  $i314 := $i328;
  goto $bb230;
$bb245:
  assume ($i330 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 184} true;
  assume {:verifier.code 0} true;
  $i331 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 184} true;
  assume {:verifier.code 0} true;
  $i332 := $trunc.i8.i1($i331);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i332} true;
  goto $bb248, $bb249;
$bb246:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i330 == 1));
  goto $bb247;
$bb247:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 212} true;
  assume {:verifier.code 0} true;
  $i334 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 212} true;
  assume {:verifier.code 0} true;
  $i335 := $trunc.i8.i1($i334);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 212} true;
  assume {:verifier.code 0} true;
  $i336 := $zext.i1.i32($i335);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  $i333 := $i336;
  goto $bb250;
$bb248:
  assume ($i332 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  $i333 := 0;
  goto $bb250;
$bb249:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i332 == 1));
  goto $bb247;
$bb250:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 840, 54} true;
  assume {:verifier.code 0} true;
  $i328 := $i333;
  goto $bb244;
$bb251:
  assume ($i340 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 36} true;
  assume {:verifier.code 0} true;
  $i341 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 36} true;
  assume {:verifier.code 0} true;
  $i342 := $trunc.i8.i1($i341);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 36} true;
  assume {:verifier.code 0} true;
  $i343 := $zext.i1.i32($i342);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  $i344 := $i343;
  goto $bb253;
$bb252:
  assume !(($i340 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 55} true;
  assume {:verifier.code 0} true;
  $i345 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 55} true;
  assume {:verifier.code 0} true;
  $i346 := $trunc.i8.i1($i345);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i346} true;
  goto $bb254, $bb255;
$bb253:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  $i365 := $ne.i32($i344, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 18} true;
  assume {:verifier.code 0} true;
  $i366 := $zext.i1.i8($i365);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i366);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i366);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  $i367 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  $i368 := $trunc.i8.i1($i367);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i368} true;
  goto $bb271, $bb272;
$bb254:
  assume ($i346 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 74} true;
  assume {:verifier.code 0} true;
  $i347 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 74} true;
  assume {:verifier.code 0} true;
  $i348 := $trunc.i8.i1($i347);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i348} true;
  goto $bb257, $bb259;
$bb255:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i346 == 1));
  goto $bb256;
$bb256:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 148} true;
  assume {:verifier.code 0} true;
  $i355 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 148} true;
  assume {:verifier.code 0} true;
  $i356 := $trunc.i8.i1($i355);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 148} true;
  assume {:verifier.code 0} true;
  $i357 := $zext.i1.i32($i356);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  $i358 := $i357;
  goto $bb267;
$bb257:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 89} true;
  assume {:verifier.code 0} true;
  assume ($i348 == 1);
  goto $bb258;
$bb258:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 112} true;
  assume {:verifier.code 0} true;
  $i351 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 112} true;
  assume {:verifier.code 0} true;
  $i352 := $trunc.i8.i1($i351);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i352} true;
  goto $bb262, $bb264;
$bb259:
  assume !(($i348 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 93} true;
  assume {:verifier.code 0} true;
  $i349 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 93} true;
  assume {:verifier.code 0} true;
  $i350 := $trunc.i8.i1($i349);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i350} true;
  goto $bb260, $bb261;
$bb260:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 108} true;
  assume {:verifier.code 0} true;
  assume ($i350 == 1);
  goto $bb258;
$bb261:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i350 == 1));
  goto $bb256;
$bb262:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 127} true;
  assume {:verifier.code 0} true;
  assume ($i352 == 1);
  goto $bb263;
$bb263:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 166} true;
  assume {:verifier.code 0} true;
  $i359 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 166} true;
  assume {:verifier.code 0} true;
  $i360 := $trunc.i8.i1($i359);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 181} true;
  assume {:verifier.code 0} true;
  $i361 := 0;
  assume {:branchcond $i360} true;
  goto $bb268, $bb269;
$bb264:
  assume !(($i352 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 131} true;
  assume {:verifier.code 0} true;
  $i353 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 131} true;
  assume {:verifier.code 0} true;
  $i354 := $trunc.i8.i1($i353);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i354} true;
  goto $bb265, $bb266;
$bb265:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  assume ($i354 == 1);
  goto $bb263;
$bb266:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i354 == 1));
  goto $bb256;
$bb267:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 20} true;
  assume {:verifier.code 0} true;
  $i344 := $i358;
  goto $bb253;
$bb268:
  assume ($i360 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 184} true;
  assume {:verifier.code 0} true;
  $i362 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 184} true;
  assume {:verifier.code 0} true;
  $i363 := $trunc.i8.i1($i362);
  assume {:verifier.code 0} true;
  $i361 := $i363;
  goto $bb270;
$bb269:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i360 == 1));
  goto $bb270;
$bb270:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 166} true;
  assume {:verifier.code 0} true;
  $i364 := (if ($i361 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 841, 54} true;
  assume {:verifier.code 0} true;
  $i358 := $i364;
  goto $bb267;
$bb271:
  assume ($i368 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 36} true;
  assume {:verifier.code 0} true;
  $i369 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 36} true;
  assume {:verifier.code 0} true;
  $i370 := $trunc.i8.i1($i369);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 36} true;
  assume {:verifier.code 0} true;
  $i371 := $zext.i1.i32($i370);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  $i372 := $i371;
  goto $bb273;
$bb272:
  assume !(($i368 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 55} true;
  assume {:verifier.code 0} true;
  $i373 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 55} true;
  assume {:verifier.code 0} true;
  $i374 := $trunc.i8.i1($i373);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i374} true;
  goto $bb274, $bb275;
$bb273:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  $i395 := $ne.i32($i372, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 18} true;
  assume {:verifier.code 0} true;
  $i396 := $zext.i1.i8($i395);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd2, $i396);
  call {:cexpr "x$r_buff0_thd2"} boogie_si_record_i8($i396);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  $i397 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  $i398 := $trunc.i8.i1($i397);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i398} true;
  goto $bb294, $bb295;
$bb274:
  assume ($i374 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 74} true;
  assume {:verifier.code 0} true;
  $i375 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 74} true;
  assume {:verifier.code 0} true;
  $i376 := $trunc.i8.i1($i375);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i376} true;
  goto $bb277, $bb279;
$bb275:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i374 == 1));
  goto $bb276;
$bb276:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 148} true;
  assume {:verifier.code 0} true;
  $i383 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 148} true;
  assume {:verifier.code 0} true;
  $i384 := $trunc.i8.i1($i383);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 148} true;
  assume {:verifier.code 0} true;
  $i385 := $zext.i1.i32($i384);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  $i386 := $i385;
  goto $bb287;
$bb277:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 89} true;
  assume {:verifier.code 0} true;
  assume ($i376 == 1);
  goto $bb278;
$bb278:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 112} true;
  assume {:verifier.code 0} true;
  $i379 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 112} true;
  assume {:verifier.code 0} true;
  $i380 := $trunc.i8.i1($i379);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i380} true;
  goto $bb282, $bb284;
$bb279:
  assume !(($i376 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 93} true;
  assume {:verifier.code 0} true;
  $i377 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 93} true;
  assume {:verifier.code 0} true;
  $i378 := $trunc.i8.i1($i377);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i378} true;
  goto $bb280, $bb281;
$bb280:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 108} true;
  assume {:verifier.code 0} true;
  assume ($i378 == 1);
  goto $bb278;
$bb281:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i378 == 1));
  goto $bb276;
$bb282:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 127} true;
  assume {:verifier.code 0} true;
  assume ($i380 == 1);
  goto $bb283;
$bb283:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  $i387 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  $i388 := $trunc.i8.i1($i387);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i388} true;
  goto $bb288, $bb289;
$bb284:
  assume !(($i380 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 131} true;
  assume {:verifier.code 0} true;
  $i381 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 131} true;
  assume {:verifier.code 0} true;
  $i382 := $trunc.i8.i1($i381);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i382} true;
  goto $bb285, $bb286;
$bb285:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  assume ($i382 == 1);
  goto $bb283;
$bb286:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i382 == 1));
  goto $bb276;
$bb287:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 20} true;
  assume {:verifier.code 0} true;
  $i372 := $i386;
  goto $bb273;
$bb288:
  assume ($i388 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 184} true;
  assume {:verifier.code 0} true;
  $i389 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 184} true;
  assume {:verifier.code 0} true;
  $i390 := $trunc.i8.i1($i389);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i390} true;
  goto $bb291, $bb292;
$bb289:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i388 == 1));
  goto $bb290;
$bb290:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 212} true;
  assume {:verifier.code 0} true;
  $i392 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 212} true;
  assume {:verifier.code 0} true;
  $i393 := $trunc.i8.i1($i392);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 212} true;
  assume {:verifier.code 0} true;
  $i394 := $zext.i1.i32($i393);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  $i391 := $i394;
  goto $bb293;
$bb291:
  assume ($i390 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  $i391 := 0;
  goto $bb293;
$bb292:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i390 == 1));
  goto $bb290;
$bb293:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 842, 54} true;
  assume {:verifier.code 0} true;
  $i386 := $i391;
  goto $bb287;
$bb294:
  assume ($i398 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 36} true;
  assume {:verifier.code 0} true;
  $i399 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 36} true;
  assume {:verifier.code 0} true;
  $i400 := $trunc.i8.i1($i399);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 36} true;
  assume {:verifier.code 0} true;
  $i401 := $zext.i1.i32($i400);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  $i402 := $i401;
  goto $bb296;
$bb295:
  assume !(($i398 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 55} true;
  assume {:verifier.code 0} true;
  $i403 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 55} true;
  assume {:verifier.code 0} true;
  $i404 := $trunc.i8.i1($i403);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i404} true;
  goto $bb297, $bb298;
$bb296:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  $i423 := $ne.i32($i402, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 18} true;
  assume {:verifier.code 0} true;
  $i424 := $zext.i1.i8($i423);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd2, $i424);
  call {:cexpr "x$r_buff1_thd2"} boogie_si_record_i8($i424);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 844, 25} true;
  assume {:verifier.code 0} true;
  $i425 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 844, 23} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, __unbuffered_p1_EBX, $i425);
  call {:cexpr "__unbuffered_p1_EBX"} boogie_si_record_i32($i425);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  $i426 := $load.i8($M.0, x$flush_delayed);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  $i427 := $trunc.i8.i1($i426);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i427} true;
  goto $bb314, $bb315;
$bb297:
  assume ($i404 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 74} true;
  assume {:verifier.code 0} true;
  $i405 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 74} true;
  assume {:verifier.code 0} true;
  $i406 := $trunc.i8.i1($i405);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i406} true;
  goto $bb300, $bb302;
$bb298:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i404 == 1));
  goto $bb299;
$bb299:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 148} true;
  assume {:verifier.code 0} true;
  $i413 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 148} true;
  assume {:verifier.code 0} true;
  $i414 := $trunc.i8.i1($i413);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 148} true;
  assume {:verifier.code 0} true;
  $i415 := $zext.i1.i32($i414);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  $i416 := $i415;
  goto $bb310;
$bb300:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 89} true;
  assume {:verifier.code 0} true;
  assume ($i406 == 1);
  goto $bb301;
$bb301:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 112} true;
  assume {:verifier.code 0} true;
  $i409 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 112} true;
  assume {:verifier.code 0} true;
  $i410 := $trunc.i8.i1($i409);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i410} true;
  goto $bb305, $bb307;
$bb302:
  assume !(($i406 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 93} true;
  assume {:verifier.code 0} true;
  $i407 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 93} true;
  assume {:verifier.code 0} true;
  $i408 := $trunc.i8.i1($i407);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i408} true;
  goto $bb303, $bb304;
$bb303:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 108} true;
  assume {:verifier.code 0} true;
  assume ($i408 == 1);
  goto $bb301;
$bb304:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i408 == 1));
  goto $bb299;
$bb305:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 127} true;
  assume {:verifier.code 0} true;
  assume ($i410 == 1);
  goto $bb306;
$bb306:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 166} true;
  assume {:verifier.code 0} true;
  $i417 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 166} true;
  assume {:verifier.code 0} true;
  $i418 := $trunc.i8.i1($i417);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 181} true;
  assume {:verifier.code 0} true;
  $i419 := 0;
  assume {:branchcond $i418} true;
  goto $bb311, $bb312;
$bb307:
  assume !(($i410 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 131} true;
  assume {:verifier.code 0} true;
  $i411 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 131} true;
  assume {:verifier.code 0} true;
  $i412 := $trunc.i8.i1($i411);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i412} true;
  goto $bb308, $bb309;
$bb308:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  assume ($i412 == 1);
  goto $bb306;
$bb309:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i412 == 1));
  goto $bb299;
$bb310:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 20} true;
  assume {:verifier.code 0} true;
  $i402 := $i416;
  goto $bb296;
$bb311:
  assume ($i418 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 184} true;
  assume {:verifier.code 0} true;
  $i420 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 184} true;
  assume {:verifier.code 0} true;
  $i421 := $trunc.i8.i1($i420);
  assume {:verifier.code 0} true;
  $i419 := $i421;
  goto $bb313;
$bb312:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i418 == 1));
  goto $bb313;
$bb313:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 166} true;
  assume {:verifier.code 0} true;
  $i422 := (if ($i419 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 843, 54} true;
  assume {:verifier.code 0} true;
  $i416 := $i422;
  goto $bb310;
$bb314:
  assume ($i427 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 25} true;
  assume {:verifier.code 0} true;
  $i428 := $load.i32($M.0, x$mem_tmp);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  $i429 := $i428;
  goto $bb316;
$bb315:
  assume !(($i427 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 37} true;
  assume {:verifier.code 0} true;
  $i430 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  $i429 := $i430;
  goto $bb316;
$bb316:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 845, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i429);
  call {:cexpr "x"} boogie_si_record_i32($i429);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 846, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, 0);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8(0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 847, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 848, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  $i431 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  $i432 := $trunc.i8.i1($i431);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i432} true;
  goto $bb317, $bb318;
$bb317:
  assume ($i432 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 25} true;
  assume {:verifier.code 0} true;
  $i433 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 25} true;
  assume {:verifier.code 0} true;
  $i434 := $trunc.i8.i1($i433);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i434} true;
  goto $bb320, $bb321;
$bb318:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i432 == 1));
  goto $bb319;
$bb319:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  $i437 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  $i438 := $trunc.i8.i1($i437);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i438} true;
  goto $bb323, $bb324;
$bb320:
  assume ($i434 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 42} true;
  assume {:verifier.code 0} true;
  $i435 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  $i436 := $i435;
  goto $bb322;
$bb321:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i434 == 1));
  goto $bb319;
$bb322:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i436);
  call {:cexpr "x"} boogie_si_record_i32($i436);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  $i444 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  $i445 := $trunc.i8.i1($i444);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i445} true;
  goto $bb329, $bb330;
$bb323:
  assume ($i438 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 73} true;
  assume {:verifier.code 0} true;
  $i439 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 73} true;
  assume {:verifier.code 0} true;
  $i440 := $trunc.i8.i1($i439);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i440} true;
  goto $bb326, $bb327;
$bb324:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i438 == 1));
  goto $bb325;
$bb325:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 102} true;
  assume {:verifier.code 0} true;
  $i443 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  $i442 := $i443;
  goto $bb328;
$bb326:
  assume ($i440 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 90} true;
  assume {:verifier.code 0} true;
  $i441 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  $i442 := $i441;
  goto $bb328;
$bb327:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i440 == 1));
  goto $bb325;
$bb328:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 849, 7} true;
  assume {:verifier.code 0} true;
  $i436 := $i442;
  goto $bb322;
$bb329:
  assume ($i445 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 38} true;
  assume {:verifier.code 0} true;
  $i446 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 38} true;
  assume {:verifier.code 0} true;
  $i447 := $trunc.i8.i1($i446);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i447} true;
  goto $bb332, $bb333;
$bb330:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i445 == 1));
  goto $bb331;
$bb331:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 66} true;
  assume {:verifier.code 0} true;
  $i449 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 66} true;
  assume {:verifier.code 0} true;
  $i450 := $trunc.i8.i1($i449);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 66} true;
  assume {:verifier.code 0} true;
  $i451 := $zext.i1.i32($i450);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  $i448 := $i451;
  goto $bb334;
$bb332:
  assume ($i447 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  $i448 := 0;
  goto $bb334;
$bb333:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i447 == 1));
  goto $bb331;
$bb334:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 20} true;
  assume {:verifier.code 0} true;
  $i452 := $ne.i32($i448, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 18} true;
  assume {:verifier.code 0} true;
  $i453 := $zext.i1.i8($i452);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 850, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i453);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i453);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  $i454 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  $i455 := $trunc.i8.i1($i454);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i455} true;
  goto $bb335, $bb336;
$bb335:
  assume ($i455 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 38} true;
  assume {:verifier.code 0} true;
  $i456 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 38} true;
  assume {:verifier.code 0} true;
  $i457 := $trunc.i8.i1($i456);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i457} true;
  goto $bb338, $bb340;
$bb336:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i455 == 1));
  goto $bb337;
$bb337:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 56} true;
  assume {:verifier.code 0} true;
  $i458 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 56} true;
  assume {:verifier.code 0} true;
  $i459 := $trunc.i8.i1($i458);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i459} true;
  goto $bb341, $bb342;
$bb338:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 53} true;
  assume {:verifier.code 0} true;
  assume ($i457 == 1);
  goto $bb339;
$bb339:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  $i462 := 0;
  goto $bb346;
$bb340:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i457 == 1));
  goto $bb337;
$bb341:
  assume ($i459 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 74} true;
  assume {:verifier.code 0} true;
  $i460 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 74} true;
  assume {:verifier.code 0} true;
  $i461 := $trunc.i8.i1($i460);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i461} true;
  goto $bb344, $bb345;
$bb342:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i459 == 1));
  goto $bb343;
$bb343:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 102} true;
  assume {:verifier.code 0} true;
  $i463 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 102} true;
  assume {:verifier.code 0} true;
  $i464 := $trunc.i8.i1($i463);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 102} true;
  assume {:verifier.code 0} true;
  $i465 := $zext.i1.i32($i464);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  $i462 := $i465;
  goto $bb346;
$bb344:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  assume ($i461 == 1);
  goto $bb339;
$bb345:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i461 == 1));
  goto $bb343;
$bb346:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 20} true;
  assume {:verifier.code 0} true;
  $i466 := $ne.i32($i462, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 18} true;
  assume {:verifier.code 0} true;
  $i467 := $zext.i1.i8($i466);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 851, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i467);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i467);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  $i468 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  $i469 := $trunc.i8.i1($i468);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i469} true;
  goto $bb347, $bb348;
$bb347:
  assume ($i469 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 38} true;
  assume {:verifier.code 0} true;
  $i470 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 38} true;
  assume {:verifier.code 0} true;
  $i471 := $trunc.i8.i1($i470);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i471} true;
  goto $bb350, $bb351;
$bb348:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i469 == 1));
  goto $bb349;
$bb349:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 66} true;
  assume {:verifier.code 0} true;
  $i473 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 66} true;
  assume {:verifier.code 0} true;
  $i474 := $trunc.i8.i1($i473);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 66} true;
  assume {:verifier.code 0} true;
  $i475 := $zext.i1.i32($i474);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  $i472 := $i475;
  goto $bb352;
$bb350:
  assume ($i471 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  $i472 := 0;
  goto $bb352;
$bb351:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i471 == 1));
  goto $bb349;
$bb352:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 20} true;
  assume {:verifier.code 0} true;
  $i476 := $ne.i32($i472, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 18} true;
  assume {:verifier.code 0} true;
  $i477 := $zext.i1.i8($i476);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 852, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd2, $i477);
  call {:cexpr "x$r_buff0_thd2"} boogie_si_record_i8($i477);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  $i478 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  $i479 := $trunc.i8.i1($i478);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i479} true;
  goto $bb353, $bb354;
$bb353:
  assume ($i479 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 38} true;
  assume {:verifier.code 0} true;
  $i480 := $load.i8($M.0, x$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 38} true;
  assume {:verifier.code 0} true;
  $i481 := $trunc.i8.i1($i480);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i481} true;
  goto $bb356, $bb358;
$bb354:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i479 == 1));
  goto $bb355;
$bb355:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 56} true;
  assume {:verifier.code 0} true;
  $i482 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 56} true;
  assume {:verifier.code 0} true;
  $i483 := $trunc.i8.i1($i482);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i483} true;
  goto $bb359, $bb360;
$bb356:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 53} true;
  assume {:verifier.code 0} true;
  assume ($i481 == 1);
  goto $bb357;
$bb357:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  $i486 := 0;
  goto $bb364;
$bb358:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i481 == 1));
  goto $bb355;
$bb359:
  assume ($i483 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 74} true;
  assume {:verifier.code 0} true;
  $i484 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 74} true;
  assume {:verifier.code 0} true;
  $i485 := $trunc.i8.i1($i484);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i485} true;
  goto $bb362, $bb363;
$bb360:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i483 == 1));
  goto $bb361;
$bb361:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 102} true;
  assume {:verifier.code 0} true;
  $i487 := $load.i8($M.0, x$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 102} true;
  assume {:verifier.code 0} true;
  $i488 := $trunc.i8.i1($i487);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 102} true;
  assume {:verifier.code 0} true;
  $i489 := $zext.i1.i32($i488);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  $i486 := $i489;
  goto $bb364;
$bb362:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  assume ($i485 == 1);
  goto $bb357;
$bb363:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i485 == 1));
  goto $bb361;
$bb364:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 20} true;
  assume {:verifier.code 0} true;
  $i490 := $ne.i32($i486, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 18} true;
  assume {:verifier.code 0} true;
  $i491 := $zext.i1.i8($i490);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 853, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd2, $i491);
  call {:cexpr "x$r_buff1_thd2"} boogie_si_record_i8($i491);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  $i492 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  $i493 := $trunc.i8.i1($i492);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i493} true;
  goto $bb365, $bb366;
$bb365:
  assume ($i493 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 25} true;
  assume {:verifier.code 0} true;
  $i494 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 25} true;
  assume {:verifier.code 0} true;
  $i495 := $trunc.i8.i1($i494);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i495} true;
  goto $bb368, $bb369;
$bb366:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i493 == 1));
  goto $bb367;
$bb367:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  $i498 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  $i499 := $trunc.i8.i1($i498);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i499} true;
  goto $bb371, $bb372;
$bb368:
  assume ($i495 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 42} true;
  assume {:verifier.code 0} true;
  $i496 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  $i497 := $i496;
  goto $bb370;
$bb369:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i495 == 1));
  goto $bb367;
$bb370:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y, $i497);
  call {:cexpr "y"} boogie_si_record_i32($i497);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  $i505 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  $i506 := $trunc.i8.i1($i505);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i506} true;
  goto $bb377, $bb378;
$bb371:
  assume ($i499 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 73} true;
  assume {:verifier.code 0} true;
  $i500 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 73} true;
  assume {:verifier.code 0} true;
  $i501 := $trunc.i8.i1($i500);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i501} true;
  goto $bb374, $bb375;
$bb372:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i499 == 1));
  goto $bb373;
$bb373:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 102} true;
  assume {:verifier.code 0} true;
  $i504 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  $i503 := $i504;
  goto $bb376;
$bb374:
  assume ($i501 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 90} true;
  assume {:verifier.code 0} true;
  $i502 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  $i503 := $i502;
  goto $bb376;
$bb375:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i501 == 1));
  goto $bb373;
$bb376:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 854, 7} true;
  assume {:verifier.code 0} true;
  $i497 := $i503;
  goto $bb370;
$bb377:
  assume ($i506 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 38} true;
  assume {:verifier.code 0} true;
  $i507 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 38} true;
  assume {:verifier.code 0} true;
  $i508 := $trunc.i8.i1($i507);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i508} true;
  goto $bb380, $bb381;
$bb378:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i506 == 1));
  goto $bb379;
$bb379:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 66} true;
  assume {:verifier.code 0} true;
  $i510 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 66} true;
  assume {:verifier.code 0} true;
  $i511 := $trunc.i8.i1($i510);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 66} true;
  assume {:verifier.code 0} true;
  $i512 := $zext.i1.i32($i511);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  $i509 := $i512;
  goto $bb382;
$bb380:
  assume ($i508 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  $i509 := 0;
  goto $bb382;
$bb381:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i508 == 1));
  goto $bb379;
$bb382:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 20} true;
  assume {:verifier.code 0} true;
  $i513 := $ne.i32($i509, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 18} true;
  assume {:verifier.code 0} true;
  $i514 := $zext.i1.i8($i513);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 855, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff0_used, $i514);
  call {:cexpr "y$w_buff0_used"} boogie_si_record_i8($i514);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  $i515 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  $i516 := $trunc.i8.i1($i515);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i516} true;
  goto $bb383, $bb384;
$bb383:
  assume ($i516 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 38} true;
  assume {:verifier.code 0} true;
  $i517 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 38} true;
  assume {:verifier.code 0} true;
  $i518 := $trunc.i8.i1($i517);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i518} true;
  goto $bb386, $bb388;
$bb384:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i516 == 1));
  goto $bb385;
$bb385:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 56} true;
  assume {:verifier.code 0} true;
  $i519 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 56} true;
  assume {:verifier.code 0} true;
  $i520 := $trunc.i8.i1($i519);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i520} true;
  goto $bb389, $bb390;
$bb386:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 53} true;
  assume {:verifier.code 0} true;
  assume ($i518 == 1);
  goto $bb387;
$bb387:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  $i523 := 0;
  goto $bb394;
$bb388:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i518 == 1));
  goto $bb385;
$bb389:
  assume ($i520 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 74} true;
  assume {:verifier.code 0} true;
  $i521 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 74} true;
  assume {:verifier.code 0} true;
  $i522 := $trunc.i8.i1($i521);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i522} true;
  goto $bb392, $bb393;
$bb390:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i520 == 1));
  goto $bb391;
$bb391:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 102} true;
  assume {:verifier.code 0} true;
  $i524 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 102} true;
  assume {:verifier.code 0} true;
  $i525 := $trunc.i8.i1($i524);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 102} true;
  assume {:verifier.code 0} true;
  $i526 := $zext.i1.i32($i525);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  $i523 := $i526;
  goto $bb394;
$bb392:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  assume ($i522 == 1);
  goto $bb387;
$bb393:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i522 == 1));
  goto $bb391;
$bb394:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 20} true;
  assume {:verifier.code 0} true;
  $i527 := $ne.i32($i523, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 18} true;
  assume {:verifier.code 0} true;
  $i528 := $zext.i1.i8($i527);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 856, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff1_used, $i528);
  call {:cexpr "y$w_buff1_used"} boogie_si_record_i8($i528);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  $i529 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  $i530 := $trunc.i8.i1($i529);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i530} true;
  goto $bb395, $bb396;
$bb395:
  assume ($i530 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 38} true;
  assume {:verifier.code 0} true;
  $i531 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 38} true;
  assume {:verifier.code 0} true;
  $i532 := $trunc.i8.i1($i531);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i532} true;
  goto $bb398, $bb399;
$bb396:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i530 == 1));
  goto $bb397;
$bb397:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 66} true;
  assume {:verifier.code 0} true;
  $i534 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 66} true;
  assume {:verifier.code 0} true;
  $i535 := $trunc.i8.i1($i534);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 66} true;
  assume {:verifier.code 0} true;
  $i536 := $zext.i1.i32($i535);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  $i533 := $i536;
  goto $bb400;
$bb398:
  assume ($i532 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  $i533 := 0;
  goto $bb400;
$bb399:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i532 == 1));
  goto $bb397;
$bb400:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 20} true;
  assume {:verifier.code 0} true;
  $i537 := $ne.i32($i533, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 18} true;
  assume {:verifier.code 0} true;
  $i538 := $zext.i1.i8($i537);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 857, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff0_thd2, $i538);
  call {:cexpr "y$r_buff0_thd2"} boogie_si_record_i8($i538);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  $i539 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  $i540 := $trunc.i8.i1($i539);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i540} true;
  goto $bb401, $bb402;
$bb401:
  assume ($i540 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 38} true;
  assume {:verifier.code 0} true;
  $i541 := $load.i8($M.0, y$r_buff0_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 38} true;
  assume {:verifier.code 0} true;
  $i542 := $trunc.i8.i1($i541);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i542} true;
  goto $bb404, $bb406;
$bb402:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i540 == 1));
  goto $bb403;
$bb403:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 56} true;
  assume {:verifier.code 0} true;
  $i543 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 56} true;
  assume {:verifier.code 0} true;
  $i544 := $trunc.i8.i1($i543);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i544} true;
  goto $bb407, $bb408;
$bb404:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 53} true;
  assume {:verifier.code 0} true;
  assume ($i542 == 1);
  goto $bb405;
$bb405:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  $i547 := 0;
  goto $bb412;
$bb406:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i542 == 1));
  goto $bb403;
$bb407:
  assume ($i544 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 74} true;
  assume {:verifier.code 0} true;
  $i545 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 74} true;
  assume {:verifier.code 0} true;
  $i546 := $trunc.i8.i1($i545);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i546} true;
  goto $bb410, $bb411;
$bb408:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i544 == 1));
  goto $bb409;
$bb409:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 102} true;
  assume {:verifier.code 0} true;
  $i548 := $load.i8($M.0, y$r_buff1_thd2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 102} true;
  assume {:verifier.code 0} true;
  $i549 := $trunc.i8.i1($i548);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 102} true;
  assume {:verifier.code 0} true;
  $i550 := $zext.i1.i32($i549);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  $i547 := $i550;
  goto $bb412;
$bb410:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  assume ($i546 == 1);
  goto $bb405;
$bb411:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i546 == 1));
  goto $bb409;
$bb412:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 20} true;
  assume {:verifier.code 0} true;
  $i551 := $ne.i32($i547, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 18} true;
  assume {:verifier.code 0} true;
  $i552 := $zext.i1.i8($i551);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 858, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd2, $i552);
  call {:cexpr "y$r_buff1_thd2"} boogie_si_record_i8($i552);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 859, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 860, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 861, 22} true;
  assume {:verifier.code 0} true;
  $i553 := $load.i32($M.0, __unbuffered_cnt);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 861, 39} true;
  assume {:verifier.code 0} true;
  $i554 := $add.i32($i553, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 861, 20} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, __unbuffered_cnt, $i554);
  call {:cexpr "__unbuffered_cnt"} boogie_si_record_i32($i554);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 862, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 863, 3} true;
  assume {:verifier.code 0} true;
  $r := $0.ref;
  $exn := false;
  return;
}
const main: ref;
axiom (main == $sub.ref(0, 54485));
procedure {:entrypoint} main()
  returns ($r: i32)
{
  var $p0: ref;
  var $p1: ref;
  var $i2: i32;
  var $i3: i32;
  var $i4: i32;
  var $i5: i1;
  var $i6: i8;
  var $i7: i8;
  var $i8: i1;
  var $i9: i32;
  var $i10: i8;
  var $i11: i1;
  var $i12: i8;
  var $i13: i1;
  var $i14: i32;
  var $i16: i8;
  var $i17: i1;
  var $i18: i8;
  var $i19: i1;
  var $i20: i32;
  var $i22: i32;
  var $i21: i32;
  var $i15: i32;
  var $i23: i8;
  var $i24: i1;
  var $i25: i8;
  var $i26: i1;
  var $i28: i8;
  var $i29: i1;
  var $i30: i32;
  var $i27: i32;
  var $i31: i1;
  var $i32: i8;
  var $i33: i8;
  var $i34: i1;
  var $i35: i8;
  var $i36: i1;
  var $i37: i8;
  var $i38: i1;
  var $i39: i8;
  var $i40: i1;
  var $i42: i8;
  var $i43: i1;
  var $i44: i32;
  var $i41: i32;
  var $i45: i1;
  var $i46: i8;
  var $i47: i8;
  var $i48: i1;
  var $i49: i8;
  var $i50: i1;
  var $i52: i8;
  var $i53: i1;
  var $i54: i32;
  var $i51: i32;
  var $i55: i1;
  var $i56: i8;
  var $i57: i8;
  var $i58: i1;
  var $i59: i8;
  var $i60: i1;
  var $i61: i8;
  var $i62: i1;
  var $i63: i8;
  var $i64: i1;
  var $i66: i8;
  var $i67: i1;
  var $i68: i32;
  var $i65: i32;
  var $i69: i1;
  var $i70: i8;
  var $i71: i8;
  var $i72: i1;
  var $i73: i8;
  var $i74: i1;
  var $i75: i32;
  var $i77: i8;
  var $i78: i1;
  var $i79: i8;
  var $i80: i1;
  var $i81: i32;
  var $i83: i32;
  var $i82: i32;
  var $i76: i32;
  var $i84: i8;
  var $i85: i1;
  var $i86: i8;
  var $i87: i1;
  var $i89: i8;
  var $i90: i1;
  var $i91: i32;
  var $i88: i32;
  var $i92: i1;
  var $i93: i8;
  var $i94: i8;
  var $i95: i1;
  var $i96: i8;
  var $i97: i1;
  var $i98: i8;
  var $i99: i1;
  var $i100: i8;
  var $i101: i1;
  var $i103: i8;
  var $i104: i1;
  var $i105: i32;
  var $i102: i32;
  var $i106: i1;
  var $i107: i8;
  var $i108: i8;
  var $i109: i1;
  var $i110: i8;
  var $i111: i1;
  var $i113: i8;
  var $i114: i1;
  var $i115: i32;
  var $i112: i32;
  var $i116: i1;
  var $i117: i8;
  var $i118: i8;
  var $i119: i1;
  var $i120: i8;
  var $i121: i1;
  var $i122: i8;
  var $i123: i1;
  var $i124: i8;
  var $i125: i1;
  var $i127: i8;
  var $i128: i1;
  var $i129: i32;
  var $i126: i32;
  var $i130: i1;
  var $i131: i8;
  var $i132: i1;
  var $i133: i8;
  var $i134: i1;
  var $i135: i8;
  var $i136: i8;
  var $i137: i1;
  var $i138: i8;
  var $i139: i32;
  var $i140: i8;
  var $i141: i1;
  var $i142: i8;
  var $i143: i1;
  var $i144: i8;
  var $i145: i1;
  var $i146: i8;
  var $i147: i1;
  var $i148: i8;
  var $i149: i1;
  var $i150: i32;
  var $i152: i8;
  var $i153: i1;
  var $i154: i8;
  var $i155: i1;
  var $i156: i32;
  var $i158: i32;
  var $i157: i32;
  var $i151: i32;
  var $i159: i8;
  var $i160: i1;
  var $i161: i32;
  var $i163: i8;
  var $i164: i1;
  var $i165: i8;
  var $i166: i1;
  var $i167: i8;
  var $i168: i1;
  var $i169: i8;
  var $i170: i1;
  var $i171: i8;
  var $i172: i1;
  var $i173: i32;
  var $i175: i8;
  var $i176: i1;
  var $i177: i8;
  var $i178: i1;
  var $i179: i32;
  var $i181: i32;
  var $i180: i32;
  var $i174: i32;
  var $i162: i32;
  var $i182: i8;
  var $i183: i1;
  var $i184: i32;
  var $i186: i8;
  var $i187: i1;
  var $i188: i8;
  var $i189: i1;
  var $i190: i8;
  var $i191: i1;
  var $i192: i8;
  var $i193: i1;
  var $i194: i8;
  var $i195: i1;
  var $i196: i32;
  var $i198: i8;
  var $i199: i1;
  var $i200: i8;
  var $i201: i1;
  var $i202: i32;
  var $i204: i32;
  var $i203: i32;
  var $i197: i32;
  var $i185: i32;
  var $i205: i8;
  var $i206: i1;
  var $i207: i8;
  var $i208: i1;
  var $i209: i32;
  var $i211: i8;
  var $i212: i1;
  var $i213: i8;
  var $i214: i1;
  var $i215: i8;
  var $i216: i1;
  var $i217: i8;
  var $i218: i1;
  var $i219: i8;
  var $i220: i1;
  var $i221: i8;
  var $i222: i1;
  var $i223: i32;
  var $i225: i8;
  var $i226: i1;
  var $i227: i8;
  var $i228: i1;
  var $i230: i8;
  var $i231: i1;
  var $i232: i32;
  var $i229: i32;
  var $i224: i32;
  var $i210: i32;
  var $i233: i1;
  var $i234: i8;
  var $i235: i8;
  var $i236: i1;
  var $i237: i8;
  var $i238: i1;
  var $i239: i32;
  var $i241: i8;
  var $i242: i1;
  var $i243: i8;
  var $i244: i1;
  var $i245: i8;
  var $i246: i1;
  var $i247: i8;
  var $i248: i1;
  var $i249: i8;
  var $i250: i1;
  var $i251: i8;
  var $i252: i1;
  var $i253: i32;
  var $i255: i8;
  var $i256: i1;
  var $i258: i8;
  var $i259: i1;
  var $i257: i1;
  var $i260: i32;
  var $i254: i32;
  var $i240: i32;
  var $i261: i1;
  var $i262: i8;
  var $i263: i8;
  var $i264: i1;
  var $i265: i8;
  var $i266: i1;
  var $i267: i32;
  var $i269: i8;
  var $i270: i1;
  var $i271: i8;
  var $i272: i1;
  var $i273: i8;
  var $i274: i1;
  var $i275: i8;
  var $i276: i1;
  var $i277: i8;
  var $i278: i1;
  var $i279: i8;
  var $i280: i1;
  var $i281: i32;
  var $i283: i8;
  var $i284: i1;
  var $i285: i8;
  var $i286: i1;
  var $i288: i8;
  var $i289: i1;
  var $i290: i32;
  var $i287: i32;
  var $i282: i32;
  var $i268: i32;
  var $i291: i1;
  var $i292: i8;
  var $i293: i8;
  var $i294: i1;
  var $i295: i8;
  var $i296: i1;
  var $i297: i32;
  var $i299: i8;
  var $i300: i1;
  var $i301: i8;
  var $i302: i1;
  var $i303: i8;
  var $i304: i1;
  var $i305: i8;
  var $i306: i1;
  var $i307: i8;
  var $i308: i1;
  var $i309: i8;
  var $i310: i1;
  var $i311: i32;
  var $i313: i8;
  var $i314: i1;
  var $i316: i8;
  var $i317: i1;
  var $i315: i1;
  var $i318: i32;
  var $i312: i32;
  var $i298: i32;
  var $i319: i1;
  var $i320: i8;
  var $i321: i32;
  var $i322: i1;
  var $i324: i32;
  var $i325: i1;
  var $i326: i32;
  var $i327: i1;
  var $i328: i32;
  var $i329: i1;
  var $i323: i1;
  var $i330: i1;
  var $i331: i8;
  var $i332: i8;
  var $i333: i1;
  var $i334: i32;
  var $i336: i32;
  var $i335: i32;
  var $i337: i8;
  var $i338: i1;
  var $i339: i32;
$bb0:
  call $initialize();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 877, 3} true;
  assume {:verifier.code 0} true;
  call {:cexpr "smack:entry:main"} boogie_si_record_ref(main);
  assume {:verifier.code 0} true;
  call $p0 := $alloc($mul.ref(8, $zext.i32.i64(1)));
  assume {:verifier.code 0} true;
  call $p1 := $alloc($mul.ref(8, $zext.i32.i64(1)));
  assume true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 877, 3} true;
  assume {:verifier.code 0} true;
  call $i2 := pthread_create($p0, $0.ref, P0, $0.ref);
  assume true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 879, 3} true;
  assume {:verifier.code 0} true;
  call $i3 := pthread_create($p1, $0.ref, P1, $0.ref);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 880, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 881, 21} true;
  assume {:verifier.code 0} true;
  $i4 := $load.i32($M.0, __unbuffered_cnt);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 881, 38} true;
  assume {:verifier.code 0} true;
  $i5 := $eq.i32($i4, 2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 881, 19} true;
  assume {:verifier.code 0} true;
  $i6 := $zext.i1.i8($i5);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 881, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, main$tmp_guard0, $i6);
  call {:cexpr "main$tmp_guard0"} boogie_si_record_i8($i6);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 882, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 883, 23} true;
  assume {:verifier.code 0} true;
  $i7 := $load.i8($M.0, main$tmp_guard0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 883, 23} true;
  assume {:verifier.code 0} true;
  $i8 := $trunc.i8.i1($i7);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 883, 23} true;
  assume {:verifier.code 0} true;
  $i9 := $zext.i1.i32($i8);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 883, 3} true;
  assume {:verifier.code 0} true;
  call assume_abort_if_not($i9);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 884, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  $i10 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  $i11 := $trunc.i8.i1($i10);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i11} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i11 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 25} true;
  assume {:verifier.code 0} true;
  $i12 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 25} true;
  assume {:verifier.code 0} true;
  $i13 := $trunc.i8.i1($i12);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i13} true;
  goto $bb4, $bb5;
$bb2:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i11 == 1));
  goto $bb3;
$bb3:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  $i16 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  $i17 := $trunc.i8.i1($i16);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i17} true;
  goto $bb7, $bb8;
$bb4:
  assume ($i13 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 42} true;
  assume {:verifier.code 0} true;
  $i14 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  $i15 := $i14;
  goto $bb6;
$bb5:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i13 == 1));
  goto $bb3;
$bb6:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i15);
  call {:cexpr "x"} boogie_si_record_i32($i15);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  $i23 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  $i24 := $trunc.i8.i1($i23);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i24} true;
  goto $bb13, $bb14;
$bb7:
  assume ($i17 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 73} true;
  assume {:verifier.code 0} true;
  $i18 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 73} true;
  assume {:verifier.code 0} true;
  $i19 := $trunc.i8.i1($i18);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i19} true;
  goto $bb10, $bb11;
$bb8:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i17 == 1));
  goto $bb9;
$bb9:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 102} true;
  assume {:verifier.code 0} true;
  $i22 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  $i21 := $i22;
  goto $bb12;
$bb10:
  assume ($i19 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 90} true;
  assume {:verifier.code 0} true;
  $i20 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  $i21 := $i20;
  goto $bb12;
$bb11:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i19 == 1));
  goto $bb9;
$bb12:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 885, 7} true;
  assume {:verifier.code 0} true;
  $i15 := $i21;
  goto $bb6;
$bb13:
  assume ($i24 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 38} true;
  assume {:verifier.code 0} true;
  $i25 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 38} true;
  assume {:verifier.code 0} true;
  $i26 := $trunc.i8.i1($i25);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i26} true;
  goto $bb16, $bb17;
$bb14:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i24 == 1));
  goto $bb15;
$bb15:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 66} true;
  assume {:verifier.code 0} true;
  $i28 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 66} true;
  assume {:verifier.code 0} true;
  $i29 := $trunc.i8.i1($i28);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 66} true;
  assume {:verifier.code 0} true;
  $i30 := $zext.i1.i32($i29);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  $i27 := $i30;
  goto $bb18;
$bb16:
  assume ($i26 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  $i27 := 0;
  goto $bb18;
$bb17:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i26 == 1));
  goto $bb15;
$bb18:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 20} true;
  assume {:verifier.code 0} true;
  $i31 := $ne.i32($i27, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 18} true;
  assume {:verifier.code 0} true;
  $i32 := $zext.i1.i8($i31);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 886, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i32);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i32);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  $i33 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  $i34 := $trunc.i8.i1($i33);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i34} true;
  goto $bb19, $bb20;
$bb19:
  assume ($i34 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 38} true;
  assume {:verifier.code 0} true;
  $i35 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 38} true;
  assume {:verifier.code 0} true;
  $i36 := $trunc.i8.i1($i35);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i36} true;
  goto $bb22, $bb24;
$bb20:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i34 == 1));
  goto $bb21;
$bb21:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 56} true;
  assume {:verifier.code 0} true;
  $i37 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 56} true;
  assume {:verifier.code 0} true;
  $i38 := $trunc.i8.i1($i37);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i38} true;
  goto $bb25, $bb26;
$bb22:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 53} true;
  assume {:verifier.code 0} true;
  assume ($i36 == 1);
  goto $bb23;
$bb23:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  $i41 := 0;
  goto $bb30;
$bb24:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i36 == 1));
  goto $bb21;
$bb25:
  assume ($i38 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 74} true;
  assume {:verifier.code 0} true;
  $i39 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 74} true;
  assume {:verifier.code 0} true;
  $i40 := $trunc.i8.i1($i39);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i40} true;
  goto $bb28, $bb29;
$bb26:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i38 == 1));
  goto $bb27;
$bb27:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 102} true;
  assume {:verifier.code 0} true;
  $i42 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 102} true;
  assume {:verifier.code 0} true;
  $i43 := $trunc.i8.i1($i42);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 102} true;
  assume {:verifier.code 0} true;
  $i44 := $zext.i1.i32($i43);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  $i41 := $i44;
  goto $bb30;
$bb28:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  assume ($i40 == 1);
  goto $bb23;
$bb29:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i40 == 1));
  goto $bb27;
$bb30:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 20} true;
  assume {:verifier.code 0} true;
  $i45 := $ne.i32($i41, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 18} true;
  assume {:verifier.code 0} true;
  $i46 := $zext.i1.i8($i45);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 887, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i46);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i46);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  $i47 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  $i48 := $trunc.i8.i1($i47);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i48} true;
  goto $bb31, $bb32;
$bb31:
  assume ($i48 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 38} true;
  assume {:verifier.code 0} true;
  $i49 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 38} true;
  assume {:verifier.code 0} true;
  $i50 := $trunc.i8.i1($i49);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i50} true;
  goto $bb34, $bb35;
$bb32:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i48 == 1));
  goto $bb33;
$bb33:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 66} true;
  assume {:verifier.code 0} true;
  $i52 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 66} true;
  assume {:verifier.code 0} true;
  $i53 := $trunc.i8.i1($i52);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 66} true;
  assume {:verifier.code 0} true;
  $i54 := $zext.i1.i32($i53);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  $i51 := $i54;
  goto $bb36;
$bb34:
  assume ($i50 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  $i51 := 0;
  goto $bb36;
$bb35:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i50 == 1));
  goto $bb33;
$bb36:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 20} true;
  assume {:verifier.code 0} true;
  $i55 := $ne.i32($i51, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 18} true;
  assume {:verifier.code 0} true;
  $i56 := $zext.i1.i8($i55);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 888, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd0, $i56);
  call {:cexpr "x$r_buff0_thd0"} boogie_si_record_i8($i56);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  $i57 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  $i58 := $trunc.i8.i1($i57);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i58} true;
  goto $bb37, $bb38;
$bb37:
  assume ($i58 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 38} true;
  assume {:verifier.code 0} true;
  $i59 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 38} true;
  assume {:verifier.code 0} true;
  $i60 := $trunc.i8.i1($i59);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i60} true;
  goto $bb40, $bb42;
$bb38:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i58 == 1));
  goto $bb39;
$bb39:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 56} true;
  assume {:verifier.code 0} true;
  $i61 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 56} true;
  assume {:verifier.code 0} true;
  $i62 := $trunc.i8.i1($i61);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i62} true;
  goto $bb43, $bb44;
$bb40:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 53} true;
  assume {:verifier.code 0} true;
  assume ($i60 == 1);
  goto $bb41;
$bb41:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  $i65 := 0;
  goto $bb48;
$bb42:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i60 == 1));
  goto $bb39;
$bb43:
  assume ($i62 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 74} true;
  assume {:verifier.code 0} true;
  $i63 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 74} true;
  assume {:verifier.code 0} true;
  $i64 := $trunc.i8.i1($i63);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i64} true;
  goto $bb46, $bb47;
$bb44:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i62 == 1));
  goto $bb45;
$bb45:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 102} true;
  assume {:verifier.code 0} true;
  $i66 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 102} true;
  assume {:verifier.code 0} true;
  $i67 := $trunc.i8.i1($i66);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 102} true;
  assume {:verifier.code 0} true;
  $i68 := $zext.i1.i32($i67);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  $i65 := $i68;
  goto $bb48;
$bb46:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  assume ($i64 == 1);
  goto $bb41;
$bb47:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i64 == 1));
  goto $bb45;
$bb48:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 20} true;
  assume {:verifier.code 0} true;
  $i69 := $ne.i32($i65, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 18} true;
  assume {:verifier.code 0} true;
  $i70 := $zext.i1.i8($i69);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 889, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd0, $i70);
  call {:cexpr "x$r_buff1_thd0"} boogie_si_record_i8($i70);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  $i71 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  $i72 := $trunc.i8.i1($i71);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 22} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i72} true;
  goto $bb49, $bb50;
$bb49:
  assume ($i72 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 25} true;
  assume {:verifier.code 0} true;
  $i73 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 25} true;
  assume {:verifier.code 0} true;
  $i74 := $trunc.i8.i1($i73);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i74} true;
  goto $bb52, $bb53;
$bb50:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 22} true;
  assume {:verifier.code 0} true;
  assume !(($i72 == 1));
  goto $bb51;
$bb51:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  $i77 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  $i78 := $trunc.i8.i1($i77);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i78} true;
  goto $bb55, $bb56;
$bb52:
  assume ($i74 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 42} true;
  assume {:verifier.code 0} true;
  $i75 := $load.i32($M.0, y$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  $i76 := $i75;
  goto $bb54;
$bb53:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i74 == 1));
  goto $bb51;
$bb54:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, y, $i76);
  call {:cexpr "y"} boogie_si_record_i32($i76);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  $i84 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  $i85 := $trunc.i8.i1($i84);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i85} true;
  goto $bb61, $bb62;
$bb55:
  assume ($i78 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 73} true;
  assume {:verifier.code 0} true;
  $i79 := $load.i8($M.0, y$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 73} true;
  assume {:verifier.code 0} true;
  $i80 := $trunc.i8.i1($i79);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i80} true;
  goto $bb58, $bb59;
$bb56:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i78 == 1));
  goto $bb57;
$bb57:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 102} true;
  assume {:verifier.code 0} true;
  $i83 := $load.i32($M.0, y);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  $i82 := $i83;
  goto $bb60;
$bb58:
  assume ($i80 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 90} true;
  assume {:verifier.code 0} true;
  $i81 := $load.i32($M.0, y$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  $i82 := $i81;
  goto $bb60;
$bb59:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  assume !(($i80 == 1));
  goto $bb57;
$bb60:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 55} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 890, 7} true;
  assume {:verifier.code 0} true;
  $i76 := $i82;
  goto $bb54;
$bb61:
  assume ($i85 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 38} true;
  assume {:verifier.code 0} true;
  $i86 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 38} true;
  assume {:verifier.code 0} true;
  $i87 := $trunc.i8.i1($i86);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i87} true;
  goto $bb64, $bb65;
$bb62:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i85 == 1));
  goto $bb63;
$bb63:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 66} true;
  assume {:verifier.code 0} true;
  $i89 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 66} true;
  assume {:verifier.code 0} true;
  $i90 := $trunc.i8.i1($i89);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 66} true;
  assume {:verifier.code 0} true;
  $i91 := $zext.i1.i32($i90);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  $i88 := $i91;
  goto $bb66;
$bb64:
  assume ($i87 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  $i88 := 0;
  goto $bb66;
$bb65:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i87 == 1));
  goto $bb63;
$bb66:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 20} true;
  assume {:verifier.code 0} true;
  $i92 := $ne.i32($i88, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 18} true;
  assume {:verifier.code 0} true;
  $i93 := $zext.i1.i8($i92);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 891, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff0_used, $i93);
  call {:cexpr "y$w_buff0_used"} boogie_si_record_i8($i93);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  $i94 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  $i95 := $trunc.i8.i1($i94);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i95} true;
  goto $bb67, $bb68;
$bb67:
  assume ($i95 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 38} true;
  assume {:verifier.code 0} true;
  $i96 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 38} true;
  assume {:verifier.code 0} true;
  $i97 := $trunc.i8.i1($i96);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i97} true;
  goto $bb70, $bb72;
$bb68:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i95 == 1));
  goto $bb69;
$bb69:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 56} true;
  assume {:verifier.code 0} true;
  $i98 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 56} true;
  assume {:verifier.code 0} true;
  $i99 := $trunc.i8.i1($i98);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i99} true;
  goto $bb73, $bb74;
$bb70:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 53} true;
  assume {:verifier.code 0} true;
  assume ($i97 == 1);
  goto $bb71;
$bb71:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  $i102 := 0;
  goto $bb78;
$bb72:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i97 == 1));
  goto $bb69;
$bb73:
  assume ($i99 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 74} true;
  assume {:verifier.code 0} true;
  $i100 := $load.i8($M.0, y$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 74} true;
  assume {:verifier.code 0} true;
  $i101 := $trunc.i8.i1($i100);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i101} true;
  goto $bb76, $bb77;
$bb74:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i99 == 1));
  goto $bb75;
$bb75:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 102} true;
  assume {:verifier.code 0} true;
  $i103 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 102} true;
  assume {:verifier.code 0} true;
  $i104 := $trunc.i8.i1($i103);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 102} true;
  assume {:verifier.code 0} true;
  $i105 := $zext.i1.i32($i104);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  $i102 := $i105;
  goto $bb78;
$bb76:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  assume ($i101 == 1);
  goto $bb71;
$bb77:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i101 == 1));
  goto $bb75;
$bb78:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 20} true;
  assume {:verifier.code 0} true;
  $i106 := $ne.i32($i102, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 18} true;
  assume {:verifier.code 0} true;
  $i107 := $zext.i1.i8($i106);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 892, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$w_buff1_used, $i107);
  call {:cexpr "y$w_buff1_used"} boogie_si_record_i8($i107);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  $i108 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  $i109 := $trunc.i8.i1($i108);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i109} true;
  goto $bb79, $bb80;
$bb79:
  assume ($i109 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 38} true;
  assume {:verifier.code 0} true;
  $i110 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 38} true;
  assume {:verifier.code 0} true;
  $i111 := $trunc.i8.i1($i110);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i111} true;
  goto $bb82, $bb83;
$bb80:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i109 == 1));
  goto $bb81;
$bb81:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 66} true;
  assume {:verifier.code 0} true;
  $i113 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 66} true;
  assume {:verifier.code 0} true;
  $i114 := $trunc.i8.i1($i113);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 66} true;
  assume {:verifier.code 0} true;
  $i115 := $zext.i1.i32($i114);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  $i112 := $i115;
  goto $bb84;
$bb82:
  assume ($i111 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  $i112 := 0;
  goto $bb84;
$bb83:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i111 == 1));
  goto $bb81;
$bb84:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 20} true;
  assume {:verifier.code 0} true;
  $i116 := $ne.i32($i112, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 18} true;
  assume {:verifier.code 0} true;
  $i117 := $zext.i1.i8($i116);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 893, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff0_thd0, $i117);
  call {:cexpr "y$r_buff0_thd0"} boogie_si_record_i8($i117);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  $i118 := $load.i8($M.0, y$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  $i119 := $trunc.i8.i1($i118);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 35} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i119} true;
  goto $bb85, $bb86;
$bb85:
  assume ($i119 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 38} true;
  assume {:verifier.code 0} true;
  $i120 := $load.i8($M.0, y$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 38} true;
  assume {:verifier.code 0} true;
  $i121 := $trunc.i8.i1($i120);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 53} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i121} true;
  goto $bb88, $bb90;
$bb86:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 35} true;
  assume {:verifier.code 0} true;
  assume !(($i119 == 1));
  goto $bb87;
$bb87:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 56} true;
  assume {:verifier.code 0} true;
  $i122 := $load.i8($M.0, y$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 56} true;
  assume {:verifier.code 0} true;
  $i123 := $trunc.i8.i1($i122);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 71} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i123} true;
  goto $bb91, $bb92;
$bb88:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 53} true;
  assume {:verifier.code 0} true;
  assume ($i121 == 1);
  goto $bb89;
$bb89:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  $i126 := 0;
  goto $bb96;
$bb90:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 53} true;
  assume {:verifier.code 0} true;
  assume !(($i121 == 1));
  goto $bb87;
$bb91:
  assume ($i123 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 74} true;
  assume {:verifier.code 0} true;
  $i124 := $load.i8($M.0, y$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 74} true;
  assume {:verifier.code 0} true;
  $i125 := $trunc.i8.i1($i124);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i125} true;
  goto $bb94, $bb95;
$bb92:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 71} true;
  assume {:verifier.code 0} true;
  assume !(($i123 == 1));
  goto $bb93;
$bb93:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 102} true;
  assume {:verifier.code 0} true;
  $i127 := $load.i8($M.0, y$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 102} true;
  assume {:verifier.code 0} true;
  $i128 := $trunc.i8.i1($i127);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 102} true;
  assume {:verifier.code 0} true;
  $i129 := $zext.i1.i32($i128);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  $i126 := $i129;
  goto $bb96;
$bb94:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  assume ($i125 == 1);
  goto $bb89;
$bb95:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  assume !(($i125 == 1));
  goto $bb93;
$bb96:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 20} true;
  assume {:verifier.code 0} true;
  $i130 := $ne.i32($i126, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 18} true;
  assume {:verifier.code 0} true;
  $i131 := $zext.i1.i8($i130);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 894, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, y$r_buff1_thd0, $i131);
  call {:cexpr "y$r_buff1_thd0"} boogie_si_record_i8($i131);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 895, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 896, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_begin();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 897, 19} true;
  assume {:verifier.code 1} true;
  call $i132 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i132);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 897, 17} true;
  assume {:verifier.code 0} true;
  $i133 := $zext.i1.i8($i132);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 897, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice0, $i133);
  call {:cexpr "weak$$choice0"} boogie_si_record_i8($i133);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 898, 19} true;
  assume {:verifier.code 1} true;
  call $i134 := __VERIFIER_nondet_bool();
  call {:cexpr "smack:ext:__VERIFIER_nondet_bool"} boogie_si_record_i1($i134);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 898, 17} true;
  assume {:verifier.code 0} true;
  $i135 := $zext.i1.i8($i134);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 898, 17} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, weak$$choice2, $i135);
  call {:cexpr "weak$$choice2"} boogie_si_record_i8($i135);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 899, 21} true;
  assume {:verifier.code 0} true;
  $i136 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 899, 21} true;
  assume {:verifier.code 0} true;
  $i137 := $trunc.i8.i1($i136);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 899, 19} true;
  assume {:verifier.code 0} true;
  $i138 := $zext.i1.i8($i137);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 899, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, $i138);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8($i138);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 900, 15} true;
  assume {:verifier.code 0} true;
  $i139 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 900, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$mem_tmp, $i139);
  call {:cexpr "x$mem_tmp"} boogie_si_record_i32($i139);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 8} true;
  assume {:verifier.code 0} true;
  $i140 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 8} true;
  assume {:verifier.code 0} true;
  $i141 := $trunc.i8.i1($i140);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 23} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i141} true;
  goto $bb97, $bb98;
$bb97:
  assume ($i141 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 27} true;
  assume {:verifier.code 0} true;
  $i142 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 27} true;
  assume {:verifier.code 0} true;
  $i143 := $trunc.i8.i1($i142);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 42} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i143} true;
  goto $bb100, $bb102;
$bb98:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 23} true;
  assume {:verifier.code 0} true;
  assume !(($i141 == 1));
  goto $bb99;
$bb99:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 101} true;
  assume {:verifier.code 0} true;
  $i150 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  $i151 := $i150;
  goto $bb110;
$bb100:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 42} true;
  assume {:verifier.code 0} true;
  assume ($i143 == 1);
  goto $bb101;
$bb101:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 65} true;
  assume {:verifier.code 0} true;
  $i146 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 65} true;
  assume {:verifier.code 0} true;
  $i147 := $trunc.i8.i1($i146);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 80} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i147} true;
  goto $bb105, $bb107;
$bb102:
  assume !(($i143 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 46} true;
  assume {:verifier.code 0} true;
  $i144 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 46} true;
  assume {:verifier.code 0} true;
  $i145 := $trunc.i8.i1($i144);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 61} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i145} true;
  goto $bb103, $bb104;
$bb103:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 61} true;
  assume {:verifier.code 0} true;
  assume ($i145 == 1);
  goto $bb101;
$bb104:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 61} true;
  assume {:verifier.code 0} true;
  assume !(($i145 == 1));
  goto $bb99;
$bb105:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 80} true;
  assume {:verifier.code 0} true;
  assume ($i147 == 1);
  goto $bb106;
$bb106:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  $i152 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  $i153 := $trunc.i8.i1($i152);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 121} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i153} true;
  goto $bb111, $bb112;
$bb107:
  assume !(($i147 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 84} true;
  assume {:verifier.code 0} true;
  $i148 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 84} true;
  assume {:verifier.code 0} true;
  $i149 := $trunc.i8.i1($i148);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i149} true;
  goto $bb108, $bb109;
$bb108:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  assume ($i149 == 1);
  goto $bb106;
$bb109:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  assume !(($i149 == 1));
  goto $bb99;
$bb110:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i151);
  call {:cexpr "x"} boogie_si_record_i32($i151);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  $i159 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  $i160 := $trunc.i8.i1($i159);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i160} true;
  goto $bb117, $bb118;
$bb111:
  assume ($i153 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 124} true;
  assume {:verifier.code 0} true;
  $i154 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 124} true;
  assume {:verifier.code 0} true;
  $i155 := $trunc.i8.i1($i154);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i155} true;
  goto $bb114, $bb115;
$bb112:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 121} true;
  assume {:verifier.code 0} true;
  assume !(($i153 == 1));
  goto $bb113;
$bb113:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 153} true;
  assume {:verifier.code 0} true;
  $i158 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  $i157 := $i158;
  goto $bb116;
$bb114:
  assume ($i155 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 141} true;
  assume {:verifier.code 0} true;
  $i156 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  $i157 := $i156;
  goto $bb116;
$bb115:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  assume !(($i155 == 1));
  goto $bb113;
$bb116:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 106} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 901, 7} true;
  assume {:verifier.code 0} true;
  $i151 := $i157;
  goto $bb110;
$bb117:
  assume ($i160 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 31} true;
  assume {:verifier.code 0} true;
  $i161 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  $i162 := $i161;
  goto $bb119;
$bb118:
  assume !(($i160 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 45} true;
  assume {:verifier.code 0} true;
  $i163 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 45} true;
  assume {:verifier.code 0} true;
  $i164 := $trunc.i8.i1($i163);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i164} true;
  goto $bb120, $bb121;
$bb119:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff0, $i162);
  call {:cexpr "x$w_buff0"} boogie_si_record_i32($i162);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  $i182 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  $i183 := $trunc.i8.i1($i182);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i183} true;
  goto $bb140, $bb141;
$bb120:
  assume ($i164 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 64} true;
  assume {:verifier.code 0} true;
  $i165 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 64} true;
  assume {:verifier.code 0} true;
  $i166 := $trunc.i8.i1($i165);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i166} true;
  goto $bb123, $bb125;
$bb121:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i164 == 1));
  goto $bb122;
$bb122:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 138} true;
  assume {:verifier.code 0} true;
  $i173 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  $i174 := $i173;
  goto $bb133;
$bb123:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 79} true;
  assume {:verifier.code 0} true;
  assume ($i166 == 1);
  goto $bb124;
$bb124:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 102} true;
  assume {:verifier.code 0} true;
  $i169 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 102} true;
  assume {:verifier.code 0} true;
  $i170 := $trunc.i8.i1($i169);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i170} true;
  goto $bb128, $bb130;
$bb125:
  assume !(($i166 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 83} true;
  assume {:verifier.code 0} true;
  $i167 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 83} true;
  assume {:verifier.code 0} true;
  $i168 := $trunc.i8.i1($i167);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i168} true;
  goto $bb126, $bb127;
$bb126:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 98} true;
  assume {:verifier.code 0} true;
  assume ($i168 == 1);
  goto $bb124;
$bb127:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i168 == 1));
  goto $bb122;
$bb128:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 117} true;
  assume {:verifier.code 0} true;
  assume ($i170 == 1);
  goto $bb129;
$bb129:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  $i175 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  $i176 := $trunc.i8.i1($i175);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i176} true;
  goto $bb134, $bb135;
$bb130:
  assume !(($i170 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 121} true;
  assume {:verifier.code 0} true;
  $i171 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 121} true;
  assume {:verifier.code 0} true;
  $i172 := $trunc.i8.i1($i171);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i172} true;
  goto $bb131, $bb132;
$bb131:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  assume ($i172 == 1);
  goto $bb129;
$bb132:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i172 == 1));
  goto $bb122;
$bb133:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 15} true;
  assume {:verifier.code 0} true;
  $i162 := $i174;
  goto $bb119;
$bb134:
  assume ($i176 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 169} true;
  assume {:verifier.code 0} true;
  $i177 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 169} true;
  assume {:verifier.code 0} true;
  $i178 := $trunc.i8.i1($i177);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i178} true;
  goto $bb137, $bb138;
$bb135:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i176 == 1));
  goto $bb136;
$bb136:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 198} true;
  assume {:verifier.code 0} true;
  $i181 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  $i180 := $i181;
  goto $bb139;
$bb137:
  assume ($i178 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 186} true;
  assume {:verifier.code 0} true;
  $i179 := $load.i32($M.0, x$w_buff0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  $i180 := $i179;
  goto $bb139;
$bb138:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i178 == 1));
  goto $bb136;
$bb139:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 902, 44} true;
  assume {:verifier.code 0} true;
  $i174 := $i180;
  goto $bb133;
$bb140:
  assume ($i183 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 31} true;
  assume {:verifier.code 0} true;
  $i184 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  $i185 := $i184;
  goto $bb142;
$bb141:
  assume !(($i183 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 45} true;
  assume {:verifier.code 0} true;
  $i186 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 45} true;
  assume {:verifier.code 0} true;
  $i187 := $trunc.i8.i1($i186);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 60} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i187} true;
  goto $bb143, $bb144;
$bb142:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 13} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x$w_buff1, $i185);
  call {:cexpr "x$w_buff1"} boogie_si_record_i32($i185);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  $i205 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  $i206 := $trunc.i8.i1($i205);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i206} true;
  goto $bb163, $bb164;
$bb143:
  assume ($i187 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 64} true;
  assume {:verifier.code 0} true;
  $i188 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 64} true;
  assume {:verifier.code 0} true;
  $i189 := $trunc.i8.i1($i188);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 79} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i189} true;
  goto $bb146, $bb148;
$bb144:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 60} true;
  assume {:verifier.code 0} true;
  assume !(($i187 == 1));
  goto $bb145;
$bb145:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 138} true;
  assume {:verifier.code 0} true;
  $i196 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  $i197 := $i196;
  goto $bb156;
$bb146:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 79} true;
  assume {:verifier.code 0} true;
  assume ($i189 == 1);
  goto $bb147;
$bb147:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 102} true;
  assume {:verifier.code 0} true;
  $i192 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 102} true;
  assume {:verifier.code 0} true;
  $i193 := $trunc.i8.i1($i192);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 117} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i193} true;
  goto $bb151, $bb153;
$bb148:
  assume !(($i189 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 83} true;
  assume {:verifier.code 0} true;
  $i190 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 83} true;
  assume {:verifier.code 0} true;
  $i191 := $trunc.i8.i1($i190);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 98} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i191} true;
  goto $bb149, $bb150;
$bb149:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 98} true;
  assume {:verifier.code 0} true;
  assume ($i191 == 1);
  goto $bb147;
$bb150:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 98} true;
  assume {:verifier.code 0} true;
  assume !(($i191 == 1));
  goto $bb145;
$bb151:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 117} true;
  assume {:verifier.code 0} true;
  assume ($i193 == 1);
  goto $bb152;
$bb152:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  $i198 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  $i199 := $trunc.i8.i1($i198);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i199} true;
  goto $bb157, $bb158;
$bb153:
  assume !(($i193 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 121} true;
  assume {:verifier.code 0} true;
  $i194 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 121} true;
  assume {:verifier.code 0} true;
  $i195 := $trunc.i8.i1($i194);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i195} true;
  goto $bb154, $bb155;
$bb154:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  assume ($i195 == 1);
  goto $bb152;
$bb155:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  assume !(($i195 == 1));
  goto $bb145;
$bb156:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 15} true;
  assume {:verifier.code 0} true;
  $i185 := $i197;
  goto $bb142;
$bb157:
  assume ($i199 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 169} true;
  assume {:verifier.code 0} true;
  $i200 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 169} true;
  assume {:verifier.code 0} true;
  $i201 := $trunc.i8.i1($i200);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i201} true;
  goto $bb160, $bb161;
$bb158:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i199 == 1));
  goto $bb159;
$bb159:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 198} true;
  assume {:verifier.code 0} true;
  $i204 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  $i203 := $i204;
  goto $bb162;
$bb160:
  assume ($i201 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 186} true;
  assume {:verifier.code 0} true;
  $i202 := $load.i32($M.0, x$w_buff1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  $i203 := $i202;
  goto $bb162;
$bb161:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  assume !(($i201 == 1));
  goto $bb159;
$bb162:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 151} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 903, 44} true;
  assume {:verifier.code 0} true;
  $i197 := $i203;
  goto $bb156;
$bb163:
  assume ($i206 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 36} true;
  assume {:verifier.code 0} true;
  $i207 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 36} true;
  assume {:verifier.code 0} true;
  $i208 := $trunc.i8.i1($i207);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 36} true;
  assume {:verifier.code 0} true;
  $i209 := $zext.i1.i32($i208);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  $i210 := $i209;
  goto $bb165;
$bb164:
  assume !(($i206 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 55} true;
  assume {:verifier.code 0} true;
  $i211 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 55} true;
  assume {:verifier.code 0} true;
  $i212 := $trunc.i8.i1($i211);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i212} true;
  goto $bb166, $bb167;
$bb165:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  $i233 := $ne.i32($i210, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 18} true;
  assume {:verifier.code 0} true;
  $i234 := $zext.i1.i8($i233);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff0_used, $i234);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8($i234);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  $i235 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  $i236 := $trunc.i8.i1($i235);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i236} true;
  goto $bb186, $bb187;
$bb166:
  assume ($i212 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 74} true;
  assume {:verifier.code 0} true;
  $i213 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 74} true;
  assume {:verifier.code 0} true;
  $i214 := $trunc.i8.i1($i213);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i214} true;
  goto $bb169, $bb171;
$bb167:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i212 == 1));
  goto $bb168;
$bb168:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 148} true;
  assume {:verifier.code 0} true;
  $i221 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 148} true;
  assume {:verifier.code 0} true;
  $i222 := $trunc.i8.i1($i221);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 148} true;
  assume {:verifier.code 0} true;
  $i223 := $zext.i1.i32($i222);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  $i224 := $i223;
  goto $bb179;
$bb169:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 89} true;
  assume {:verifier.code 0} true;
  assume ($i214 == 1);
  goto $bb170;
$bb170:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 112} true;
  assume {:verifier.code 0} true;
  $i217 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 112} true;
  assume {:verifier.code 0} true;
  $i218 := $trunc.i8.i1($i217);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i218} true;
  goto $bb174, $bb176;
$bb171:
  assume !(($i214 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 93} true;
  assume {:verifier.code 0} true;
  $i215 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 93} true;
  assume {:verifier.code 0} true;
  $i216 := $trunc.i8.i1($i215);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i216} true;
  goto $bb172, $bb173;
$bb172:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 108} true;
  assume {:verifier.code 0} true;
  assume ($i216 == 1);
  goto $bb170;
$bb173:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i216 == 1));
  goto $bb168;
$bb174:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 127} true;
  assume {:verifier.code 0} true;
  assume ($i218 == 1);
  goto $bb175;
$bb175:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  $i225 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  $i226 := $trunc.i8.i1($i225);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i226} true;
  goto $bb180, $bb181;
$bb176:
  assume !(($i218 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 131} true;
  assume {:verifier.code 0} true;
  $i219 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 131} true;
  assume {:verifier.code 0} true;
  $i220 := $trunc.i8.i1($i219);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i220} true;
  goto $bb177, $bb178;
$bb177:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  assume ($i220 == 1);
  goto $bb175;
$bb178:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i220 == 1));
  goto $bb168;
$bb179:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 20} true;
  assume {:verifier.code 0} true;
  $i210 := $i224;
  goto $bb165;
$bb180:
  assume ($i226 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 184} true;
  assume {:verifier.code 0} true;
  $i227 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 184} true;
  assume {:verifier.code 0} true;
  $i228 := $trunc.i8.i1($i227);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i228} true;
  goto $bb183, $bb184;
$bb181:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i226 == 1));
  goto $bb182;
$bb182:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 212} true;
  assume {:verifier.code 0} true;
  $i230 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 212} true;
  assume {:verifier.code 0} true;
  $i231 := $trunc.i8.i1($i230);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 212} true;
  assume {:verifier.code 0} true;
  $i232 := $zext.i1.i32($i231);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  $i229 := $i232;
  goto $bb185;
$bb183:
  assume ($i228 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  $i229 := 0;
  goto $bb185;
$bb184:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i228 == 1));
  goto $bb182;
$bb185:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 904, 54} true;
  assume {:verifier.code 0} true;
  $i224 := $i229;
  goto $bb179;
$bb186:
  assume ($i236 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 36} true;
  assume {:verifier.code 0} true;
  $i237 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 36} true;
  assume {:verifier.code 0} true;
  $i238 := $trunc.i8.i1($i237);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 36} true;
  assume {:verifier.code 0} true;
  $i239 := $zext.i1.i32($i238);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  $i240 := $i239;
  goto $bb188;
$bb187:
  assume !(($i236 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 55} true;
  assume {:verifier.code 0} true;
  $i241 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 55} true;
  assume {:verifier.code 0} true;
  $i242 := $trunc.i8.i1($i241);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i242} true;
  goto $bb189, $bb190;
$bb188:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  $i261 := $ne.i32($i240, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 18} true;
  assume {:verifier.code 0} true;
  $i262 := $zext.i1.i8($i261);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$w_buff1_used, $i262);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8($i262);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  $i263 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  $i264 := $trunc.i8.i1($i263);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i264} true;
  goto $bb206, $bb207;
$bb189:
  assume ($i242 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 74} true;
  assume {:verifier.code 0} true;
  $i243 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 74} true;
  assume {:verifier.code 0} true;
  $i244 := $trunc.i8.i1($i243);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i244} true;
  goto $bb192, $bb194;
$bb190:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i242 == 1));
  goto $bb191;
$bb191:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 148} true;
  assume {:verifier.code 0} true;
  $i251 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 148} true;
  assume {:verifier.code 0} true;
  $i252 := $trunc.i8.i1($i251);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 148} true;
  assume {:verifier.code 0} true;
  $i253 := $zext.i1.i32($i252);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  $i254 := $i253;
  goto $bb202;
$bb192:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 89} true;
  assume {:verifier.code 0} true;
  assume ($i244 == 1);
  goto $bb193;
$bb193:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 112} true;
  assume {:verifier.code 0} true;
  $i247 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 112} true;
  assume {:verifier.code 0} true;
  $i248 := $trunc.i8.i1($i247);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i248} true;
  goto $bb197, $bb199;
$bb194:
  assume !(($i244 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 93} true;
  assume {:verifier.code 0} true;
  $i245 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 93} true;
  assume {:verifier.code 0} true;
  $i246 := $trunc.i8.i1($i245);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i246} true;
  goto $bb195, $bb196;
$bb195:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 108} true;
  assume {:verifier.code 0} true;
  assume ($i246 == 1);
  goto $bb193;
$bb196:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i246 == 1));
  goto $bb191;
$bb197:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 127} true;
  assume {:verifier.code 0} true;
  assume ($i248 == 1);
  goto $bb198;
$bb198:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 166} true;
  assume {:verifier.code 0} true;
  $i255 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 166} true;
  assume {:verifier.code 0} true;
  $i256 := $trunc.i8.i1($i255);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 181} true;
  assume {:verifier.code 0} true;
  $i257 := 0;
  assume {:branchcond $i256} true;
  goto $bb203, $bb204;
$bb199:
  assume !(($i248 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 131} true;
  assume {:verifier.code 0} true;
  $i249 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 131} true;
  assume {:verifier.code 0} true;
  $i250 := $trunc.i8.i1($i249);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i250} true;
  goto $bb200, $bb201;
$bb200:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  assume ($i250 == 1);
  goto $bb198;
$bb201:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i250 == 1));
  goto $bb191;
$bb202:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 20} true;
  assume {:verifier.code 0} true;
  $i240 := $i254;
  goto $bb188;
$bb203:
  assume ($i256 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 184} true;
  assume {:verifier.code 0} true;
  $i258 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 184} true;
  assume {:verifier.code 0} true;
  $i259 := $trunc.i8.i1($i258);
  assume {:verifier.code 0} true;
  $i257 := $i259;
  goto $bb205;
$bb204:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i256 == 1));
  goto $bb205;
$bb205:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 166} true;
  assume {:verifier.code 0} true;
  $i260 := (if ($i257 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 905, 54} true;
  assume {:verifier.code 0} true;
  $i254 := $i260;
  goto $bb202;
$bb206:
  assume ($i264 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 36} true;
  assume {:verifier.code 0} true;
  $i265 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 36} true;
  assume {:verifier.code 0} true;
  $i266 := $trunc.i8.i1($i265);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 36} true;
  assume {:verifier.code 0} true;
  $i267 := $zext.i1.i32($i266);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  $i268 := $i267;
  goto $bb208;
$bb207:
  assume !(($i264 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 55} true;
  assume {:verifier.code 0} true;
  $i269 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 55} true;
  assume {:verifier.code 0} true;
  $i270 := $trunc.i8.i1($i269);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i270} true;
  goto $bb209, $bb210;
$bb208:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  $i291 := $ne.i32($i268, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 18} true;
  assume {:verifier.code 0} true;
  $i292 := $zext.i1.i8($i291);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff0_thd0, $i292);
  call {:cexpr "x$r_buff0_thd0"} boogie_si_record_i8($i292);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  $i293 := $load.i8($M.0, weak$$choice2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  $i294 := $trunc.i8.i1($i293);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i294} true;
  goto $bb229, $bb230;
$bb209:
  assume ($i270 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 74} true;
  assume {:verifier.code 0} true;
  $i271 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 74} true;
  assume {:verifier.code 0} true;
  $i272 := $trunc.i8.i1($i271);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i272} true;
  goto $bb212, $bb214;
$bb210:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i270 == 1));
  goto $bb211;
$bb211:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 148} true;
  assume {:verifier.code 0} true;
  $i279 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 148} true;
  assume {:verifier.code 0} true;
  $i280 := $trunc.i8.i1($i279);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 148} true;
  assume {:verifier.code 0} true;
  $i281 := $zext.i1.i32($i280);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  $i282 := $i281;
  goto $bb222;
$bb212:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 89} true;
  assume {:verifier.code 0} true;
  assume ($i272 == 1);
  goto $bb213;
$bb213:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 112} true;
  assume {:verifier.code 0} true;
  $i275 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 112} true;
  assume {:verifier.code 0} true;
  $i276 := $trunc.i8.i1($i275);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i276} true;
  goto $bb217, $bb219;
$bb214:
  assume !(($i272 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 93} true;
  assume {:verifier.code 0} true;
  $i273 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 93} true;
  assume {:verifier.code 0} true;
  $i274 := $trunc.i8.i1($i273);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i274} true;
  goto $bb215, $bb216;
$bb215:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 108} true;
  assume {:verifier.code 0} true;
  assume ($i274 == 1);
  goto $bb213;
$bb216:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i274 == 1));
  goto $bb211;
$bb217:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 127} true;
  assume {:verifier.code 0} true;
  assume ($i276 == 1);
  goto $bb218;
$bb218:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  $i283 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  $i284 := $trunc.i8.i1($i283);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 181} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i284} true;
  goto $bb223, $bb224;
$bb219:
  assume !(($i276 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 131} true;
  assume {:verifier.code 0} true;
  $i277 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 131} true;
  assume {:verifier.code 0} true;
  $i278 := $trunc.i8.i1($i277);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i278} true;
  goto $bb220, $bb221;
$bb220:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  assume ($i278 == 1);
  goto $bb218;
$bb221:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i278 == 1));
  goto $bb211;
$bb222:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 20} true;
  assume {:verifier.code 0} true;
  $i268 := $i282;
  goto $bb208;
$bb223:
  assume ($i284 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 184} true;
  assume {:verifier.code 0} true;
  $i285 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 184} true;
  assume {:verifier.code 0} true;
  $i286 := $trunc.i8.i1($i285);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i286} true;
  goto $bb226, $bb227;
$bb224:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i284 == 1));
  goto $bb225;
$bb225:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 212} true;
  assume {:verifier.code 0} true;
  $i288 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 212} true;
  assume {:verifier.code 0} true;
  $i289 := $trunc.i8.i1($i288);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 212} true;
  assume {:verifier.code 0} true;
  $i290 := $zext.i1.i32($i289);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  $i287 := $i290;
  goto $bb228;
$bb226:
  assume ($i286 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  $i287 := 0;
  goto $bb228;
$bb227:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  assume !(($i286 == 1));
  goto $bb225;
$bb228:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 166} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 906, 54} true;
  assume {:verifier.code 0} true;
  $i282 := $i287;
  goto $bb222;
$bb229:
  assume ($i294 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 36} true;
  assume {:verifier.code 0} true;
  $i295 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 36} true;
  assume {:verifier.code 0} true;
  $i296 := $trunc.i8.i1($i295);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 36} true;
  assume {:verifier.code 0} true;
  $i297 := $zext.i1.i32($i296);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  $i298 := $i297;
  goto $bb231;
$bb230:
  assume !(($i294 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 55} true;
  assume {:verifier.code 0} true;
  $i299 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 55} true;
  assume {:verifier.code 0} true;
  $i300 := $trunc.i8.i1($i299);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 70} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i300} true;
  goto $bb232, $bb233;
$bb231:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  $i319 := $ne.i32($i298, 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 18} true;
  assume {:verifier.code 0} true;
  $i320 := $zext.i1.i8($i319);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 18} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$r_buff1_thd0, $i320);
  call {:cexpr "x$r_buff1_thd0"} boogie_si_record_i8($i320);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 23} true;
  assume {:verifier.code 0} true;
  $i321 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 25} true;
  assume {:verifier.code 0} true;
  $i322 := $eq.i32($i321, 2);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 30} true;
  assume {:verifier.code 0} true;
  $i323 := 0;
  assume {:branchcond $i322} true;
  goto $bb249, $bb250;
$bb232:
  assume ($i300 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 74} true;
  assume {:verifier.code 0} true;
  $i301 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 74} true;
  assume {:verifier.code 0} true;
  $i302 := $trunc.i8.i1($i301);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 89} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i302} true;
  goto $bb235, $bb237;
$bb233:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 70} true;
  assume {:verifier.code 0} true;
  assume !(($i300 == 1));
  goto $bb234;
$bb234:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 148} true;
  assume {:verifier.code 0} true;
  $i309 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 148} true;
  assume {:verifier.code 0} true;
  $i310 := $trunc.i8.i1($i309);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 148} true;
  assume {:verifier.code 0} true;
  $i311 := $zext.i1.i32($i310);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  $i312 := $i311;
  goto $bb245;
$bb235:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 89} true;
  assume {:verifier.code 0} true;
  assume ($i302 == 1);
  goto $bb236;
$bb236:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 112} true;
  assume {:verifier.code 0} true;
  $i305 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 112} true;
  assume {:verifier.code 0} true;
  $i306 := $trunc.i8.i1($i305);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 127} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i306} true;
  goto $bb240, $bb242;
$bb237:
  assume !(($i302 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 93} true;
  assume {:verifier.code 0} true;
  $i303 := $load.i8($M.0, x$w_buff1_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 93} true;
  assume {:verifier.code 0} true;
  $i304 := $trunc.i8.i1($i303);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 108} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i304} true;
  goto $bb238, $bb239;
$bb238:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 108} true;
  assume {:verifier.code 0} true;
  assume ($i304 == 1);
  goto $bb236;
$bb239:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 108} true;
  assume {:verifier.code 0} true;
  assume !(($i304 == 1));
  goto $bb234;
$bb240:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 127} true;
  assume {:verifier.code 0} true;
  assume ($i306 == 1);
  goto $bb241;
$bb241:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 166} true;
  assume {:verifier.code 0} true;
  $i313 := $load.i8($M.0, x$w_buff0_used);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 166} true;
  assume {:verifier.code 0} true;
  $i314 := $trunc.i8.i1($i313);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 181} true;
  assume {:verifier.code 0} true;
  $i315 := 0;
  assume {:branchcond $i314} true;
  goto $bb246, $bb247;
$bb242:
  assume !(($i306 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 131} true;
  assume {:verifier.code 0} true;
  $i307 := $load.i8($M.0, x$r_buff1_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 131} true;
  assume {:verifier.code 0} true;
  $i308 := $trunc.i8.i1($i307);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i308} true;
  goto $bb243, $bb244;
$bb243:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  assume ($i308 == 1);
  goto $bb241;
$bb244:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  assume !(($i308 == 1));
  goto $bb234;
$bb245:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 20} true;
  assume {:verifier.code 0} true;
  $i298 := $i312;
  goto $bb231;
$bb246:
  assume ($i314 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 184} true;
  assume {:verifier.code 0} true;
  $i316 := $load.i8($M.0, x$r_buff0_thd0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 184} true;
  assume {:verifier.code 0} true;
  $i317 := $trunc.i8.i1($i316);
  assume {:verifier.code 0} true;
  $i315 := $i317;
  goto $bb248;
$bb247:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 181} true;
  assume {:verifier.code 0} true;
  assume !(($i314 == 1));
  goto $bb248;
$bb248:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 166} true;
  assume {:verifier.code 0} true;
  $i318 := (if ($i315 == 1) then 0 else 0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 907, 54} true;
  assume {:verifier.code 0} true;
  $i312 := $i318;
  goto $bb245;
$bb249:
  assume ($i322 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 33} true;
  assume {:verifier.code 0} true;
  $i324 := $load.i32($M.0, __unbuffered_p0_EAX);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 53} true;
  assume {:verifier.code 0} true;
  $i325 := $eq.i32($i324, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 58} true;
  assume {:verifier.code 0} true;
  $i323 := 0;
  assume {:branchcond $i325} true;
  goto $bb252, $bb253;
$bb250:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 30} true;
  assume {:verifier.code 0} true;
  assume !(($i322 == 1));
  goto $bb251;
$bb251:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 0, 0} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 21} true;
  assume {:verifier.code 0} true;
  // llvm2bpl: output/rfi002_tso.opt_tmp.c:908:21: warning: over-approximating bitwise operation xor;
  $i330 := $xor.i1($i323, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 19} true;
  assume {:verifier.code 0} true;
  $i331 := $zext.i1.i8($i330);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, main$tmp_guard1, $i331);
  call {:cexpr "main$tmp_guard1"} boogie_si_record_i8($i331);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  $i332 := $load.i8($M.0, x$flush_delayed);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  $i333 := $trunc.i8.i1($i332);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  assume {:branchcond $i333} true;
  goto $bb256, $bb257;
$bb252:
  assume ($i325 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 61} true;
  assume {:verifier.code 0} true;
  $i326 := $load.i32($M.0, __unbuffered_p1_EAX);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 81} true;
  assume {:verifier.code 0} true;
  $i327 := $eq.i32($i326, 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 86} true;
  assume {:verifier.code 0} true;
  $i323 := 0;
  assume {:branchcond $i327} true;
  goto $bb254, $bb255;
$bb253:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 58} true;
  assume {:verifier.code 0} true;
  assume !(($i325 == 1));
  goto $bb251;
$bb254:
  assume ($i327 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 89} true;
  assume {:verifier.code 0} true;
  $i328 := $load.i32($M.0, __unbuffered_p1_EBX);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 109} true;
  assume {:verifier.code 0} true;
  $i329 := $eq.i32($i328, 0);
  assume {:verifier.code 0} true;
  $i323 := $i329;
  goto $bb251;
$bb255:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 908, 86} true;
  assume {:verifier.code 0} true;
  assume !(($i327 == 1));
  goto $bb251;
$bb256:
  assume ($i333 == 1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 25} true;
  assume {:verifier.code 0} true;
  $i334 := $load.i32($M.0, x$mem_tmp);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  $i335 := $i334;
  goto $bb258;
$bb257:
  assume !(($i333 == 1));
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 37} true;
  assume {:verifier.code 0} true;
  $i336 := $load.i32($M.0, x);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  $i335 := $i336;
  goto $bb258;
$bb258:
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 7} true;
  assume {:verifier.code 0} true;
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 909, 5} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i32($M.0, x, $i335);
  call {:cexpr "x"} boogie_si_record_i32($i335);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 910, 19} true;
  assume {:verifier.code 0} true;
  $M.0 := $store.i8($M.0, x$flush_delayed, 0);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8(0);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 911, 3} true;
  assume {:verifier.code 0} true;
  call __VERIFIER_atomic_end();
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 912, 21} true;
  assume {:verifier.code 1} true;
  $i337 := $load.i8($M.0, main$tmp_guard1);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 912, 21} true;
  assume {:verifier.code 1} true;
  $i338 := $trunc.i8.i1($i337);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 912, 21} true;
  assume {:verifier.code 1} true;
  $i339 := $zext.i1.i32($i338);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 912, 3} true;
  assume {:verifier.code 1} true;
  call __VERIFIER_assert($i339);
  assume {:sourceloc "output/rfi002_tso.opt_tmp.c", 913, 3} true;
  assume {:verifier.code 0} true;
  $r := 0;
  $exn := false;
  return;
}
const pthread_create: ref;
axiom (pthread_create == $sub.ref(0, 55517));
procedure  pthread_create($p0: ref, $p1: ref, $p2: ref, $p3: ref)
  returns ($r: i32);
const __VERIFIER_assume: ref;
axiom (__VERIFIER_assume == $sub.ref(0, 56549));
procedure  __VERIFIER_assume($i0: i32)
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  call {:cexpr "__VERIFIER_assume:arg:x"} boogie_si_record_i32($i0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  assume true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 38, 3} true;
  assume {:verifier.code 1} true;
  assume $i0 != $0;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 39, 1} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const __SMACK_code: ref;
axiom (__SMACK_code == $sub.ref(0, 57581));
procedure  __SMACK_code.ref($p0: ref);
procedure  __SMACK_code.ref.i32($p0: ref, p.1: i32);
const __SMACK_dummy: ref;
axiom (__SMACK_dummy == $sub.ref(0, 58613));
procedure  __SMACK_dummy($i0: i32)
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  call {:cexpr "__SMACK_dummy:arg:v"} boogie_si_record_i32($i0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  assume true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 59} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const __SMACK_check_overflow: ref;
axiom (__SMACK_check_overflow == $sub.ref(0, 59645));
procedure  __SMACK_check_overflow($i0: i32)
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  call {:cexpr "__SMACK_check_overflow:arg:flag"} boogie_si_record_i32($i0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 247, 29} true;
  assume {:verifier.code 1} true;
  assume true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 52, 3} true;
  assume {:verifier.code 1} true;
  assert {:overflow} $i0 == $0;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 53, 1} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const __SMACK_nondet_char: ref;
axiom (__SMACK_nondet_char == $sub.ref(0, 60677));
procedure  __SMACK_nondet_char()
  returns ($r: i8);
const __SMACK_nondet_signed_char: ref;
axiom (__SMACK_nondet_signed_char == $sub.ref(0, 61709));
procedure  __SMACK_nondet_signed_char()
  returns ($r: i8);
const __SMACK_nondet_unsigned_char: ref;
axiom (__SMACK_nondet_unsigned_char == $sub.ref(0, 62741));
procedure  __SMACK_nondet_unsigned_char()
  returns ($r: i8);
const __SMACK_nondet_short: ref;
axiom (__SMACK_nondet_short == $sub.ref(0, 63773));
procedure  __SMACK_nondet_short()
  returns ($r: i16);
const __SMACK_nondet_signed_short: ref;
axiom (__SMACK_nondet_signed_short == $sub.ref(0, 64805));
procedure  __SMACK_nondet_signed_short()
  returns ($r: i16);
const __SMACK_nondet_signed_short_int: ref;
axiom (__SMACK_nondet_signed_short_int == $sub.ref(0, 65837));
procedure  __SMACK_nondet_signed_short_int()
  returns ($r: i16);
const __SMACK_nondet_unsigned_short: ref;
axiom (__SMACK_nondet_unsigned_short == $sub.ref(0, 66869));
procedure  __SMACK_nondet_unsigned_short()
  returns ($r: i16);
const __SMACK_nondet_unsigned_short_int: ref;
axiom (__SMACK_nondet_unsigned_short_int == $sub.ref(0, 67901));
procedure  __SMACK_nondet_unsigned_short_int()
  returns ($r: i16);
const __VERIFIER_nondet_int: ref;
axiom (__VERIFIER_nondet_int == $sub.ref(0, 68933));
procedure  __VERIFIER_nondet_int()
  returns ($r: i32)
{
  var $i0: i32;
  var $i1: i1;
  var $i3: i1;
  var $i2: i1;
  var $i4: i32;
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 104, 11} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 104, 11} true;
  assume {:verifier.code 1} true;
  call $i0 := __SMACK_nondet_int();
  call {:cexpr "smack:ext:__SMACK_nondet_int"} boogie_si_record_i32($i0);
  call {:cexpr "x"} boogie_si_record_i32($i0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 23} true;
  assume {:verifier.code 0} true;
  $i1 := $sge.i32($i0, $sub.i32(0, 2147483648));
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 34} true;
  assume {:verifier.code 0} true;
  $i2 := 0;
  assume {:branchcond $i1} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i1 == 1);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 39} true;
  assume {:verifier.code 1} true;
  $i3 := $sle.i32($i0, 2147483647);
  assume {:verifier.code 0} true;
  $i2 := $i3;
  goto $bb3;
$bb2:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 34} true;
  assume {:verifier.code 0} true;
  assume !(($i1 == 1));
  goto $bb3;
$bb3:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 0, 0} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 34} true;
  assume {:verifier.code 1} true;
  $i4 := $zext.i1.i32($i2);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 105, 3} true;
  assume {:verifier.code 1} true;
  call __VERIFIER_assume($i4);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 106, 3} true;
  assume {:verifier.code 0} true;
  $r := $i0;
  $exn := false;
  return;
}
const __SMACK_nondet_int: ref;
axiom (__SMACK_nondet_int == $sub.ref(0, 69965));
procedure  __SMACK_nondet_int()
  returns ($r: i32);
const __SMACK_nondet_signed_int: ref;
axiom (__SMACK_nondet_signed_int == $sub.ref(0, 70997));
procedure  __SMACK_nondet_signed_int()
  returns ($r: i32);
const __SMACK_nondet_unsigned: ref;
axiom (__SMACK_nondet_unsigned == $sub.ref(0, 72029));
procedure  __SMACK_nondet_unsigned()
  returns ($r: i32);
const __SMACK_nondet_unsigned_int: ref;
axiom (__SMACK_nondet_unsigned_int == $sub.ref(0, 73061));
procedure  __SMACK_nondet_unsigned_int()
  returns ($r: i32);
const __SMACK_nondet_long: ref;
axiom (__SMACK_nondet_long == $sub.ref(0, 74093));
procedure  __SMACK_nondet_long()
  returns ($r: i64);
const __SMACK_nondet_long_int: ref;
axiom (__SMACK_nondet_long_int == $sub.ref(0, 75125));
procedure  __SMACK_nondet_long_int()
  returns ($r: i64);
const __SMACK_nondet_signed_long: ref;
axiom (__SMACK_nondet_signed_long == $sub.ref(0, 76157));
procedure  __SMACK_nondet_signed_long()
  returns ($r: i64);
const __SMACK_nondet_signed_long_int: ref;
axiom (__SMACK_nondet_signed_long_int == $sub.ref(0, 77189));
procedure  __SMACK_nondet_signed_long_int()
  returns ($r: i64);
const __SMACK_nondet_unsigned_long: ref;
axiom (__SMACK_nondet_unsigned_long == $sub.ref(0, 78221));
procedure  __SMACK_nondet_unsigned_long()
  returns ($r: i64);
const __SMACK_nondet_unsigned_long_int: ref;
axiom (__SMACK_nondet_unsigned_long_int == $sub.ref(0, 79253));
procedure  __SMACK_nondet_unsigned_long_int()
  returns ($r: i64);
const __SMACK_nondet_long_long: ref;
axiom (__SMACK_nondet_long_long == $sub.ref(0, 80285));
procedure  __SMACK_nondet_long_long()
  returns ($r: i64);
const __SMACK_nondet_long_long_int: ref;
axiom (__SMACK_nondet_long_long_int == $sub.ref(0, 81317));
procedure  __SMACK_nondet_long_long_int()
  returns ($r: i64);
const __SMACK_nondet_signed_long_long: ref;
axiom (__SMACK_nondet_signed_long_long == $sub.ref(0, 82349));
procedure  __SMACK_nondet_signed_long_long()
  returns ($r: i64);
const __SMACK_nondet_signed_long_long_int: ref;
axiom (__SMACK_nondet_signed_long_long_int == $sub.ref(0, 83381));
procedure  __SMACK_nondet_signed_long_long_int()
  returns ($r: i64);
const __SMACK_nondet_unsigned_long_long: ref;
axiom (__SMACK_nondet_unsigned_long_long == $sub.ref(0, 84413));
procedure  __SMACK_nondet_unsigned_long_long()
  returns ($r: i64);
const __SMACK_nondet_unsigned_long_long_int: ref;
axiom (__SMACK_nondet_unsigned_long_long_int == $sub.ref(0, 85445));
procedure  __SMACK_nondet_unsigned_long_long_int()
  returns ($r: i64);
const __VERIFIER_nondet_bool: ref;
axiom (__VERIFIER_nondet_bool == $sub.ref(0, 86477));
procedure  __VERIFIER_nondet_bool()
  returns ($r: i1)
{
  var $i0: i32;
  var $i1: i1;
  var $i2: i8;
  var $i3: i1;
  var $i4: i32;
  var $i5: i1;
  var $i7: i1;
  var $i8: i32;
  var $i9: i1;
  var $i6: i1;
  var $i10: i32;
  var $i11: i1;
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 220, 20} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 220, 20} true;
  assume {:verifier.code 1} true;
  call $i0 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 220, 13} true;
  assume {:verifier.code 0} true;
  $i1 := $ne.i32($i0, 0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 220, 9} true;
  assume {:verifier.code 0} true;
  $i2 := $zext.i1.i8($i1);
  call {:cexpr "x"} boogie_si_record_i8($i2);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 21} true;
  assume {:verifier.code 0} true;
  $i3 := $trunc.i8.i1($i2);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 21} true;
  assume {:verifier.code 0} true;
  $i4 := $zext.i1.i32($i3);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 23} true;
  assume {:verifier.code 0} true;
  $i5 := $eq.i32($i4, 0);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 28} true;
  assume {:verifier.code 0} true;
  $i6 := 1;
  assume {:branchcond $i5} true;
  goto $bb1, $bb3;
$bb1:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 28} true;
  assume {:verifier.code 0} true;
  assume ($i5 == 1);
  goto $bb2;
$bb2:
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 28} true;
  assume {:verifier.code 1} true;
  $i10 := $zext.i1.i32($i6);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 3} true;
  assume {:verifier.code 1} true;
  call __VERIFIER_assume($i10);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 222, 10} true;
  assume {:verifier.code 0} true;
  $i11 := $trunc.i8.i1($i2);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 222, 3} true;
  assume {:verifier.code 0} true;
  $r := $i11;
  $exn := false;
  return;
$bb3:
  assume !(($i5 == 1));
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 31} true;
  assume {:verifier.code 1} true;
  $i7 := $trunc.i8.i1($i2);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 31} true;
  assume {:verifier.code 1} true;
  $i8 := $zext.i1.i32($i7);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 33} true;
  assume {:verifier.code 1} true;
  $i9 := $eq.i32($i8, 1);
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 221, 28} true;
  assume {:verifier.code 0} true;
  $i6 := $i9;
  goto $bb2;
}
const __SMACK_decls: ref;
axiom (__SMACK_decls == $sub.ref(0, 87509));
type $mop;
procedure  boogie_si_record_mop(m: $mop);
const $MOP: $mop;
var $exn: bool;
var $exnv: int;
procedure  corral_atomic_begin();
procedure  corral_atomic_end();
procedure  $alloc(n: ref) returns (p: ref)
{
  call corral_atomic_begin();
  call p := $$alloc(n);
  call corral_atomic_end();
}

procedure  $malloc(n: ref) returns (p: ref)
{
  call corral_atomic_begin();
  call p := $$alloc(n);
  call corral_atomic_end();
}

var $CurrAddr:ref;

procedure {:inline 1} $$alloc(n: ref) returns (p: ref)
modifies $CurrAddr;
{
  assume $sge.ref.bool(n, $0.ref);
  if ($sgt.ref.bool(n, $0.ref)) {
    p := $CurrAddr;
    havoc $CurrAddr;
    assume $sge.ref.bool($sub.ref($CurrAddr, n), p);
    assume $sgt.ref.bool($CurrAddr, $0.ref) && $slt.ref.bool($CurrAddr, $MALLOC_TOP);
  } else {
    p := $0.ref;
  }
}

procedure  $free(p: ref);

const __SMACK_top_decl: ref;
axiom (__SMACK_top_decl == $sub.ref(0, 88541));
procedure  __SMACK_top_decl.ref($p0: ref);
const __SMACK_init_func_memory_model: ref;
axiom (__SMACK_init_func_memory_model == $sub.ref(0, 89573));
procedure  __SMACK_init_func_memory_model()
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 528, 3} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 528, 3} true;
  assume {:verifier.code 1} true;
  $CurrAddr := $1024.ref;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 533, 1} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const __VERIFIER_atomic_begin: ref;
axiom (__VERIFIER_atomic_begin == $sub.ref(0, 90605));
procedure  __VERIFIER_atomic_begin()
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 535, 34} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 535, 34} true;
  assume {:verifier.code 1} true;
  call corral_atomic_begin();
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 535, 79} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const __VERIFIER_atomic_end: ref;
axiom (__VERIFIER_atomic_end == $sub.ref(0, 91637));
procedure  __VERIFIER_atomic_end()
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 537, 32} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 537, 32} true;
  assume {:verifier.code 1} true;
  call corral_atomic_end();
  assume {:sourceloc "/usr/local/share/smack/lib/smack.c", 537, 75} true;
  assume {:verifier.code 0} true;
  $exn := false;
  return;
}
const abort: ref;
axiom (abort == $sub.ref(0, 92669));
procedure  abort()
{
$bb0:
  assume {:sourceloc "/usr/local/share/smack/lib/stdlib.c", 21, 3} true;
  assume {:verifier.code 1} true;
  assume {:sourceloc "/usr/local/share/smack/lib/stdlib.c", 21, 3} true;
  assume {:verifier.code 1} true;
  assume false;
  assume {:sourceloc "/usr/local/share/smack/lib/stdlib.c", 22, 3} true;
  assume {:verifier.code 0} true;
  goto $bb1;
$bb1:
  assume {:sourceloc "/usr/local/share/smack/lib/stdlib.c", 22, 3} true;
  assume {:verifier.code 0} true;
  goto $bb1;
}
const llvm.dbg.value: ref;
axiom (llvm.dbg.value == $sub.ref(0, 93701));
procedure  llvm.dbg.value($p0: ref, $p1: ref, $p2: ref);
const __SMACK_static_init: ref;
axiom (__SMACK_static_init == $sub.ref(0, 94733));
procedure  __SMACK_static_init()
{
$bb0:
  $M.0 := $store.i32($M.0, __unbuffered_cnt, 0);
  call {:cexpr "__unbuffered_cnt"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, __unbuffered_p0_EAX, 0);
  call {:cexpr "__unbuffered_p0_EAX"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, __unbuffered_p1_EAX, 0);
  call {:cexpr "__unbuffered_p1_EAX"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, __unbuffered_p1_EBX, 0);
  call {:cexpr "__unbuffered_p1_EBX"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, x, 0);
  call {:cexpr "x"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, y, 0);
  call {:cexpr "y"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, x$w_buff0_used, 0);
  call {:cexpr "x$w_buff0_used"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$r_buff0_thd1, 0);
  call {:cexpr "x$r_buff0_thd1"} boogie_si_record_i8(0);
  $M.0 := $store.i32($M.0, x$w_buff0, 0);
  call {:cexpr "x$w_buff0"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, x$w_buff1_used, 0);
  call {:cexpr "x$w_buff1_used"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$r_buff1_thd1, 0);
  call {:cexpr "x$r_buff1_thd1"} boogie_si_record_i8(0);
  $M.0 := $store.i32($M.0, x$w_buff1, 0);
  call {:cexpr "x$w_buff1"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, weak$$choice0, 0);
  call {:cexpr "weak$$choice0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, weak$$choice2, 0);
  call {:cexpr "weak$$choice2"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$flush_delayed, 0);
  call {:cexpr "x$flush_delayed"} boogie_si_record_i8(0);
  $M.0 := $store.i32($M.0, x$mem_tmp, 0);
  call {:cexpr "x$mem_tmp"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, x$r_buff0_thd0, 0);
  call {:cexpr "x$r_buff0_thd0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$r_buff1_thd0, 0);
  call {:cexpr "x$r_buff1_thd0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$r_buff0_thd2, 0);
  call {:cexpr "x$r_buff0_thd2"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$r_buff1_thd2, 0);
  call {:cexpr "x$r_buff1_thd2"} boogie_si_record_i8(0);
  $M.0 := $store.i32($M.0, y$w_buff0, 0);
  call {:cexpr "y$w_buff0"} boogie_si_record_i32(0);
  $M.0 := $store.i32($M.0, y$w_buff1, 0);
  call {:cexpr "y$w_buff1"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, y$w_buff0_used, 0);
  call {:cexpr "y$w_buff0_used"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$w_buff1_used, 0);
  call {:cexpr "y$w_buff1_used"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff0_thd0, 0);
  call {:cexpr "y$r_buff0_thd0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff1_thd0, 0);
  call {:cexpr "y$r_buff1_thd0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff0_thd1, 0);
  call {:cexpr "y$r_buff0_thd1"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff1_thd1, 0);
  call {:cexpr "y$r_buff1_thd1"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff0_thd2, 0);
  call {:cexpr "y$r_buff0_thd2"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$r_buff1_thd2, 0);
  call {:cexpr "y$r_buff1_thd2"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, y$flush_delayed, 0);
  call {:cexpr "y$flush_delayed"} boogie_si_record_i8(0);
  $M.0 := $store.i32($M.0, y$mem_tmp, 0);
  call {:cexpr "y$mem_tmp"} boogie_si_record_i32(0);
  $M.0 := $store.i8($M.0, main$tmp_guard0, 0);
  call {:cexpr "main$tmp_guard0"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, main$tmp_guard1, 0);
  call {:cexpr "main$tmp_guard1"} boogie_si_record_i8(0);
  $M.0 := $store.i8($M.0, x$read_delayed, 0);
  call {:cexpr "x$read_delayed"} boogie_si_record_i8(0);
  $M.0 := $store.ref($M.0, x$read_delayed_var, $0.ref);
  $M.0 := $store.i8($M.0, y$read_delayed, 0);
  call {:cexpr "y$read_delayed"} boogie_si_record_i8(0);
  $M.0 := $store.ref($M.0, y$read_delayed_var, $0.ref);
  $M.0 := $store.ref($M.0, env_value_str, .str.1.12);
  $M.0 := $store.i32($M.0, errno_global, 0);
  call {:cexpr "errno_global"} boogie_si_record_i32(0);
  $exn := false;
  return;
}
procedure  boogie_si_record_i1(x: i1);
procedure  boogie_si_record_i32(x: i32);
procedure  boogie_si_record_i8(x: i8);
procedure  boogie_si_record_ref(x: ref);
procedure  $initialize()
{
  call __SMACK_static_init();
  call __SMACK_init_func_memory_model();
  return;
}
