...
public class LinuxTest extends AbstractDartagnanTest {

    @Parameterized.Parameters(name = "{index}: {0} {4}")
    public static Iterable<Object[]> data() throws IOException {
    	// We want the files to be created every time we run the unit tests
        for(Method method : Method.values()) {
        	initialiseCSVFile(LinuxTest.class, method.asStringOption(), "");
        }

        return buildParameters("litmus/C/", "cat/linux-kernel.cat", Arch.NONE);
    }

    public LinuxTest(String path, Result expected, Arch target, Wmm wmm, Settings settings) {
