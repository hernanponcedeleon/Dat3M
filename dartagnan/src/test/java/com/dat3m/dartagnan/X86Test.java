...
public class X86Test extends AbstractDartagnanTest {

    @Parameterized.Parameters(name = "{index}: {0} {4}")
    public static Iterable<Object[]> data() throws IOException {
    	// We want the files to be created every time we run the unit tests
        for(Method method : Method.values()) {
        	initialiseCSVFile(X86Test.class, method.asStringOption(), "");
        }

		return buildParameters("litmus/X86/", "cat/tso.cat", Arch.TSO);
    }

    public X86Test(String path, Result expected, Arch target, Wmm wmm, Settings settings) {
