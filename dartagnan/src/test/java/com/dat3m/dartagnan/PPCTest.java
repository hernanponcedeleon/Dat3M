...
public class PPCTest extends AbstractDartagnanTest {

    @Parameterized.Parameters(name = "{index}: {0} {4}")
    public static Iterable<Object[]> data() throws IOException {
    	// We want the files to be created every time we run the unit tests
        for(Method method : Method.values()) {
        	initialiseCSVFile(PPCTest.class, method.asStringOption(), "");
        }

        return buildParameters("litmus/PPC/", "cat/power.cat", Arch.POWER);
    }

    public PPCTest(String path, Result expected, Arch target, Wmm wmm, Settings settings) {
