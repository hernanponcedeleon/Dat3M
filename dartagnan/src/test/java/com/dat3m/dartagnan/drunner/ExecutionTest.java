...
public class ExecutionTest extends AbstractTest {
    ...
    private static final Object[][] TESTS = {
            {"checks-litmus-vulkan", "litmus-vulkan", "spirv-check", CAT_SPEC},
            {"races-litmus-vulkan", "litmus-vulkan", "spirv", CAT_SPEC},
            {"spec-litmus-vulkan", "litmus-vulkan", "spirv", PROGRAM_SPEC},
        ...
        for (Object[] params : TESTS) {
            ...
            for (String path : listFiles(rootPath)) {
                try (SolverContext ctx = mkCtx(); ProverWithTracker prover = mkProver(ctx)) {
                    ...
                    try {
                        Result result = AssumeSolver.run(ctx, prover, mkTask(modelPath, path, property)).getResult();
