...
public class VisitorOpsMemoryTest {
    ...
    @Test
    public void testCopyScalarMemory() {
        // given
        ...
        // The mocked program may contain setup events, so access the events generated by OpCopyMemory from the end.
        Load load = (Load) events.get(events.size() - 2);
        Store store = (Store) events.get(events.size() - 1);
        ...
        assertTrue(load.hasTag(Tag.Spirv.SC_WORKGROUP));
        assertTrue(store.hasTag(Tag.Spirv.SC_WORKGROUP));
        assertFalse(load.hasTag(Tag.Spirv.MEM_VOLATILE));
        assertFalse(store.hasTag(Tag.Spirv.MEM_VOLATILE));
    }
    ...
    public void testCopyMemoryBetweenStorageClasses() {
        ...
        Store store = (Store) events.get(events.size() - 1);
        assertTrue(load.hasTag(Tag.Spirv.SC_WORKGROUP));
        assertTrue(store.hasTag(Tag.Spirv.SC_UNIFORM));
    }

    @Test
    public void testCopyMemoryWithSharedMemoryOperands() {
        // given
        String input = "OpCopyMemory %target %source Volatile";
        builder.mockIntType("%int", 32);
        builder.mockPtrType("%int_ptr", "%int", "Workgroup");
        ...
        Load load = (Load) events.get(events.size() - 2);
        Store store = (Store) events.get(events.size() - 1);
        assertTrue(load.hasTag(Tag.Spirv.MEM_VOLATILE));
        assertTrue(store.hasTag(Tag.Spirv.MEM_VOLATILE));
    }
    ...
    public void testCopyMemoryWithSharedAvailabilityAndVisibility() {
        ...
        Load load = (Load) events.get(events.size() - 2);
        Store store = (Store) events.get(events.size() - 1);
        assertTrue(load.hasTag(Tag.Spirv.MEM_NON_PRIVATE));
        assertTrue(load.hasTag(Tag.Spirv.MEM_VISIBLE));
        assertTrue(load.hasTag(Tag.Spirv.WORKGROUP));
        assertFalse(load.hasTag(Tag.Spirv.MEM_AVAILABLE));
        assertFalse(load.hasTag(Tag.Spirv.DEVICE));
        assertTrue(store.hasTag(Tag.Spirv.MEM_NON_PRIVATE));
        assertTrue(store.hasTag(Tag.Spirv.MEM_AVAILABLE));
        assertTrue(store.hasTag(Tag.Spirv.DEVICE));
        assertFalse(store.hasTag(Tag.Spirv.MEM_VISIBLE));
        assertFalse(store.hasTag(Tag.Spirv.WORKGROUP));
    }

    @Test
    public void testCopyMemoryWithSeparateMemoryOperands() {
        // given
        String input = "OpCopyMemory %target %source Nontemporal Volatile";
        builder.mockIntType("%int", 32);
        builder.mockPtrType("%int_ptr", "%int", "Workgroup");
        builder.mockVariable("%target", "%int_ptr");
        builder.mockVariable("%source", "%int_ptr");
        ...
        Store store = (Store) events.get(events.size() - 1);
        assertTrue(load.hasTag(Tag.Spirv.MEM_VOLATILE));
        assertFalse(load.hasTag(Tag.Spirv.MEM_NONTEMPORAL));
        assertTrue(store.hasTag(Tag.Spirv.MEM_NONTEMPORAL));
        assertFalse(store.hasTag(Tag.Spirv.MEM_VOLATILE));
    }
    ...
    public void testCopyMemoryRejectsVisibleTargetMask() {
        // given
        String input = "OpCopyMemory %target %source "
                + "NonPrivatePointer|MakePointerVisible %device Volatile";
        builder.mockIntType("%int", 32);
        builder.mockPtrType("%int_ptr", "%int", "Workgroup");
        builder.mockVariable("%target", "%int_ptr");
        builder.mockVariable("%source", "%int_ptr");
    ...
    public void testCopyMemoryUsesUniqueRegisters() {
        ...
        List<Event> events = builder.getCurrentFunction().getEvents();
        // The mocked program may contain setup events, so access the events generated by OpCopyMemory from the end.
        Load firstLoad = (Load) events.get(events.size() - 4);
        Store firstStore = (Store) events.get(events.size() - 3);
        Load secondLoad = (Load) events.get(events.size() - 2);
        Store secondStore = (Store) events.get(events.size() - 1);
