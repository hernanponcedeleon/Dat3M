...
public class VisitorOpsExtensionTest {
    ...
    public void testSupportedInstruction() {
        ...
        MemoryObject memObj = builder.mockVariable("%var", "%ptr_1x_v3uint");

        // when
        visit(input);

        // then
        assertEquals(1, ((IntLiteral) memObj.getInitialValue(0)).getValueAsInt());
        assertEquals(1, ((IntLiteral) memObj.getInitialValue(4)).getValueAsInt());
        assertEquals(1, ((IntLiteral) memObj.getInitialValue(8)).getValueAsInt());
    }
